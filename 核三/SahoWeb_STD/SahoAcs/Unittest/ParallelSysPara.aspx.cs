...
{
    public partial class ParallelSysPara : System.Web.UI.Page
    {
        OrmDataObject odo = new OrmDataObject("MsSql", Pub.GetDapperConnString());

        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request["ParaNo"] != null)
            {
                string ParaNo = Request["ParaNo"];
                foreach(var o in this.odo.GetQueryResult("SELECT * FROM B00_SysParameter WHERE ParaNo=@ParaNo",new { ParaNo = ParaNo }))
                {
                    ...
                    {
                        doc.SetElementValue(ParaNo, Convert.ToString(o.ParaValue));
                        doc.Save(System.Web.HttpContext.Current.Server.MapPath("~/SysPara.xml"));
                    }
                }
