...
namespace SahoAcs
{
    public partial class CardFloor2 : Sa.BasePage
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (hFloorData != null && hFloorData.ContainsKey(Request.QueryString["EquID"]))
                {
                    ...
                    if (Request.QueryString["CardID"]!=null)
                    {
                        ...
                        string floor_data = oAcsDB.GetStrScalar(@"SELECT 
	                                                CardExtData 
                                                FROM  
	                                                B01_Card C
	                                                INNER JOIN B01_CardAuth B ON C.CardNo=B.CardNo WHERE CardID=? AND EquID=? ", paras);
