...
function LoadCardData() {
    ...
    for (var i = 0; i < popB_CardList2.options.length; i++) {
        ...
        hDataList.value += popB_CardList2.options[i].value+"|";
    }
    //透過非同步產生要做重整的卡號資訊
    $.ajax({
        type: "POST",
        url: "CardEquAdj4.aspx",
        dataType: "json",
        data: { "PageEvent": "LoadCardData", "Condition": $('[name*="hSelectState"]').val(), "DataList": $('[name*="hDataList"]').val() },
        success: function (data) {
            hDataList.value = data.datalist;
...
function SetEquType(val) {
    $('[name*="hEquType"]').val(val);
}
...
function StartRefresh() {
    ...
    $('[name*="$List_Msg"] option').remove();
    var proc_cnt = 0;
    var EquType = "CardEquAdj";
    if ($('[name*="hEquType"]').val() == "EquGrp") {
        EquType="CardEquGroup"
    }
    card_list.forEach(function (ele) {
        ...
        $.ajax({
            ...
            data: {
                "PageEvent": EquType,
                "VerifiMode": ele.VerifiMode, "FinalEquData": $('[name*="FinalEquData"]').val(), "CardNo": ele.CardNo, "CardID": ele.CardID, "OpType": $('[name*="OpType"]').val(),
                "ddlState": $('[name*="hddlState"]').val(), "FloorData": $('[name*="hFinalFloorData"]').val(), "UserID": $('[name*="hUserId"]').val()
            },
            success: function (data) {
                console.log(ele.CardNo + "........." + data.message);
                proc_cnt++;
                $('[name*="List_Msg"]').prepend($("<option></option>").text("重整完成...." + ele.CardNo + "....end" + proc_cnt));
                if (proc_cnt == (card_list.length)) {
                    $('[name*="List_Msg"]').prepend($("<option></option>").text("總共完成筆數...." + proc_cnt));
                    $.unblockUI();
                }
            },
            fail: function () {
...
function RemoveFloor() {
        ...
        card_list.forEach(function (ele) {        
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {
                    "PageEvent": "RemoveFloorData",
                    "FinalEquData": $('[name*="FinalEquData"]').val(), "CardNo": ele.CardNo, "CardID": ele.CardID,"OpType":$('[name*="OpType"]').val(),
                    "ddlState": $('[name*="hddlState"]').val(), "FloorData": $('[name*="hFinalFloorData"]').val(), "UserID": $('[name*="hUserId"]').val()
                },
                success: function (data) {
                    console.log(ele.CardNo + "........." + data.message);
                    proc_cnt++;
                    $('[name*="List_Msg"]').prepend($("<option></option>").text("重整完成...." + ele.CardNo + "....+" + proc_cnt));
                    if (proc_cnt == (card_list.length)) {
                        $('[name*="List_Msg"]').prepend($("<option></option>").text("總共完成筆數...." + proc_cnt));
                        $.unblockUI();
                    }
