...
namespace SahoAcs
{
    public partial class ResetCardAuth : System.Web.UI.Page
    {
        ...
        public void QueryCardList()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    td = new TableCell();
                    td.Width = 250;
                    td.Text = dt.Rows[i]["PsnName"] + " - [" + dt.Rows[i]["CardNo"] + "]";
        ...
        protected void ResetAuthButton_Click(object sender, EventArgs e)
        {
            ...
            string sSql = "";
            List<string> liSqlPara = new List<string>();
            List<string> reproc_list = new List<string>();
            DB_Acs oAcsDB = new DB_Acs(this.Page, Pub.GetConnectionString(Pub.sConnName));
            ...
            foreach (DataRow dr in PersonListDataTable.Rows)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    reproc_list.Add(PersonListDataTable.Rows[iCount]["CardNo"].ToString());
                }
                iCount++;
            }
            //失敗卡號重新停用再啟用的同步            
            foreach (var card_no in reproc_list)
            {                                
                try
                {
                    liSqlPara.Clear();
                    sSql = "UPDATE B01_Card SET CardAuthAllow=0 WHERE CardNo=? ;";
                    liSqlPara.Add("S:" + card_no);                                        
                    sSql+= "EXEC dbo.CardAuth_Update ?, ? ,'ResetCardAuth:ExcuteCardAuthReset','','卡片權限重整'; ";
                    liSqlPara.Add("S:" + card_no);
                    liSqlPara.Add("S:" + this.hideUserID.Value);
