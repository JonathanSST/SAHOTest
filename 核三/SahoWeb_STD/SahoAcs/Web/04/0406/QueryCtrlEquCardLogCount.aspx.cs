...
namespace SahoAcs
{
    public partial class QueryCtrlEquCardLogCount : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            string js = @"<script src='/scripts/vue.min.js'></script>
                        <script src='QueryCtrlEquCardLogCount.js'></script>"; 
        ...
        private void QueryEquConn()
        {
            ...
            {
                this.EquDatas = this.od_saho.GetQueryResult<EquDataModel>("SELECT * FROM B01_EquData WHERE EquID IN @ids",
        ...
        private void GetResult()
        {
            ...
            try
            {
                this.EquDatas = this.od_saho.GetQueryResult<EquDataModel>("SELECT * FROM B01_EquData WHERE 1=1 ").ToList();
                if (Request["EquList[]"] != null && Request["EquList[]"].ToString().Split(',').Length > 0)
                {                    
                    var eququery = this.od_saho.GetQueryResult<EquDataModel>("SELECT * FROM B01_EquData WHERE EquID IN @ids",
                        new { ids = Request["EquList[]"].ToString().Split(',') }).ToList();
                    message = "Query...";
                    oplogs = this.od_saho.GetQueryResult<B00Sysdeviceoplog>(@"SELECT TOP 100 * FROM B00_SysDeviceOPLog 
                                                                                                    WHERE DOPActive='CardLogCountGetSP' AND EquNo IN @nos AND CreateUserID=@UserID
