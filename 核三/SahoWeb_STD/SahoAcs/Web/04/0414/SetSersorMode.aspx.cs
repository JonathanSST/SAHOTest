...
namespace SahoAcs
{
    public partial class SetSersorMode : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Debug.WriteLine(hSaveGVRCheckStateList.Value);
        }
        #endregion
        ...
        private void RegisterComponeAndScript()
        {
            ...
            ClientScript.RegisterClientScriptInclude("SetSersorMode", "SetSersorMode.js?jsdate=" + DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
            #endregion
        }
        ...
        private void RegisterWindowsAndButton()
        {
            ...
            SetButton.Attributes["onClick"] = "SetStatus();";
        ...
        private void SetDDLCtrlItemList()
        {
            ...
            try
            {
                ...
                sSql = @"SELECT distinct CtrlModel FROM B01_IOMaster";

                oAcsDB.GetDataTable("ListTable", sSql, out dt);
                #endregion
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Item.Selected = false;
                    Item.Text = Convert.ToString(dt.Rows[i]["CtrlModel"].ToString().Trim());
                    Item.Value = Convert.ToString(dt.Rows[i]["CtrlModel"].ToString().Trim());
                    if (sCtrlModel.Value == dt.Rows[i]["CtrlModel"].ToString().Trim())
                    {
                        Item.Selected = true;
                    }
                    this.ddlCtrlModel.Items.Add(Item);
                }

                this.ddlCtrlModel.Items.Insert(0, new ListItem("選取資料", ""));
            }
            finally
            {
                oAcsDB = null;
        ...
        protected void GridView_Data_DataBound(object sender, EventArgs e)
        {
            ...
            {
                sSenID = oGridViewRow.Cells[1].Text.Trim();
                if (m_GVRCheckStateHashtable.ContainsKey(sSenID)) { ((CheckBox)oGridViewRow.Cells[0].Controls[1]).Checked = (m_GVRCheckStateHashtable[sSenID].ToString() == "1") ? true : false; }
            }
            #endregion
        }
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    ...
                    e.Row.Cells[1].Width = 50;   //識別碼
                    e.Row.Cells[2].Width = 120;  //IO編號
                    e.Row.Cells[3].Width = 120;   //Sen編號
                    e.Row.Cells[4].Width = 200;   //Sen名稱
                    e.Row.Cells[5].Width = 220;  //位置
                    //e.Row.Cells[6].Width = 120;  //目前狀態
                    //e.Row.Cells[7].Width = 40;   //自訂：傳送狀態
                    //e.Row.Cells[8].Width = 150;  //自訂：回應狀態(此部份不用設定)
                    #endregion
                    ...
                    //cbxRow.Attributes.Add("OnClick", "ChangeAllRowCheckState(this);");
                ...
                case DataControlRowType.DataRow:
                    ...
                    e.Row.Cells[1].Width = 54;   //識別碼
                    e.Row.Cells[2].Width = 125;  //IO編號
                    e.Row.Cells[3].Width = 124;   //Sen編號
                    e.Row.Cells[4].Width = 204;   //Sen名稱
                    e.Row.Cells[5].Width = 224;  //位置
                    //e.Row.Cells[6].Width = 124;  //目前狀態
                    //e.Row.Cells[7].Width = 44;   //自訂：傳送狀態
                    //e.Row.Cells[8].Width = 154;  //自訂：回應狀態(此部份不用設定)
                    #endregion
                    ...
                    e.Row.Cells[2].HorizontalAlign = HorizontalAlign.Center;  //IO編號
                    e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Center;  //Sen編號
                    //e.Row.Cells[7].HorizontalAlign = HorizontalAlign.Center;  //10.自訂：指令狀態
                    #endregion

                    #region B-4.限制欄位資料的長度 & 文字顏色
                    for (int x = 0; x < e.Row.Cells.Count; x++)
                    {
                        ...
                        if (x == 6)
                        {
                            e.Row.Cells[x].ForeColor = Color.Red;
                            if (e.Row.Cells[x].Text == "啟用") e.Row.Cells[x].ForeColor = Color.Black;
                        }
                    }
                    #endregion
        ...
        private void GirdViewDataBind(GridView oGridView, DataTable dt)
        {
            if (dt.Rows.Count != 0)
            {
                ...
                oGridView.DataKeyNames = new string[] { "SenID" };
                oGridView.DataBind();
                #endregion
            }
            else
        ...
        private Hashtable GetGVRCheckStateHashtable(string sGVRCheckStateList)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(sGVRCheckStateList))
                {
                    ...
                    {
                        htResult.Clear();
                        for (int i = 0; i < sGVCheckStateArray.Length; i += 2) { htResult.Add(sGVCheckStateArray[i], sGVCheckStateArray[i + 1]); } //.Split(new char[] { ',' })[2]
                    }
                }
            }
            finally
        ...
        private int Query(bool bIsResetGVCheckState, string sQueryMode, bool isFirst = false)
        {
            DB_Acs oAcsDB = null;
            ...
            try
            {
                ...
                #region 設定查詢條件
                string sCtrl = this.sCtrlModel.Value.ToString().Trim();
                ...
                if (!string.IsNullOrEmpty(sCtrl))
                {
                    sWhereSql += "AND ( IoMst.CtrlModel = '" + sCtrl + "' )";
                }

                if (!string.IsNullOrEmpty(sArea))
                {
                    sWhereSql += "AND ( dbo.Get_LocParentID(Sen.LocID, 'AREA') = " + sArea + " )";
                }

                if (!string.IsNullOrEmpty(sBuilding))
                {
                    sWhereSql += "AND ( dbo.Get_LocParentID(Sen.LocID, 'BUILDING') = " + sBuilding + " )";
                }

                if (!string.IsNullOrEmpty(sFloor))
                {
                    sWhereSql += "AND ( dbo.Get_LocParentID(Sen.LocID, 'FLOOR') = " + sFloor + " )";
                }
                #endregion
                ...
                //if (sWhereSql != "") { sWhereSql = " AND " + sWhereSql; }

                //設定排序條件
                sSql += (sWhereSql + " ORDER BY Sen.SenNo;");

                oAcsDB.GetDataTable("DataTable", sSql, out m_MainGridViewDataTable);

                this.m_MainGridViewDataTable.Columns.Add(new DataColumn("RowStatus"));
                foreach (DataRow r in this.m_MainGridViewDataTable.Rows)
                {
                    if (r["IOMstStatus"].ToString() == "0" || r["IOMstStatus"].ToString() == "2")
                    {
                        r["RowStatus"] = "連線裝置限制或停用";
                    }
                    else if (r["IOMStatus"].ToString() == "0" || r["IOMStatus"].ToString() == "2")
                    {
                        r["RowStatus"] = "IO模組限制或停用";
                    }
                ...
                if (bIsResetGVCheckState)
                {
                    ...
                    if ((m_MainGridViewDataTable != null) && (m_MainGridViewDataTable.Rows.Count > 0))
                    {
                        foreach (DataRow dataRow in m_MainGridViewDataTable.Rows)
                        {
                            ...
                            sGVRCheckStateList += (dataRow["SenID"].ToString() + "/" + "0");
                            //this.CheckIDList.Value += dataRow["IOMstID"].ToString() + "," + dataRow["IOMID"].ToString() + "," + dataRow["SenID"].ToString() + "/";
                        }
                    }

                    hSaveGVHCheckState.Value = "false";
                    this.hSaveGVRCheckStateList.Value = sGVRCheckStateList;
                }
                #endregion
                ...
                if (sQueryMode == "popPagePost")
                {
                    ...
                    for (int i = 0; i < m_MainGridViewDataTable.Rows.Count; i++) { if (hSelectValue.Value == m_MainGridViewDataTable.Rows[i]["SenID"].ToString()) { iFind = i; break; } }
        ...
        ///// </summary>
        ///// <param name="sSenID">設備識別碼</param>
        ///// <returns>string[] LoadViewWindosData</returns>
        //[System.Web.Services.WebMethod()]
        //[System.Web.Script.Services.ScriptMethod()]
        //public static string[] LoadViewWindowData(string sSenID)
        //{
        //    DB_Acs oAcsDB = null;
        ...
        //        sSql = @"SELECT DISTINCT EquData.SenID, EquData.EquNo, EquData.EquName, EquData.EquEName, EquData.IsAndTrt,
        ...
        //            WHERE EquData.SenID = '" + sSenID.Trim() + "';";
        ...
        protected void RefreshButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (GridViewRow oGridViewRow in MainGridView.Rows)
                {
                    sSenID = oGridViewRow.Cells[1].Text;
        ...
        {
            string[] sSenIDArray = null, sGVRCheckStateArray = null;
            DB_Acs oAcsDB = null;
            DataTable dt = null;

            try
            {
                string sSql = "", sCmdStrList = "", sSenIDList = "";
                ...
                if (sGVRCheckStateArray.Length >= 2)
                {
                    for (int i = 1; i < sGVRCheckStateArray.Length; i += 2)
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(sSenIDList)) { sSenIDList += ","; }

                            sSenIDList += sGVRCheckStateArray[i - 1];
                        }
                    }
                }
                #endregion
                ...
                if (!string.IsNullOrEmpty(sSenIDList))
                {
                    oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));

                    #region SQL：SELECT B01_EquData
                    sSql = " SELECT SenID,SenNo";
                    sSql += " FROM B01_Sensor";
                    sSql += " ORDER BY SenID ASC; ";
                    ...
                    {
                        sSenIDArray = sSenIDList.Split(',');

                        for (int i = 0; i < sSenIDArray.Length; i++)
                        {
                            foreach (DataRow dataRow in dt.Rows)
                            {
                                if (sSenIDArray[i].Trim() == dataRow["SenID"].ToString().Trim())
                                {
                                    if (sCmdStrList != "") { sCmdStrList += ";"; }
                                    sCmdStrList += dataRow["SenID"].ToString().Trim();
        ...
        protected void SetButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<string> sSenIDList = GetCheckBoxItemList();

                if (sSenIDList.Count > 0)
                {
                    string whereSql = "";
                    foreach (string s in sSenIDList)
                    {
                        if (!string.IsNullOrEmpty(whereSql)) whereSql += ",";
                        whereSql += " ?";
                    }

                    sSql = @"select SenID, SenStatus, Iom.IOMID, Iom.IOMStatus, IoMst.IOMstID, IoMst.IOMstStatus 
                                from B01_Sensor Sen
                                left join B01_IOModule Iom ON Sen.IOMID = Iom.IOMID
                                left join B01_IOMaster IoMst ON Iom.IOMstID = IoMst.IOMstID
                                where SenID IN (" + whereSql + ") ";
                    foreach (string s in sSenIDList)
                    {
                        liSqlPara.Add("I:" + s);
                    }

                    oAcsDB.GetDataTable("IOTable", sSql, liSqlPara, out dt);
                }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        sSql2 = "";
                        if (sSenStatus.Value == "1")
                        {
                            if (sSenStatus.Value != dr["SenStatus"].ToString().Trim())
                            {
                                sSql2 += "UPDATE B01_Sensor SET SenStatus=@sStatus, isUpdate=1, UpdateUserID=@UserID, UpdateTime=getdate() WHERE SenID=@SenID; ";
                            }
                            if (sSenStatus.Value != dr["IOMStatus"].ToString().Trim())
                            {
                                sSql2 += "UPDATE B01_IOModule SET IOMStatus=@sStatus, isUpdate=1, UpdateUserID=@UserID, UpdateTime=getdate() WHERE IOMID=@IOMID; ";
                            }
                            if (sSenStatus.Value != dr["IOMstStatus"].ToString().Trim())
                            {
                                sSql2 += "UPDATE B01_IOMaster SET IOMstStatus=@sStatus, isUpdate=1, UpdateUserID=@UserID, UpdateTime=getdate() WHERE IOMstID=@IOMstID; ";
                            }

                            if (!string.IsNullOrEmpty(sSql2))
                            {
                                var effectRow = this.odo.Execute(sSql2, new
                                {
                                    sStatus = sSenStatus.Value,
                                    UserID = Sa.Web.Fun.GetSessionStr(this.Page, "UserID"),
                                    SenID = dr["SenID"].ToString().Trim(),
                                    IOMID = dr["IOMID"].ToString().Trim(),
                                    IOMstID = dr["IOMstID"].ToString().Trim()
                                });
                            }
                        }
                        else
                    ...
                    ScriptManager.RegisterStartupScript(this, GetType(), "SelectState", "SelectState();", true);
        ...
        private List<string> GetCheckBoxItemList()
        {
        ...
        //private List<string> GetGVRCheckArray()
        //{
        //    string[] sGVRCheckStateArray = null;
        //    List<string> sItemList = new List<string>();
        //    string sSenIDList = "";

        //    #region 取得被勾選的Sersor清單
        //    sGVRCheckStateArray = hSaveGVRCheckStateList.Value.Split('/');
        ...
        //                //sSenIDList += sGVRCheckStateArray[i - 1];
        //                sItemList.Add(sGVRCheckStateArray[i - 1]);
        //            }
        //        }
        //    }
        //    #endregion
