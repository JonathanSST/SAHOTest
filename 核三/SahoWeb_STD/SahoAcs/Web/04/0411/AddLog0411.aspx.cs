...
{
    public partial class AddLog0411 : System.Web.UI.Page
    {
        ...
        public void SetInit()
        {
            try
            {
                ...
                //預設為當天的日期
                this.NewCalendar1.DateValue = DateTime.Now.ToString("yyyy/MM/dd");
            }
            catch (Exception ex)
            {
        ...
        public void SaveData()
        {
            ...
            if (list.Count() > 0)
            {
                if (this.odo.GetQueryResult(@"SELECT CardNo,SyncMark3 FROM B01_CardLog WHERE CardNo=@CardNo AND SyncMark3=@Status AND CONVERT(VARCHAR(10),CardTime,111)=@CardTime
                    UNION SELECT CardNo,SyncMark3 FROM B01_CardLogFill WHERE CardNo=@CardNo AND SyncMark3=@Status AND CONVERT(VARCHAR(10),CardTime,111)=@CardTime"
                    , new {CardNo=list.First().CardNo,Status=status,CardTime=cardtime }).Count() > 0)
                {
                    Response.Clear();
                    Response.Write("用餐記錄重複補登");
                    Response.End();
                }
                ...
                data.CardTime = cardtime+" " + CardTimeExt;
                sql = @" INSERT INTO B01_CardLogFill (LogTime, CardTime, CardNo, CardVer, PsnNo, PsnName, OrgStruc
                    ...
                    ,SyncMark3, CardType) VALUES (getdate(), @CardTime, @CardNo, @CardVer, @PsnNo, @PsnName, @OrgStruc
                     ,@EquClass, @EquNo, @EquName, @EquDir, @MstConnParam, @CtrlAddr, @ReaderNo, @IsAndTrt, @CardInfo, @VerifyMode
                     ,@LogStatus, @CardType) ";
