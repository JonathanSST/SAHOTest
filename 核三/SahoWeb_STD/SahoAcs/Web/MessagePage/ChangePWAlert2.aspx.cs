...
{
    public partial class ChangePWAlert2 : System.Web.UI.Page
    {
        #region Events
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("ChangePWAlert", "ChangePWAlert2.js");//加入同一頁面所需的JavaScript檔案
            ...
            #endregion
            if (Sa.Web.Fun.GetSessionStr(this.Page, "UserID").Equals(""))
            {
        ...
        public static object ChangePW(string UserID, string OldPWD, string NewPWD, string CheckPWD)
        {
            ...
            if (objRet.result)
            {
                ...
                #region Process String - Updata Pwd
                sql = @" UPDATE B00_SysUser 
                         SET UserPW = ?, UserPWCtrl = ?, PWChgTime = ?, 
                         UpdateUserID = ?, UpdateTime = ?
                         WHERE UserID = ? ";
                var page = HttpContext.Current.Handler as Page;
                if (!string.IsNullOrEmpty(Sa.Web.Fun.GetSessionStr(page, "PsnID"))){
                    sql = "UPDATE B01_Person SET PsnPW=?, UpdateUserID=?, UpdateTime=?, Text2=? WHERE PsnID=?";
                    liSqlPara.Add("S:" + NewPWD.Trim());                                        
                    liSqlPara.Add("S:" + UserID.Trim());
                    liSqlPara.Add("D:" + Time);
                    liSqlPara.Add("D:" + Time.ToString("yyyy/MM/dd"));
                    liSqlPara.Add("S:" + Sa.Web.Fun.GetSessionStr(page,"PsnID"));
                }
                else
                {
                    liSqlPara.Add("S:" + NewPWD.Trim());
