...
namespace SahoAcs
{
    public partial class EquGroupFloorSetting : System.Web.UI.Page
    {
        ...
        DapperDataObjectLib.OrmDataObject odo = new OrmDataObject("MsSql",Pub.GetDapperConnString());
        public string EquIndex = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            */
            ClientScript.RegisterClientScriptInclude("EquGroupFloorSetting", "EquGroupFloorSetting.js");//加入同一頁面所需的JavaScript檔案
            #region 語系切換
            ...
            if (!IsPostBack)
            {
                ...
                if (Request["card_ext_data"] !=null&&!Request["card_ext_data"].ToString().Equals(""))
                {
                    this.ext_data_bin = Sa.Change.HexToBin(Request["card_ext_data"], 48);
                }
                if (Request["equ_index"] != null)
                {
                    this.EquIndex = Request["equ_index"];
                }
                if (Request["equ_name"] != null)
                {
                    this.EquName = Request["equ_name"].ToString();
                }
            }
            else
            {
