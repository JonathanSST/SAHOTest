var objForm = document.forms["formPop"];
...
// 預先讀取資料
function LoadData() {
    //var vArguments = $("#hideParaValue").val();
    ...
    if (hideParaValue.value != "") {
        ...
        BlockLengthstr = parseInt(BlockLengthstr, 16).toString(10);
        BlockLengthstr = padLeft(BlockLengthstr, 2);
        //if (Buzzerstr == "AA")
        //    Buzzerstr = "0";
        //else
        //    Buzzerstr = "1";
        ...
        $("#BlockLength").val(BlockLengthstr);
        $("#Buzzer").val(Buzzerstr);

        //直接設定變數至Block位置及蜂鳴器模式

    } else {
        $("input[name='LoadMode'][value='0A']").prop('checked', true);
        $("input[name='Face'][value='0A']").prop('checked', true);
    }
    GetBuzzerStr();
    GetBlockLengthStr();
    Def();
...
function GetBlockLengthStr() {
    //PageMethods.set_path('/web/02/ParaPop/ReaderParaPop.aspx');
    $.ajax({
        type: "POST",
        url: "/Web/02/ParaPop/ReaderParaPop.aspx",
        dataType: "json",
        data: { "PageEvent": "GetBlockLength", "BlockLength": $("#BlockLength").val() },
        success: function (data) {
            //alert(data.Message);
            SetBlockLengthUI(data);
        },
        fail: function () {
...
function GetBuzzerStr() {
    ...
    $.ajax({
        ...
        url: "/Web/02/ParaPop/ReaderParaPop.aspx",
        dataType: "json",
        data: { "PageEvent": "GetBuzzer", "Buzzer": $("#Buzzer").val() },
        success: function (data) {
            SetBuzzerUI(data);
        },
        fail: function () {
...
function DoSave() {    
    var CheckObj =  {
        "Target": hideTarget.value,
        "Face": $('input[name*="Face"]:checked').val(),
        "LoadMode": $('input[name*="LoadMode"]:checked').val(),
    ...
    $.ajax({
        type: "POST",
        url: "/Web/02/paraPop/ReaderParaPop.aspx",
        data: "PageEvent=CheckInput&PostData=" + JSON.stringify(CheckObj),
        dataType: "json",
        success: function (data) {
            if (data.result != true) {
                alert(data.message);
            } else {
                var result_val = $('input[name*="Face"]:checked').val()
                + $('input[name*="LoadMode"]:checked').val()
                + padLeft(parseInt($("#BlockLength").val()).toString(16), 2).toUpperCase()
                + $("#Buzzer").val() + "AA";
