...
namespace SahoAcs
{
    public partial class EquSettingOnFloor : System.Web.UI.Page
    {
        #region 一.宣告
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            CreateQueryCtrlEquTimePageComponent();
        }
        #endregion
        ...
        private void RegisterComponeAndScript()
        {
            ...
            ClientScript.RegisterClientScriptInclude("EquSettingOnFloor", "EquSettingOnFloor.js");//加入同一頁面所需的JavaScript檔案
            #endregion
        }
        ...
        private void RegisterWindowsAndButton()
        {
            ...
            ComplexQueryButton.Attributes["onClick"] = "CallComplexQueryWindow('" + "設定「複合查詢」條件資料" + "'); return false;";
            ViewButton.Attributes["onClick"] = "CallViewWindow('" + "檢視「控制器」設備資料" + "', '" +
        ...
        /// 建立控制器時間查詢網頁的相關元件
        /// </summary>
        private void CreateQueryCtrlEquTimePageComponent()
        {
            DB_Acs oAcsDB = null;
            DataTable dt  = null;
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    ...
                    e.Row.Cells[0].Width  = 100;  //設備編號
                    e.Row.Cells[1].Width  = 150;  //護備名稱
                    e.Row.Cells[2].Width  = 150;  //護備英文名稱                   
                    e.Row.Cells[3].Width  = 90;   //設備型號                   
                    e.Row.Cells[4].Width  = 40;   //樓層
                    e.Row.Cells[5].Width  = 100;  //建築物名稱
                    //e.Row.Cells[6].Width = 40;   //自訂：傳送狀態
                    //e.Row.Cells[11].Width = 150;  //自訂：回應狀態(此部份不用設定)
                    #endregion
                ...
                case DataControlRowType.DataRow:
                    ...
                    e.Row.Cells[0].Width = 103;  //設備編號
                    e.Row.Cells[1].Width = 154;  //護備名稱
                    e.Row.Cells[2].Width = 154;  //護備英文名稱                   
                    e.Row.Cells[3].Width = 94;   //設備型號                   
                    e.Row.Cells[4].Width = 44;   //樓層
                    e.Row.Cells[5].Width = 104;  //建築物名稱
                    #endregion
                    ...
                    //e.Row.Cells[5].HorizontalAlign  = HorizontalAlign.Center;  //5.是否兼做卡鐘
                    //e.Row.Cells[6].HorizontalAlign  = HorizontalAlign.Center;  //6.設備型號
                    //e.Row.Cells[7].HorizontalAlign  = HorizontalAlign.Center;  //7.設備類別
                    //e.Row.Cells[8].HorizontalAlign  = HorizontalAlign.Center;  //8.樓層
                    //e.Row.Cells[10].HorizontalAlign = HorizontalAlign.Center;  //10.自訂：指令狀態
                    ...
                    //e.Row.Cells[3].Text = LimitText(e.Row.Cells[3].Text, 50, true);  //3.設備名稱
                    //e.Row.Cells[4].Text = LimitText(e.Row.Cells[4].Text, 50, true);  //4.設備英文名稱
                    //e.Row.Cells[9].Text = LimitText(e.Row.Cells[9].Text, 50, true);  //9.建築物名稱
                    #endregion
        ...
        private int Query(bool bIsResetGVCheckState, string sQueryMode)
        {
            ...
            try
            {
                ...
                sSql = @"SELECT DISTINCT EquData.EquID, EquData.EquNo, EquData.EquName, EquData.EquEName, EquData.IsAndTrt,
                    EquData.EquModel, EquData.EquClass, EquData.Floor, EquData.Building,
                    IsAndTrt.IsAndTrtName AS IsAndTrtName , ItemList.ItemName AS EquModelName
                ...
                else                                                     //關鍵字查詢
                {
                    ...
                    {                        
                        sWhereSql  = "((EquData.EquNo LIKE '%" + QueryKeyWord.Trim() + "%') OR ";
                        sWhereSql += "(EquData.EquName LIKE '%" + QueryKeyWord.Trim() + "%') OR ";
                        sWhereSql += "(EquData.EquEName LIKE '%" + QueryKeyWord.Trim() + "%') OR ";
                        sWhereSql += "(IsAndTrtName LIKE '%" + QueryKeyWord.Trim() + "%') OR ";
                        sWhereSql += "(ItemList.ItemName LIKE '%" + QueryKeyWord.Trim() + "%') OR ";
                        sWhereSql += "(EquData.EquClass LIKE '%" + QueryKeyWord.Trim() + "%') OR ";
                        sWhereSql += "(EquData.Floor = '" + QueryKeyWord.Trim() + "') OR ";
                        sWhereSql += "(EquData.Building LIKE '%" + QueryKeyWord.Trim() + "%'))";
                    }
                }              
                #endregion
        ...
        public static object SetComplexQueryWheresql(string[] sParamArray)
        {
            ...
            try
            {
                ...
                if ((sParamArray != null) && (sParamArray.Length > 0))
                {
                    ...
                    //6.樓層
                    if (!string.IsNullOrEmpty(sParamArray[6].Trim()))
                    {
                        if (sWhereSql != "") { sWhereSql += " AND "; }
                        sWhereSql += "(EquData.Floor = '" + sParamArray[6].Trim() + "')";
                    }

                    //7.建築物名稱
                    if (!string.IsNullOrEmpty(sParamArray[7].Trim()))
                    {
                        if (sWhereSql != "") { sWhereSql += " AND "; }
                        sWhereSql += "(EquData.Building LIKE '%" + sParamArray[7].Trim() + "%')";
                    }
                }
                #endregion
