...
namespace SahoAcs
{
    public partial class ReaderPara : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("ReaderPara", "ReaderPara.js");//加入同一頁面所需的JavaScript檔案
        ...
        public static object SaveData(string UserID, string EquID, string EquParaID, string Target, string FaceValue, string LoadModeValue, string BlockLengthValue, string BuzzerValue)
        {
            string ReaderParaStr = "";
            Pub.MessageObject objRet = new Pub.MessageObject();

            objRet = Check_Input(Target, FaceValue, LoadModeValue, BlockLengthValue, BuzzerValue);

            if (objRet.result)
            {
                BlockLengthValue = Convert.ToString(int.Parse(BlockLengthValue.Trim()), 16).ToUpper().PadLeft(2, '0');

                if (string.Compare(BuzzerValue, "00") == 0)
                    BuzzerValue = "AA";
                else
                    BuzzerValue = "55";

                ReaderParaStr = FaceValue + LoadModeValue + BlockLengthValue + BuzzerValue + "AA";
                objRet.act = "SaveData";
                objRet.message = ReaderParaStr;
            }
        ...
        #region Check_Input
        protected static Pub.MessageObject Check_Input(string Target, string Face, string LoadMode, string BlockLength, string Buzzer)
        {
            Pub.MessageObject objRet = new Pub.MessageObject();
            int tempint = 0;
            ...
            if (string.Compare(Target, "popL_LTitle") == 0)
            {
                ...
                if (!int.TryParse(Buzzer.Trim(), out tempint) && !string.IsNullOrEmpty(Buzzer))
                {
                    if (!string.IsNullOrEmpty(objRet.message)) objRet.message += "\n";
                    objRet.result = false;
                    objRet.message += "讀卡機蜂鳴器 為無效參數";
                }
                else if (int.TryParse(Buzzer.Trim(), out tempint))
                {
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static string[] GetBlockLengthStr(string BlockLength)
        {
            int BlockLengthInt;
            string[] StrItem = new string[2];
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static string[] GetBuzzerStr(string Buzzer)
        {
            int BuzzerInt;
            string[] StrItem = new string[2];
            if (int.TryParse(Buzzer.Trim(), out BuzzerInt))
            {
                if (BuzzerInt >= 0)
                {
                    #region Left Buzzer
                    if (BuzzerInt >= 0 && BuzzerInt < 3)
                        StrItem[0] = LBuzzer[BuzzerInt];
                    else
                        StrItem[0] = LBuzzer[2];
                    #endregion
                    #region Right Buzzer
