            ...
            if (IsSuccess)
            {
                for (int i = 0; i < PsnCard_set.Count; i++)
                {
                    ...
                    sSql += @" INSERT dbo.B00_SyncTable_PersonCard
                               (PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, Company, Unit, Department, Title, PsnAccount, PsnPW, PsnAuthAllow, PsnSTime, PsnETime, PsnPicSource, CardNo, CardVer, CardPW, CardSerialNo, Text1)
                               VALUES (" + sParaStr + ") ";
            ...
            if (PsnCardCorrectFlag && PsnTableCount > 0)
            {
                ...
                {                    
                    var sql_master = @"SELECT 
                              ...
                              PsnAccount, Text1, PsnPW, PsnAuthAllow, 
                              PsnSTime,CASE ISNULL(PsnETime,'') WHEN '' THEN '2099/12/31' ELSE PsnETime END AS PsnETime, PsnPicSource, 
                              'Saho', GETDATE()
                    ...
                    sSql = @"INSERT INTO B01_Person(PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, OrgStrucID, PsnAccount, Text1
                ...
                if (iRet > -1)
                {
                    sSql = @" INSERT INTO dbo.B01_Card (CardNo, CardVer, CardPW, CardSerialNo, CardType, PsnID, CardAuthAllow, CardSTime, CardETime, CreateUserID, CreateTime)
                              ...
                              PersonCard.PsnSTime, CASE ISNULL(PersonCard.PsnETime,'') WHEN '' THEN '2099/12/31' ELSE PersonCard.PsnETime END AS PsnETime,
                              'Saho', GETDATE()
                ...
                if (iRet > -1)
                {
                    ...
                    iRet = oAcsDB.SqlCommandExecute(sSql);
                    sSql = @"INSERT INTO B01_CardEquAdj (CardID,EquID,CreateUserID,OpMode,CardExtData,CardRule,CreateTime)
                                    SELECT DISTINCT
	                                    Card.CardID, OrgEquAdj.EquID,'Saho',OrgEquAdj.OpMode,OrgEquAdj.CardExtData,OrgEquAdj.CardRule,GETDATE()
                                    FROM 
	                                    dbo.B01_Person AS Person
                                        INNER JOIN dbo.B01_Card AS Card ON Card.PsnID = Person.PsnID
                                        INNER JOIN B01_OrgEquAdj AS OrgEquAdj ON OrgEquAdj.OrgStrucID = Person.OrgStrucID
	                                    LEFT JOIN B01_CardEquAdj AS B01_CardEquAdj ON Card.CardID=B01_CardEquAdj.CardID
                                    WHERE 
	                                    B01_CardEquAdj.CardID IS NULL";
                    iRet = oAcsDB.SqlCommandExecute(sSql);
                }
                #endregion   
        ...
        public static object ShowPsnCardErrorData()
        {
            ...
            if (ErrorCount == 0)
            {
                ...
                if (ErrorCount != 0)
                {
                    ...
                    objRet.message += "必填欄位請填寫(PsnNo、PsnType、Company、PsnAuthAllow、PsnSTime、CardNo、CardPW)：\n";
                    objRet.message += "-----====================================================================-----\n";
                }
            }
            #endregion
            ...
            if (ErrorCount == 0)
            {
                ...
                if (ErrorCount != 0)
                {
                    sSql = @" SELECT * FROM B00_SyncTable_PersonCard AS SyncTable_PersonCard
                              WHERE PsnType NOT IN ( SELECT ItemNo FROM dbo.B00_ItemList WHERE ItemClass = 'PsnType' ) ";
                    oAcsDB.GetDataTable("ErrorPsnCardData", sSql, out ErrorPsnCardData);
                    objRet.message += "必填欄位(PsnType)有誤：\n";
                    objRet.message += "-----====================================================================-----\n";
                }
            }
            #endregion
            ...
            if (ErrorCount == 0)
            {
                ...
                if (ErrorCount != 0)
                {
                    sSql = @" SELECT * FROM B00_SyncTable_PersonCard AS SyncTable_PersonCard
                              WHERE PsnAuthAllow NOT IN ('0','1') ";
                    oAcsDB.GetDataTable("ErrorPsnCardData", sSql, out ErrorPsnCardData);
                    objRet.message += "必填欄位(PsnAuthAllow)有誤：\n";
                    objRet.message += "-----====================================================================-----\n";
                }
            }
            #endregion
            ...
            if (ErrorCount == 0)
            {
                ...
                if (ErrorCount != 0)
                {
                    sSql = @" SELECT * FROM B00_SyncTable_PersonCard AS SyncTable_PersonCard
                              INNER JOIN B01_Person AS Person ON Person.PsnNo = SyncTable_PersonCard.PsnNo ";
                    oAcsDB.GetDataTable("ErrorPsnCardData", sSql, out ErrorPsnCardData);
                    objRet.message += "資料已存在於系統中(PsnNo)：\n";
                    objRet.message += "-----====================================================================-----\n";
                }
            }
            #endregion
            ...
            if (ErrorCount == 0)
            {
                ...
                if (ErrorCount != 0)
                {
                    sSql = @" SELECT * FROM B00_SyncTable_PersonCard AS SyncTable_PersonCard
                              INNER JOIN B01_Card AS Card ON Card.CardNo = SyncTable_PersonCard.CardNo ";
                    oAcsDB.GetDataTable("ErrorPsnCardData", sSql, out ErrorPsnCardData);
                    objRet.message += "資料已存在於系統中(CardNo)：\n";
                    objRet.message += "-----====================================================================-----\n";
                }
            }
            #endregion
            ...
            if (ErrorCount == 0)
            {
                ...
                if (ErrorCount != 0)
                {
                    sSql = @" SELECT *
                              ...
                              WHERE StrucAllData.OrgStrucID IS NULL ";
                    oAcsDB.GetDataTable("ErrorPsnCardData", sSql, out ErrorPsnCardData);
                    objRet.message += "人員卡片所設定的組織架構並不存在(OrgData、OrgStruc)：\n";
                    objRet.message += "-----====================================================================-----\n";
                }
            }
            #endregion
