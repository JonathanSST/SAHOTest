...
namespace SahoAcs
{
    public partial class UserV2 : BasePage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("User", "UserV2.js?"+Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
        ...
        void SetUpdate()
        {
            ...
            if (objRet.result)
            {
                var sql = @" UPDATE B00_SysUser SET UserID=@UserID, UserPW = @UserPW, UserName = @UserName, UserEName = @UserEName ,
        ...
        public void Query(string SortExpression, string SortDire)
        {
            ...
            if (cmd != "")
                cmd = " WHERE " + cmd;
            this.UserList = this.odo.GetQueryResult<B00Sysuser>(sql+cmd,new {UserID=p1, UserName=p2, OwnList = "%"+ OwnerList+UserID+@"\%"}).OrderByField(SortExpression, true).ToList();
        }
        #endregion
