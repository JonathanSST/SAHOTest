...
namespace SahoAcs
{
    public partial class ManageArea : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("ManageArea", "ManageArea.js?"+Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
        }
        #endregion
        ...
        private void SetInsertDataS()
        {
            ...
            if (objRet.result)
            {
                this.odo.Execute(@"INSERT INTO B00_ManageArea (MgaNo ,MgaName ,MgaEName ,MgaDesc ,Remark ,OwnerID ,OwnerList ,CreateUserID ,UpdateUserID ,UpdateTime,Email) VALUES (
                                                @MgaNo ,@MgaName ,@MgaEName ,@MgaDesc ,@Remark ,@OwnerID ,@OwnerList ,@UserID ,@UserID ,GETDATE(),@Email)", new
                {
                    MgaNo = Request["MgaNo"],
                    OwnerID = User,
                    ...
                    OwnerList,
                    UserID = User,
                    Email = Request["MgaEmail"].Replace("，", ",")
                });
                ...
                if (this.odo.isSuccess)
                {
                    ...
                    {
                        objRet.message = o.MgaID.ToString();

                        //20230617 TINA 新增管理區時，自動新增無組識 
                        string NewMga = "";
                        NewMga = this.odo.GetStrScalar("select top 1 MgaID from B01_MgnOrgStrucs where  MgaID = @mgaID and OrgStrucID = 1", new { MgaNo = o.MgaID });

                        if (NewMga == "")
                        {
                            this.odo.Execute(@"INSERT INTO B01_MgnOrgStrucs(MgaID, OrgStrucID, CreateUserID, CreateTime) 
                                           VALUES (@MgaID, @OrgStrucID, @UserID, GETDATE())", new
                            {
                                MgaID = o.MgaID,
                                OrgStrucID = 1,
                                UserID = User
        ...
        private void SetInsertData()
        {
            ...
            if (objRet.result)
            {
                var paramsOfInsert = new DynamicParameters();
                paramsOfInsert.Add("@MgaNo", Request["MgaNo"]);
                paramsOfInsert.Add("@OwnerID", User);
                paramsOfInsert.Add("@MgaEName", Request["MgaEName"]);
                paramsOfInsert.Add("@MgaName", Request["MgaName"]);
                paramsOfInsert.Add("@MgaDesc", Request["MgaDesc"]);
                paramsOfInsert.Add("@Remark", Request["Remark"]);
                paramsOfInsert.Add("@OwnerList", OwnerList);
                paramsOfInsert.Add("@UserID", User);
                paramsOfInsert.Add("@Email", Request["MgaEmail"].Replace("，", ","));
                paramsOfInsert.Add("@mgaID", dbType: DbType.Int64, direction: ParameterDirection.Output);
                paramsOfInsert.Add("@rtn", dbType: DbType.Int32, direction: ParameterDirection.ReturnValue);

                this.odo.ExecuteSP(@"SP_ManageArea_Add", paramsOfInsert);

                mgaID = paramsOfInsert.Get<long>("@mgaID");
                spReturnValue = paramsOfInsert.Get<int>("@rtn");

                this.odo.isSuccess = spReturnValue > 0 ? true : false;
                objRet.result = this.odo.isSuccess;
                if (this.odo.isSuccess)
                {
                    objRet.message = mgaID.ToString();
                }
                else
                {
                    objRet.message = this.odo.DbExceptionMessage;
        ...
        private void SetEquGrList()
        {
            ...
            if (entity.MgaNo != null && entity.MgaNo != "")
            {
                ...
                var GrList1 = this.odo.GetQueryResult<EquGroupModel>(@"SELECT EquGrpName,EquGrpNo,A.EquGrpID
                                                FROM B01_EquGroup A
                                                LEFT JOIN B01_MgnEquGroup B ON A.EquGrpID=B.EquGrpID AND MgaID=@MgaID                                                        
                                                WHERE B.EquGrpID IS NULL AND A.OwnerList like @OwnerList"
                                                , new { MgaID = Request["MgaID"]
                                                , OwnerList = string.Format("%\\{0}\\%", userid) }).ToList();
        ...
        public void SetAuthUpdate2()
        {
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    string equGrList = Request["EquGrList"];
                    //2022/8/5 當已加入的群組清單為空值時，javascript傳過來的EquGrList不會是null，所以這裡要修改
                    if (!string.IsNullOrEmpty(equGrList) && equGrList.Split(',').Count() > 0)
                    {
        ...
        {            
            ...
            string RequestMgaEmail = Request["MgaEmail"].ToString().Replace("，", ",").Trim();
            ...
            {
                foreach(var mail in RequestMgaEmail.Split(','))
                {
                    bool EmailOK = IsEmail(mail);
                    if (EmailOK == false)
                    {
                        sRet.result = false;
