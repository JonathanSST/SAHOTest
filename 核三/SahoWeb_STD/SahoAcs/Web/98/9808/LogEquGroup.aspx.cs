...
namespace SahoAcs
{
    public partial class LogEquGroup : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("LogEquGroup", "LogEquGroup.js");//加入同一頁面所需的JavaScript檔案
            ...
            DeleteButton.Attributes["onClick"] = "CallDelete('" + this.GetLocalResourceObject("CallDelete_Title") + "', '" +
                ...
                this.GetLocalResourceObject("CallDelete_DelLabel") + "'); return false;";
            #endregion

            #region 註冊pop1頁Button動作
            ImgCloseButton1.Attributes["onClick"] = "CancelTrigger1.click(); return false;";
            popB_Join.Attributes["onClick"] = "DataJoinRemove('Add'); return false;";
            popB_Remove.Attributes["onClick"] = "DataJoinRemove('Del'); return false;";
            popB_OK.Attributes["onClick"] = "SaveExcute(); return false;";
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.DataRow:
                    ...
                    GrRow.ID = "GV_Row" + oRow.Row["LogEquGrpID"].ToString();
                    #endregion
                    ...
                    e.Row.Attributes.Add("OnClick", "SingleRowSelect(0, this, SelectValue,'" + oRow.Row["LogEquGrpID"].ToString() + "', '', '');");
        ...
        public void Query()
        {
            ...
            sql = @" SELECT ROW_NUMBER() OVER(ORDER BY LogEquGrpID) AS NewIDNum, LogEquGrpID, LogEquGrpName FROM B01_LogEquGroup ";

            if (!string.IsNullOrEmpty(QueryTextBox_KeyWord.Text))
            {
                if (wheresql != "") wheresql += " AND ";
                wheresql += " ( LogEquGrpID LIKE ? OR LogEquGrpName LIKE ? ) ";
                liSqlPara.Add("S:" + "%" + QueryTextBox_KeyWord.Text + "%");
                liSqlPara.Add("S:" + "%" + QueryTextBox_KeyWord.Text + "%");
            }
            ...
            sql += wheresql + " GROUP BY LogEquGrpID, LogEquGrpName";
        ...
        public static string[] LoadData(string SelectValue)
        {
            ...
            string sql = "", strSource = "", strSet = "";
            string[] arrResult = new string[3];
            List<string> liSqlPara = new List<string>();
            DBReader drGroup = null, drSource = null, drSet = null;

            #region Process String
            if (!string.IsNullOrEmpty(SelectValue))
            {
                sql = @"SELECT DISTINCT LogEquGrpID, LogEquGrpName
                FROM B01_LogEquGroup
                WHERE LogEquGrpID = ? ";
                liSqlPara.Add("S:" + SelectValue.Trim());
                oAcsDB.GetDataReader(sql, liSqlPara, out drGroup);

                sql = @" SELECT EquNo, EquName FROM B01_EquData
                    WHERE EquNo NOT IN( SELECT EquNo FROM B01_LogEquGroup WHERE LogEquGrpID = ? )";
                oAcsDB.GetDataReader(sql, liSqlPara, out drSource);

                sql = @"SELECT B01_LogEquGroup.EquNo, B01_EquData.EquName
                    FROM B01_LogEquGroup
                    INNER JOIN B01_EquData ON B01_LogEquGroup.EquNo = B01_EquData.EquNo
                    WHERE LogEquGrpID = ? ";
                oAcsDB.GetDataReader(sql, liSqlPara, out drSet);
            }
            else
            {
                sql = @" SELECT EquNo, EquName FROM B01_EquData";
            ...
            if (drGroup != null)
            {
                string sData = "";
                while (drGroup.Read())
                    sData += drGroup.DataReader["LogEquGrpID"].ToString() + "|" + drGroup.DataReader["LogEquGrpName"].ToString() + ",";
                arrResult[0] = sData.Substring(0, sData.Length - 1);
            }
            else
            {
                arrResult[0] = "";
            ...
            if (drSource != null)
            {
                string sData = "";
                while (drSource.Read())
                    sData += drSource.DataReader["EquNo"].ToString() + "|" + drSource.DataReader["EquName"].ToString() + ",";
                arrResult[1] = sData.Substring(0, sData.Length - 1);
            }
            else
            {
                arrResult[1] = "";
            ...
            if (drSet != null)
            {
                string sData = "";
                while (drSet.Read())
                    sData += drSet.DataReader["EquNo"].ToString() + "|" + drSet.DataReader["EquName"].ToString() + ",";
                arrResult[2] = sData.Substring(0, sData.Length - 1);
            }
            else
            {
                arrResult[2] = "";
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object Save(string sLogEquGrpID, string sLogEquGrpName, string sEquList, string sMode)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            ...
            NoArray[0] = sLogEquGrpID;
            NoArray[1] = sLogEquGrpName;
            NoArray[2] = sEquList;

            sRet = Check_Input_DB(NoArray, sMode);

            if (sRet.result)
            {
                #region Process String
                sql = @" DELETE FROM B01_LogEquGroup WHERE LogEquGrpID = ? ";
                liSqlPara.Add("S:" + sLogEquGrpID.Trim());
                #endregion
                ...
                if (istat > -1)
                {
                    ...
                    if (!sMode.Equals("Delete"))
                    {
                        string[] arrEquList = sEquList.Split('|');

                        for (int i = 0; i < arrEquList.Length; i++)
                        {
                            sql += @" INSERT INTO B01_LogEquGroup(LogEquGrpID, LogEquGrpName, EquNo)  
                            VALUES (?, ?, ?); ";
                            liSqlPara.Add("S:" + sLogEquGrpID.Trim());
                            liSqlPara.Add("S:" + sLogEquGrpName.Trim());
                            liSqlPara.Add("S:" + arrEquList[i].ToString().Trim());
                        }
                        ...
                        if (istat > -1)
                        {
                            ...
                            {
                                sRet.message = "新增刷卡記錄設備群組資料成功！！";
                            }
                            else
                            {
                                sRet.message = "編輯刷卡記錄設備群組資料成功！！";
                            }
                        ...
                        else
                        {
                            if (sMode.Equals("Insert"))
                            {
                                sRet.message = "新增刷卡記錄設備群組資料失敗！！";
                            }
                            else
                            {
                                sRet.message = "編輯刷卡記錄設備群組資料失敗！！";
                            }
                    ...
                    {
                        sRet.message = "刪除刷卡記錄設備群組資料成功！！";
                        oAcsDB.Commit();
                    }
                }
                else
                {
                    sRet.message = "刪除刷卡記錄設備群組資料失敗！！";
                    oAcsDB.Rollback();
                }
            }
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string sMode)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Pub.MessageObject sRet = new Pub.MessageObject();
            string sql = "";
            ...
            if (string.IsNullOrEmpty(NoArray[0].Trim()))
            {
                ...
                sRet.result = false;
                sRet.message += "群組編號 必須輸入";
            }

            if (NoArray[0].Contains(" "))
            {
                sRet.result = false;
                sRet.message += "群組編號字串中不可有空白符號";
            }
            ...
            if (string.IsNullOrEmpty(NoArray[1].Trim()))
            {
                if (!string.IsNullOrEmpty(sRet.message)) sRet.message += "\n";
                sRet.result = false;
                sRet.message += "群組名稱 必須輸入";
            }

            if (NoArray[1].Contains(" "))
            {
                sRet.result = false;
                sRet.message += "群組名稱字串中不可有空白符號";
            }
            ...
            {
                case "Insert":
                    sql = @" SELECT DISTINCT LogEquGrpID FROM B01_LogEquGroup WHERE LogEquGrpID = ? ";
