...
namespace SahoAcs.Web._07._0703
{
    public partial class DiningRecordInq : System.Web.UI.Page
    {
        #region Global block
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("JsInclude1", "DiningRecordInq.js?" + DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
        ...
        private void SetQueryData()
        {
            ...
            string OrderSrc = Request["OrderSrc"];
...
MO.[Status],MO.ProcTime,MO.CancelSrc,MO.ProcTime ,
CASE (MO.OrderSrc) When '1' Then '刷卡' When '5' Then '補登' When '6' Then '取消' Else '' END AS 'OrderSrcDesc'
...
Where 1=1 
";
            Sql += " And convert(varchar, MO.ProcTime, 120) Between '" + CardTimeSDate.Replace(@"/", "-") + "' And '" + CardTimeEDate.Replace(@"/", "-") + "'";
            ...
            if (!string.IsNullOrEmpty(OrderSrc))
            {
                Sql += " And MO.OrderSrc = '" + OrderSrc.ToString().Trim() + "' ";
            }

            Sql += " ORDER BY MO.ProcTime ";
            ...
            this.ListLog = this.odo.GetQueryResult<B03_MealOrder>(Sql.ToString(), new
            {
                ...
                OrderSrc = OrderSrc,
                UserID = UserID
            }).OrderByField("ProcTime", true).ToList();
        ...
        private void ExportExcel()
        {
            ...
            if (this.ListLog.Count != 0)
            {
                ...
                ExcelWorksheet ws = pck.Workbook.Worksheets.Add("用餐查詢報表");
                ws.Cells[1, 1].Value = "用餐時間";
                ...
                ws.Cells[1, 8].Value = "資料來源";

                int count = 2;
                foreach (var o in this.ListLog)
                {
                    ws.Cells[count, 1].Value = string.Format("{0:yyyy/MM/dd HH:mm:ss}", o.ProcTime);
                    ...
                    ws.Cells[count, 8].Value = o.OrderSrcDesc;
                    count++;
                }

                ws.Cells.AutoFitColumns(); //自動欄寬
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment; filename=用餐查詢報表.xls");
