...
{
    public partial class KeyinMeal : System.Web.UI.Page
    {
        #region Global block
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (Request["PageEvent"] != null && Request["PageEvent"] == "KeyIn")
            {
                KeyIn();
            }
            else
            {
                this.Calendar_CardTimeSDate.DateValue = DateTime.Now.AddDays(-1).ToString("yyyy/MM/dd");
            }
            ClientScript.RegisterClientScriptInclude("JsInclude1", "KeyinMeal.js?" + DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
...
WHERE 1 = 1 ";
            ...
            if (Result.Count() == 0)
            {
                this.odo.Execute(@"Insert into B03_MealOrder
                        (OrderTime,MealDate,MealNo,CardNo,OrderSrc,CancelSrc,Status,ProcTime,MealFood) 
                        VALUES (@OrderTime,@MealDate,@MealNo,@CardNo,@OrderSrc,@CancelSrc,@Status,@ProcTime,@MealFood)",
                           new
                           {
                               OrderTime = Convert.ToDateTime(SDate),
                               MealDate = SDate,
                               MealNo = MealNo,
                               CardNo = CardNo,
                               OrderSrc = 5,
            ...
            else
            {
                ...
                if (Result.Count() !=0 )
                {
                    foreach (var o in Result)
                    {
                       ...
                       MealDate=@MealDate,OrderSrc=5,Status=3,ProcTime=@ProcTime 
                       Where  RecordID=@RecordID ",
                        ...
                        else
                        {
                            ...
                            Pub.MessageObject sRet = new Pub.MessageObject()
                            {
                                result = _result,
                                act = "Update",
                                message = "該員, 這一天的這一餐，已經有 [刷卡用餐] 或 [補登用餐] 過了！"
                            };
                            Response.Clear();
                            Response.Write(new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(sRet));
                            Response.End();
                ...
                case "Insert":
                    this.odo.Execute(@"INSERT INTO B00_SysLog (LogTime, LogType, UserID , UserName, LogFrom, EquNo, EquName, LogInfo, LogIP) 
                                       ...
                                       new
                                       {
                                           ...
                                           EquNo = "",
                                           EquName = "",
                                           LogInfo = "補登用餐(Web)寫入成功，CardNo : " + CardNo + " OrderDate : " + OrderDate,
                ...
                case "Update":
                    this.odo.Execute(@"INSERT INTO B00_SysLog (LogTime, LogType, UserID , UserName, LogFrom, EquNo, EquName, LogInfo, LogIP) 
                                       ...
                                       new
                                       {
                                           ...
                                           EquNo = "",
                                           EquName = "",
                                           LogInfo = "補登用餐(Web)修改成功，CardNo : " + CardNo + " OrderDate : " + OrderDate,
                                           LogIP = Request.UserHostAddress
                                       });
                    break;
                case "Fail":
                    this.odo.Execute(@"INSERT INTO B00_SysLog (LogTime, LogType, UserID , UserName, LogFrom, EquNo, EquName, LogInfo, LogIP) 
                                       ...
                                       new
                                       {
                                           ...
                                           EquNo = "",
                                           EquName = "",
                                           LogInfo = "補登用餐(Web)-嘗試了重覆補登，CardNo : " + CardNo + " OrderDate : " + OrderDate,
                                           LogIP = Request.UserHostAddress
                                       });
                    break;
            }
