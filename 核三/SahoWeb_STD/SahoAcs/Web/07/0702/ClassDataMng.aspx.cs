...
namespace SahoAcs._0702
{
    public partial class ClassDataMng : System.Web.UI.Page
    {
        ...
        {            
            ...
            ClientScript.RegisterClientScriptInclude("SelfScript", "ClassDataMng.js?"+Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
        ...
        private void Query(bool select_state, string SortExpression, string SortDire)
        {
            ...
            sql = " SELECT * FROM B00_ClassData ORDER BY " + SortExpression + " " + SortDire ;           
            #endregion
        ...
        public void Insert()
        {
            ...
            string sql = @" INSERT INTO B00_ClassData(CNo,CName,WBTime,WETime,OBTime,OETime,BERange,WTMin,CreateUserID,CreateTime) VALUES 
                                    (@CNo, @CName, @WBTime, @WETime, @OBTime, @OETime, @BERange, @WTMin,@User,GETDATE())";
            var effectRow = 0;
            
            Pub.MessageObject sRet = new Pub.MessageObject();
            if (this.odo.GetQueryResult("SELECT * FROM B00_ClassData WHERE CNo=@CNo", new {CNo = this.GetFormEqlValue("CNo") }).Count() > 0)
            {
                sRet.result = false;
                sRet.message = "班別代碼重複使用";
            }
            if (sRet.result)
            {
                ...
                {
                    User = UserID,
                    CNo = this.GetFormEqlValue("CNo"),
                    CName = this.GetFormEqlValue("CName"),
                    WTMin = this.GetFormEqlValue("WTMin"),
                    CID = this.GetFormEqlValue("CID"),
                    WETime = this.GetFormEqlValue("WETime"),
                    WBTime = this.GetFormEqlValue("WBTime"),
                    OETime = this.GetFormEqlValue("OETime"),
                    OBTime = this.GetFormEqlValue("OBTime"),
                    BERange = this.GetFormEqlValue("BERange")
                });
                if (effectRow > 0)
                {
                    sRet.result = true;
        ...
        {            
            string UserID = Sa.Web.Fun.GetSessionStr(this.Page, "UserID");
            string sql = @" UPDATE B00_ClassData SET CNo=@CNo, CName=@CName, WBTime=@WBTime, WETime=@WETime, OBTime=@OBTime, OETime=@OETime
                                , BERange=@BERange, WTMin=@WTMin, UpdateUserID=@User, UpdateTime=GETDATE() WHERE CID=@CID";
            Pub.MessageObject sRet = new Pub.MessageObject();
            if (this.odo.GetQueryResult("SELECT * FROM B00_ClassData WHERE CNo=@CNo AND CID<>@CID", new {CID=this.GetFormEqlValue("CID"), CNo=this.GetFormEqlValue("CNo")}).Count() > 0)
            {
                sRet.result = false;
                sRet.message = "班別代碼重複使用";                
            }
        ...
        {
            string UserID = Sa.Web.Fun.GetSessionStr(this.Page, "UserID");
            string sql = " Delete B00_ClassData WHERE CID=@CID";
            var effectRow = this.odo.Execute(sql, new {User = UserID, CID = this.GetFormEndValue("CID") });
            Pub.MessageObject sRet = new Pub.MessageObject();
            if (effectRow > 0)
            {
                sRet.result = true;
        ...
        public void LoadData()
        {
            ...
            Pub.MessageObject sRet = new Pub.MessageObject();
            sRet.result = false;
            var result = this.odo.GetQueryResult("SELECT * FROM B00_ClassData WHERE CID=@Cid", new { Cid = this.GetFormEndValue("CID") });            
            dynamic obj = null;
            if (result.Count() > 0)
            {
                sRet.result = true;
