...
namespace SahoAcs
{
    public partial class QueryCardVms : System.Web.UI.Page
    {
        ...
        OrmDataObject od = new OrmDataObject("MsSql", Pub.GetDapperConnString());
         List<EquVms> EquVmsList = new List<EquVms>();
        #endregion
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("QueryCardVms", "QueryCardVms.js");//加入同一頁面所需的JavaScript檔案
            ...
            #endregion
            //this.EquVmsList = this.od.GetQueryResult<EquVms>("")
            ...
            else
            {
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertS", "alert('Start Time Format is error!!!'); $.unblockUI();", true);
                    return;
                }

                if (!IsDate(Calendar_CardTimeEDate.DateValue.Trim()))
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertE", "alert('End Time Format is error!!!'); $.unblockUI();", true);
                    return;
                }
                #endregion
                ...
                else if (sFormTarget == this.ADVQueryButton.ClientID)
                {
                    ...
                    ViewState["query_ADVDep"] = this.ADVDropDownList_Dep.SelectedTextCSV;
                    ...
                    ViewState["query_ADVPsnNo"] = this.ADVTextBox_PsnNo.Text.Trim();
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            var paras = this.od.GetQueryResult("SELECT * FROM B00_SysParameter WHERE ParaNo IN ('VmsList','VmsHost')");
            ...
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    ...
                    #region 設定欄位寛度
                    int[] HeaderWidth = { 120, 80, 120, 80, 100, 80, 60, 80, 100, 100, 120};
                ...
                case DataControlRowType.DataRow:
                    ...
                    #region 設定欄位寛度
                    int[] DataWidth = { 123, 85, 124, 84, 104, 84, 64, 84, 104, 104, 124 };
                    for (int i = 0; i < e.Row.Cells.Count - 1; i++)
                    ...
                    if (!string.IsNullOrEmpty(oRow.Row["CardTime"].ToString()))
                    {
                        ...
                        btn.Attributes["href"] = "#";
                        if(this.EquVmsList.Where(i => i.EquNo == Convert.ToString(oRow.Row["EquNo"])).Count() > 0 && vms_dic["VmsHost"]!="")
                        {
        ...
        public void Query(string sMode, bool bMode, string SortExpression, string SortDire)
        {
            ...
            string strRD = RD.Next(0, 10000).ToString();
            string strTmpDT = "[##TEMP0601_" + DateTime.Now.ToString("mmssfff") + "_" + strRD + "]";

            sql = @"
                SELECT
                    RecordID, LogTime, CardTime, CardNo, CardVer, CtrlAddr, ReaderNo, PsnNo,
                    PsnName, OrgStruc, EquNo, EquName, LogStatus   
                INTO " + strTmpDT + " FROM B01_Cardlog ";
            ...
            if (sMode == "Normal")
            {
                ...
                #region 使用SQLClient (測試程式)
                //string strDbSource = WebConfigurationManager.AppSettings["DbSource"];
                //string strDbName = ConfigurationManager.AppSettings["DbName"].ToString();
                //string strDbID = WebConfigurationManager.AppSettings["DbID"];
                //string strDbPW = WebConfigurationManager.AppSettings["DbPW"];

                //string strConn = string.Format("Addr={0};Database={1};uid={2};pwd={3};", strDbSource, strDbName, strDbID, strDbPW);
                ...
                //    sql = @"
                //        SELECT
                //            RecordID, LogTime, CardTime, CardNo, CardVer, PsnNo,
                //            PsnName, OrgStruc, EquNo, EquName, LogStatus, DepID, DepName
                //        INTO [##TEMP0601] FROM B01_Cardlog 
                //        WHERE CardTime >= @CardSTime AND CardTime <= @CardETime ";

                //    using (SqlCommand cmd = new SqlCommand(sql, conn))
                //    {
                //        cmd.Parameters.AddWithValue("@CardSTime", ViewState["query_CardTimeSDate"].ToString());
                //        cmd.Parameters.AddWithValue("@CardETime", ViewState["query_CardTimeEDate"].ToString());
                //        cmd.ExecuteNonQuery();

                //        sql = @" SELECT * FROM [##TEMP0601] ";
            ...
            else if (sMode == "ADV")
            {
                ...
                //{
                //    if (wheresql != "") wheresql += " AND ";
                //    wheresql += " ( DepID LIKE ? OR DepName LIKE ? ) ";
                //    liSqlPara.Add("A:" + "%" + ViewState["query_ADVDepNoDepName"].ToString() + "%");
                ...
                if (!string.IsNullOrEmpty(ViewState["query_ADVPsnNo"].ToString()))
                {
                    if (wheresql != "") wheresql += " AND ";
                    wheresql += " PsnNo LIKE ? ";
                    liSqlPara.Add("A:" + "%" + ViewState["query_ADVPsnNo"].ToString() + "%");
                }
            ...
            intRet = oAcsDB.SqlCommandExecute(sql, liSqlPara);

            #region 從暫存TABLE依條件得到結果
            if (intRet > -1)
            {
                #region 組成SQL
                sql = @"
                ...
                #region 因應人員登入需求使用，PsnID 存在與否，PsnID不存在就執行跟UserID這邊有關的CODE
                if (this.sPsnID == "")
                {
                    sql += @" 
                        INNER JOIN 
                        (
                            SELECT DISTINCT ED.EquNo, ED.EquName 
                            FROM B01_EquData ED
                            ...
                            WHERE SUMS.UserID = ?
                        ) AS ED1 ON ED1.EquNo = CL.EquNo 
                        INNER JOIN 
                        (
                            SELECT DISTINCT OS.OrgIDList, OS.OrgStrucNo 
                            ...
                            WHERE SUMS.UserID = ?
                        ) AS OS1 ON OS1.OrgIDList = CL.OrgStruc ";
                ...
                sql += string.Format(" DROP TABLE {0} ", strTmpDT);     // 清除暫存##TABLE
                #endregion
                ...
                {
                    string strSQL = string.Format(" DROP TABLE {0} ", strTmpDT);
                ...
                if (isOk)
                {
                    ...
                    //dtTmp.Columns.Remove("OrgStrucNo");
                ...
                if (isOK)
                {
                    if (dtTmp.Rows.Count > 0)
                    {
                        ...
                        //    NewIDNum >= {0} AND NewIDNum <= {1}", 
                        //    Convert.ToString((_pagesize * _pageindex + 1)), 
                        //    Convert.ToString((_pagesize * (_pageindex + 1))));
                        ...
                        else if (sMode == "ADV")
                        {
                            ...
                            if (!string.IsNullOrEmpty(ViewState["query_ADVDep"].ToString().Trim()))
                            {
                                ...
                                if (strCondition != "") strCondition += " AND ";
                                strCondition += " DepName IN ( " + DepSql + " ) ";
                            }
                        }
        ...
        public void ExportExcel(DataTable ProcDt)
        {
            ...
            // 把不必要的欄位去掉
            dtCardLog.Columns.Remove("OrgStrucNo");
            dtCardLog.Columns.Remove("DepNo");
