...
namespace SahoAcs
{
    public partial class QueryInOutList : System.Web.UI.Page
    {
        ...
        {            
            ...
            ClientScript.RegisterClientScriptInclude("0638", "QueryInOutList.js?"+DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetQueryData()
        {
            ...
            string sql = "";            
            sql = @"SELECT PsnNo,CardNo,EquNo,EquName,EquDir,CardTime,PsnName,DepName
                            ,CONVERT(VARCHAR(10),CardTime,111)+' '+CONVERT(VARCHAR(5),CardTime,108) FROM B01_CardLog
                            WHERE CardTime BETWEEN @CardDateS AND @CardDateE AND (PsnNo LIKE @PsnNo OR PsnName LIKE @PsnNo OR CardNo LIKE @PsnNo) AND EquNo IN (@Door1,@Door2)
            ...
            this.ListLog.ForEach(i => { i.Last = i.CardTime.ToString("yyyy/MM/dd"); });
            List<CardLogTrt> TempCardLog = new List<CardLogTrt>();
            TempCardLog = this.ListLog.GroupBy(i => new {i.CardNo,i.Last}).Select(i=>i.Last()).ToList();
            this.ListLog = TempCardLog.Where(i => i.EquNo == Out).OrderByDescending(i=>i.CardTime).OrderByDescending(i=>i.PsnNo).ToList();
        ...
        private void ExportExcel()
        {
            ...
            for (int i = 0; i < this.ListLog.Count; i++)
            {
                ...
                ws.Cells[i+2, 6].Value = string.Format("{0:HH:mm:ss}", this.ListLog[i].CardTime);
                //i++;
            }
