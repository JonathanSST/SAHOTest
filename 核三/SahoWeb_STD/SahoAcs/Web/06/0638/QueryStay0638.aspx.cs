...
namespace SahoAcs
{
    public partial class QueryStay0638 : System.Web.UI.Page
    {
        ...
        {            
            ...
            ClientScript.RegisterClientScriptInclude("0638", "QueryStay0638.js?"+DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetQueryData()
        {
            ...
            sql = @"SELECT * FROM
                            (SELECT 
	                            A.PsnName,A.CardNo,
	                            B.OrgStrucID,
	                            C.OrgName AS Company,DepName,
	                            CONVERT(VARCHAR(10),CardTime,111) AS CardDate,
	                            SUBSTRING(CONVERT(VARCHAR(50),CardTime,121),12,8) AS CardTime
                            FROM 
	                            B01_CardLog A
                            ...
                            WHERE
	                            LogStatus=0 AND Equdir='進' AND CardTime BETWEEN @CardDateS AND @CardDateE ) AS RESULT1 ";
            var logstatus = this.odo.GetQueryResult("SELECT * FROM B00_CardLogState");
            //sql += this.GetConditionCmdStr();
            this.ListLog = this.odo.GetQueryResult<StayCardLog>(sql, new
            {
