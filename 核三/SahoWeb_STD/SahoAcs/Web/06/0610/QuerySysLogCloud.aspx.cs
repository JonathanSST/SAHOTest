...
namespace SahoAcs
{
    public partial class QuerySysLogCloud : System.Web.UI.Page
    {
        #region 一.宣告
        ...
        private void RegisterComponeAndScript()
        {
            ...
            ClientScript.RegisterClientScriptInclude("QueryCardLogScript", "QuerySysLogCloud.js?"+Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
        }
        ...
        public void Query(bool bMode, string SortExpression, string SortDire)
        {
            ...
            foreach(DataRow r in SysLogTable.Rows)
            {
        ...
        public static object SetComplexQueryWheresql(string[] qryParam)
        {
            ...
            if ((qryParam != null) && (qryParam.Length > 0))
            {
                ...
                if (!string.IsNullOrEmpty(qryParam[0].Trim()) || !string.IsNullOrEmpty(qryParam[1].Trim()))
                {
                    ...
                    if (page != null)
                    {
                        if (!string.IsNullOrEmpty(qryParam[0].Trim()))
                        {
                            qryParam[0] = DateTime.Parse(qryParam[0]).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                        }
                        if (!string.IsNullOrEmpty(qryParam[1].Trim()))
                        {
                            qryParam[1] = DateTime.Parse(qryParam[1]).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                        }
                    }
