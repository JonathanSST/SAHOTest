...
namespace SahoAcs
{
    public partial class QueryMobileLog : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("0650", "QueryMobileLog.js?" + DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetInitData()
        {
            ...
            DateTime dtLastCardTime = this.odo.GetDateTimeScalar(@"SELECT MAX(CardTime) FROM B01_CardLog WHERE CardTime <= @ZoneTime",new {ZoneTime=this.GetZoneTime()});
            ...
            {
                this.CalendarS.DateValue = dtLastCardTime.GetZoneTime(this).ToString("yyyy/MM/dd") + " 00:00:00";
                this.CalendarE.DateValue = dtLastCardTime.GetZoneTime(this).ToString("yyyy/MM/dd") + " 23:59:59";
            }
            #endregion
        ...
        private void SetQueryData()
        {
            ...
            sql = @"SELECT *,PsnName AS EmpName,ISNULL(CardPic,'') AS CardPic2,
                        ISNULL((SELECT TOP 1 StateDesc FROM B00_CardLogState WHERE CONVERT(VARCHAR,Code)=OpType),'') AS OpName
                        FROM FillCardLog INNER JOIN B01_Person ON EmpID=PsnNo WHERE CardTime BETWEEN @CardTimeS AND @CardTimeE AND (PsnNo LIKE @EmpName OR PsnName LIKE @EmpName) ";
            ...
            }).OrderByField("CardTime",false).ToList();
        ...
        private void ExportExcel()
        {
            ...
            if (this.ListLog.Count > 0)
            {
                ...
                {
                    sb.Append("<div style='width:100%;text-align:center;font-size:14pt'>行動打卡紀錄明細</div>");
            ...
            Response.AddHeader("content-disposition", "attachment;filename=行動打卡紀錄列印.pdf");
            Response.End();
        }
