...
namespace SahoAcs
{
    public partial class _0602_1 : System.Web.UI.Page
    {
        ...
        {
            ((Label)this.Master.FindControl("labprocName")).Text = "個人讀卡紀錄查詢";
            if (Session["IDNum"] != null)
            {
                this.txt_CardNo_PsnName = Session["IDNum"].ToString();
            }
            else
            {
                //Response.Redirect("/IDLogin.aspx");
            }
            //Session["UserID"] = "User";
            ...
            ClientScript.RegisterClientScriptInclude("0602", "0602_1.js?"+Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetQueryData()
        {
            ...
            sql = @" SELECT A.*,B.IDNum FROM B01_CardLog A INNER JOIN B01_Person B ON A.PsnNo=B.PsnNo WHERE 1=1 ";
        ...
        private string GetConditionCmdStr()
        {
            ...
            //sql += " AND B.PsnNo IN (SELECT PsnNo FROM B01_Person WHERE IDNum=@PsnName)";
            sql += " AND B.IDNum=@PsnName ";
            if (Request["PsnID"] != "")
            {
                sql += " AND B.PsnNo IN (SELECT PsnNo FROM B01_Person WHERE PsnID=@PsnID)";
            }
            return sql;
        }
