...
namespace SahoAcs
{
    public partial class _0602Cloud : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("0602", "0602Cloud.js");//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetQueryData()
        {
            ...
            sql = @" SELECT A.*,  DATEADD(HOUR,-B.TimeZone,A.CardTime) AS CardTime FROM 
	                    B01_CardLog A 
	                    INNER JOIN V_McrInfo B ON A.EquNo=B.EquNo
                        WHERE (A.EquNo IN @EquList) AND (A.EquClass='TRT' OR A.IsAndTrt=1) ";
            ...
            this.ListLog = this.odo.GetQueryResult<CardLogModel>(sql, new
            {
                ...
                CardTimeS = DateTime.Parse(Request["CardTimeS"]).GetUtcTime(this).AddDays(-1),
                CardTimeE = DateTime.Parse(Request["CardTimeE"]).GetUtcTime(this).AddDays(1),
            ...
            this.ListLog = this.ListLog.Where(i => i.CardTime >= DateTime.Parse(Request["CardTimeS"]).GetUtcTime(this)
                                && i.CardTime <= DateTime.Parse(Request["CardTimeE"]).GetUtcTime(this)).OrderByField(this.SortName, boolSort).ToList();
            ...
            foreach (DataRow r in this.DataResult.Rows)
            {
                ...
                r["CardTimeVal"] = string.Format("{0:yyyy/MM/dd HH:mm:ss}", Convert.ToDateTime(r["CardTime"]).GetUtcToZone(this));
        ...
        void SetOneCardLog()
        {
            ...
            var logs = this.odo.GetQueryResult<CardLogModel>(@"SELECT A.*, DATEADD(HOUR, -B.TimeZone, A.CardTime) AS CardTime FROM B01_CardLog A
                                INNER JOIN V_McrInfo B ON A.EquNo = B.EquNo WHERE RecordID=@RecordID", new { RecordID = RecordID });
            logs.First().CardTime = logs.First().CardTime.GetUtcToZone(this);
