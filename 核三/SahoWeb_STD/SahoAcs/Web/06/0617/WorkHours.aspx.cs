...
namespace SahoAcs
{
    public partial class WorkHours : System.Web.UI.Page
    {
        ...
        int m_PageSize = 100, m_PageIndex = 0, m_PageCount = 0, m_DataCount = 1, m_PsnCount = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("WorkHours", "WorkHours.js");
            #endregion
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    #region 設定欄位寛度
                    int[] HeaderWidth = { 100, 100, 100, 100, 100, 100, 100, 359 };
                ...
                case DataControlRowType.DataRow:
                    ...
                    float fWorkTime = 0, fRealWorkTime = 0;
                    #region 指定Row的ID
                    ...
                    if (float.TryParse(e.Row.Cells[6].Text, out fWorkTime)) { e.Row.Cells[6].Text = string.Format("{0:F}", fWorkTime); }
                    if (float.TryParse(e.Row.Cells[7].Text, out fRealWorkTime)) { e.Row.Cells[7].Text = string.Format("{0:F}", fRealWorkTime); }

                    #region 設定欄位寛度
                    int[] DataWidth = { 103, 104, 104, 104, 104, 104, 104, 363 };
                ...
                case DataControlRowType.Pager:
                    ...
                    PlaceHolder phdTotalRealWorkHours = e.Row.FindControl("phdTotalRealWorkTime") as PlaceHolder;
                    #endregion
                    ...
                    phdTotalWorkTime.Controls.Add(new LiteralControl("　　總工時小計：" + string.Format("{0:F}", m_WorkTime) + "時"));
                    phdTotalRealWorkHours.Controls.Add(new LiteralControl("　　總扣除午休實際工時：" + string.Format("{0:F}", m_RealWorkTime) + "時"));
                    #endregion
        ...
        {
            float fWorkTime = 0, fRealWorkTime = 0;
            string sSql = "", sWhere = "", sM = "", sD = "", sY = "", sKey = "", sCardNo = "", sYMD = "", sItemNo = "", sLogEquGrpID1 = "", sLogEquGrpID2 = "";
            DataTable oDataTable   = null;
            DB_Acs oAcsDB          = new DB_Acs(this.Page, Pub.GetConnectionString(Pub.sConnName));
            ...
            #region 查詢報表資料
            #region 設定SQL語法
            sSql = string.Format(@" SELECT ROW_NUMBER() OVER(ORDER BY CompanyName, DeptName, ItemName, PsnName, InTime) AS NewIDNum, CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, WorkTime, WorkTime - DelWorkTime AS 'RealWorkTime', InTime, InEquName, OutTime, OutEquName, 
                                    ...
                                    SELECT * FROM dbo.GetInOutTable('C5', {0}, '{1}', '{2}', '{3}')) YA ", Pub.WorkOverTime, Pub.RestRange, 
            ...
            //oAcsDB.GetDataTable("ReportTable", sSql, liSqlPara, out oDataTable);            

            //解決資料庫連線timeout的問題，將timeout時間增加為300秒
            oAcsDB.GetDataTable("ReportTable", sSql, liSqlPara, "WorkHours.aspx", out oDataTable);

            #endregion

            #region 處理廠商考勤及2與3號門設備的工時與午休工時並計算相關的網頁資料
            m_ReportTable = null; Session["ReportTable"] = null; Session["PastPageData"] = null;
            m_DataCount = 0; m_PsnCount = 0; m_WorkTime = 0; m_RealWorkTime = 0;

            if (oDataTable != null)
            {
                m_ReportTable = oDataTable.Clone();
                ...
                foreach (DataRow oDataRow in oDataTable.Rows)
                {
                    ...
                    if (float.TryParse(oDataRow["WorkTime"].ToString(), out fWorkTime)) { m_WorkTime += fWorkTime; } else { oDataRow["WorkTime"] = 0; }
                    if (float.TryParse(oDataRow["RealWorkTime"].ToString(), out fRealWorkTime)) { m_RealWorkTime += fRealWorkTime; } else { oDataRow["RealWorkTime"] = 0; }
                    #endregion

                    if (!oHashtable.ContainsKey(sCardNo))
                    {
                        oList.Add(sCardNo);
                        oHashtable.Add(sCardNo, oDataRow);
                    }
                    else
                    {
                        ((DataRow)oHashtable[sCardNo])["WorkTime"]     = fWorkTime + float.Parse(((DataRow)oHashtable[sCardNo])["WorkTime"].ToString());
                        ((DataRow)oHashtable[sCardNo])["RealWorkTime"] = fRealWorkTime + float.Parse(((DataRow)oHashtable[sCardNo])["RealWorkTime"].ToString());
                    }
                }

                foreach (string sValue in oList)
                {
                    m_ReportTable.ImportRow((DataRow)oHashtable[sValue]);
                }

                m_PsnCount  = oHashtable.Count;  //處理總人數
                m_DataCount = oHashtable.Count;  //處理總筆數
            }

            //儲存查詢資料與網頁資料
            oList = null; oHashtable = null; m_FTRTHashtable = null;
        ...
        public void ExportExcel(DataTable oDataTable)
        {
            ...
            ExcelWorksheet ws     = pck.Workbook.Worksheets.Add("WorkHours");
            ...
            ws.Cells[1, 7].Value = "工時小計";
            ws.Cells[1, 8].Value = "扣除午休實際工時";

            //設定內容
            for (int i = 0, iCount = oReportData.Rows.Count; i < iCount; i++)
            {
                for (int j = 0, jCount = 8; j < jCount; j++)
                {
                    if (j < 6)
                    {
                    ...
                    {
                        ws.Cells[i + 2, j + 1].Value = string.Format("{0:F}", float.Parse(oReportData.Rows[i][j + 1].ToString().Replace("&nbsp;", "").Trim()));
                    }
                }
            }

            //設定註記
            ws.Cells[oReportData.Rows.Count + 3, 1].Value = "總人數：" + oReportData.Rows.Count + "人";
            ws.Cells[oReportData.Rows.Count + 4, 1].Value = "總工時小計：" + string.Format("{0:F}", m_WorkTime) + "時";
            ws.Cells[oReportData.Rows.Count + 5, 1].Value = "總扣除午休實際工時：" + string.Format("{0:F}", m_RealWorkTime) + "時";

            //設定格式
            ws.Cells.AutoFitColumns();
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment; filename=" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_WorkHours.xlsx");
