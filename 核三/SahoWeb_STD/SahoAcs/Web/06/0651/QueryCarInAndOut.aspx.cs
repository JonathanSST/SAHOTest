...
namespace SahoAcs
{
    public partial class QueryCarInAndOut : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("0651", "QueryCarInAndOut.js?" + DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetInitData()
        {
            ...
            DateTime dtLastCardTime = oAcsDB.GetDateTimeScalar(@"SELECT MAX(CardTime) FROM B01_CardLog WHERE CardTime <= GETDATE() AND EquClass='Door Access'");
        ...
        private void CreateDropDownList_EquItem()
        {
            ...
            #region DataAuth
            if (wheresql != "") wheresql += " AND ";
            wheresql += " (SysUserMgns.UserID = @UserID ) AND EquData.EquClass='Door Access' ";
            #endregion
            ...
            if (result.Count() > 0)
            {
                foreach (var o in result)
                {
                    if (Pub.ETAGEquList.Contains(o.EquNo))
                    {
                        Item = new System.Web.UI.WebControls.ListItem();
            ...
            if (Pub.ETAGEquList.Contains("IPCAM"))
            {
                ADVDropDownList_Equ.Items.Add(new ListItem() { Text = "IPCAM", Value = "IPCAM" });
            }

            if (ADVDropDownList_Equ.Items.Count == 0)
            {
