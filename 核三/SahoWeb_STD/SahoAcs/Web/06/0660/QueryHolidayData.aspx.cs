...
namespace SahoAcs.Web._06._0660
{
    public partial class QueryHolidayData : System.Web.UI.Page
    {
        ...
        private void SetQueryData()
        {
            ...
            string QueryPsnsql = @"SELECT A.*,P.PsnName,VName AS HoliNo FROM B03_PsnHoliday A 
                                                ...
                                                WHERE (PsnName LIKE @PsnNo OR A.PsnNo LIKE @PsnNo) AND ((StartTime BETWEEN @QryTimeS AND @QryTimeE) OR (EndTime BETWEEN @QryTimeS AND @QryTimeE))";
            bool boolSort = this.SortType.Equals("ASC");
            this.ScheduleDatas = this.odo.GetQueryResult<B03PsnHoliday>(QueryPsnsql, new {PsnNo=Request.Form["PsnNo"]+"%", QryTimeS=Request["QryTimeS"].ToString(), QryTimeE=Request["QryTimeE"].ToString()}).ToList();
        ...
        private void ExportExcel()
        {
            ...
            if (this.ScheduleDatas.Count > 0)
            {
                ...
                foreach (var p in PsnNoList)
                {
                    ...
                    sb.Append("<div style='width:100%;text-align:center;font-size:14pt'>休假明細表</div>");
            ...
            Response.AddHeader("content-disposition", "attachment;filename=休假明細表.pdf");
            Response.End();
        }
