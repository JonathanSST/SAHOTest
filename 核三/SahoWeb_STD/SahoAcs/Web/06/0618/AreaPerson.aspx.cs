...
namespace SahoAcs
{
    public partial class AreaPerson : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("AreaPerson", "AreaPerson.js");
            #endregion
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    #region 設定欄位寛度
                    int[] HeaderWidth = { 100, 100, 100, 100, 100, 100, 150 };
                ...
                case DataControlRowType.DataRow:
                    ...
                    #region 設定欄位寛度
                    int[] DataWidth = { 103, 104, 104, 104, 104, 104, 154 };
                    ...
                    {
                        e.Row.Cells[6].Text = DateTime.Parse(oRow.Row["CardTime"].ToString()).ToString("yyyy/MM/dd HH:mm:ss");
                    }
                    #endregion
                ...
                case DataControlRowType.Pager:
                    ...
                    LinkButton lbtnPage;
                    PlaceHolder phdAreaPsnCount = e.Row.FindControl("phdAreaPsnCount") as PlaceHolder;
                    #endregion
                    ...
                    phdPageNumber.Controls.Add(new LiteralControl("  "));
                    phdAreaPsnCount.Controls.Add(new LiteralControl("　　總人數：" + m_DataCount + "人"));
                    #endregion
        ...
        private void Query()
        {
            ...
            #region 查詢報表資料
            #region 設定SQL語法
            if (Input_AreaPsnCount.SelectedValue == "WorkArea")               //場區人數     
            {
                sSql = @" SELECT CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, CardTime, EquName, 
                          CompanyID, DeptID, ItemNo, CtrlNo, LogEquGrpID, ROW_NUMBER() OVER(ORDER BY CompanyName, DeptName, ItemName, CardTime) AS NewIDNum FROM GetWorkAreaTable() ";
            }
            else if (Input_AreaPsnCount.SelectedValue == "AdminArea")         //行政區人數     
            {
                sSql = @" SELECT CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, CardTime, EquName, 
                          CompanyID, DeptID, ItemNo, CtrlNo, LogEquGrpID, ROW_NUMBER() OVER(ORDER BY CompanyName, DeptName, ItemName, CardTime) AS NewIDNum FROM GetAdminAreaTable() ";
            }
            else if (Input_AreaPsnCount.SelectedValue == "WorkAndAdminArea")  //場區+行政區人數
            {
                sSql = @" SELECT *, ROW_NUMBER() OVER(ORDER BY CompanyName, DeptName, ItemName, CardTime) AS NewIDNum FROM 
                          (SELECT CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, CardTime, EquName, CompanyID, DeptID, ItemNo, CtrlNo, LogEquGrpID FROM GetWorkAreaTable() UNION 
                          SELECT CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, CardTime, EquName, CompanyID, DeptID, ItemNo, CtrlNo, LogEquGrpID FROM GetAdminAreaTable()) YA ";
            }
            else if (Input_AreaPsnCount.SelectedValue == "OtherArea")         //其它管制區人數
            {
                sSql = @" SELECT CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, CardTime, EquName, 
                          CompanyID, DeptID, ItemNo, CtrlNo, LogEquGrpID, ROW_NUMBER() OVER(ORDER BY CompanyName, DeptName, ItemName, CardTime) AS NewIDNum FROM GetOtherAreaTable() ";
            }
            else                                                              //請選擇
            {
                sSql = @" SELECT CompanyName, DeptName, ItemName, PsnName, PsnNo, CardNo, CardTime, EquName, 
                          CompanyID, DeptID, ItemNo, CtrlNo, LogEquGrpID, ROW_NUMBER() OVER(ORDER BY CompanyName, DeptName, ItemName, CardTime) AS NewIDNum FROM V_AreaCardLog ";

                sWhere = " CardNo = '99999999' ";
            }
            #endregion
            ...
            m_PsnCount  = m_ReportTable.Rows.Count;

            //儲存查詢資料與網頁資料
            Session["ReportTable"]  = m_ReportTable;
            Session["PastPageData"] = string.Format("{0};{1};{1}", m_PageCount, m_DataCount, m_PsnCount);
            #endregion
            #endregion
        }
        #endregion
        ...
        public void ExportExcel(DataTable oDataTable)
        {
            ...
            ExcelWorksheet ws     = pck.Workbook.Worksheets.Add("AreaPerson");
            ...
            ws.Cells[1, 7].Value = "刷卡時間";
            ws.Cells[1, 8].Value = "刷卡設備";
            ...
            for (int i = 0, iCount = oReportData.Rows.Count; i < iCount; i++)
            {
                ...
                {
                    ws.Cells[i + 2, j + 1].Value = oReportData.Rows[i][j].ToString().Replace("&nbsp;", "").Trim();
                }
            }
            ...
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment; filename=" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_AreaPerson.xlsx");
