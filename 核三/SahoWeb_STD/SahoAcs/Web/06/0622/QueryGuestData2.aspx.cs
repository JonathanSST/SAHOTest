...
namespace SahoAcs
{
    public partial class QueryGuestData2 : System.Web.UI.Page
    {
        ...
        {            
            ...
            ClientScript.RegisterClientScriptInclude("0622", "QueryGuestData2.js?" + DateTime.Now.ToString("yyyyMMddHHmmss"));//加入同一頁面所需的JavaScript檔案
        }
        ...
        {            
            ...
            string sql = "";            
            sql = @"SELECT COUNT(*) AS CardQty,A.PsnName,A.PsnNo,CONVERT(VARCHAR(12),CardTime,111) AS CardDate,PsnEName
                            FROM 
	                            B01_CardLog A 
	                            INNER JOIN B01_Person B ON A.PsnNo=B.PsnNo WHERE CardTime BETWEEN @CardTimeS AND @CardTimeE AND A.LogStatus=0
                            GROUP BY A.PsnName,A.PsnNo,CONVERT(VARCHAR(12),CardTime,111),PsnEName
                            ORDER BY PsnName,PsnNo,CardDate ";            
            this.ListLog = this.odo.GetQueryResult<CardLogModel>(sql, new
            {
                CardTimeS = Request["CardTimeS"],
                CardTimeE = Request["CardTimeE"]
                //OrgID = Request["Company"]
            }).ToList();
        ...
        private void ExportExcel()
        {
            ...
            ws.Cells[1, 2].Value = "業戶代碼";            
            ws.Cells[1, 3].Value = "業戶名稱";
            ws.Cells[1, 4].Value = "日期";
            ws.Cells[1, 5].Value = "刷卡數量";            
            //Content
            for (int i = 0; i < this.ListLog.Count; i++)
            {
                ...
                ws.Cells[i + 2, 4].Value = this.ListLog[i].CardDate;
                ws.Cells[i + 2, 5].Value = this.ListLog[i].CardQty;
            }            
