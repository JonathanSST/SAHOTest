...
namespace SahoAcs
{
    public partial class QueryPsnEquAuth : Page
    {
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    #region 設定欄位寛度
                    int[] HeaderWidth = { 80, 80, 100, 80, 80, 100,150 };
                ...
                case DataControlRowType.DataRow:
                    ...
                    #region 設定欄位寛度
                    int[] DataWidth = { 84, 84, 104, 84, 84, 104,154 };
                    for (int i = 0; i < e.Row.Cells.Count - 1; i++)
                    ...
                    e.Row.Cells[6].Text = LimitText(e.Row.Cells[6].Text, 40, true); // 設備名稱
                    #endregion
        ...
        public void Query(string SortExpression, string SortDire,bool _mode=false)
        {
            ...
            var sql1 = @" 
                SELECT DISTINCT 
                    ...
                    ,CONVERT(VARCHAR,CardAuth.BeginTime,111)+'~'+CONVERT(VARCHAR,CardAuth.EndTime,111) AS TimeRange
            ...
            if (!this.ChkOutDate.Checked){
                condition += " AND GETDATE() BETWEEN CardAuth.BeginTime AND CardAuth.EndTime ";
            }
