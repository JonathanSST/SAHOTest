...
{
    public partial class QueryInArea : System.Web.UI.Page
    {
        ...
        {
            ClientScript.RegisterClientScriptInclude("QueryInArea", "QueryInArea.js");        //加入同一頁面所需的JavaScript檔案
        ...
        public void ExportExcel()
        {
            ...
            for (int i = 0; i < this.logmaps.Count; i++)
            {
                ...
                ws.Cells[int_state + 2, 8].Value = logmaps[i].LogStatus;
                int_state++;
            }
        ...
        {            
            var param = new {
                DateS = Request["Calendar1"]+" 00:00:00", DateE= Request["Calendar1"] + " 23:59:59"
            };
        ...
        {            
            ...
            logmaps = this.odo.GetQueryResult<CardLogModel>(this.MainQueryLogStr, param).ToList();
            var statelist = this.odo.GetQueryResult(this.LogStateCmdStr);
            var OrgCompany = this.odo.GetQueryResult<OrgStrucInfo>("SELECT * FROM OrgStrucAllData('Company')");
            logmaps.ForEach(i =>
            {
