...
namespace SahoAcs.Web._0623
{
    public partial class PersonAreaInfo : System.Web.UI.Page
    {
        ...
        public List<dynamic> ListArea = new List<dynamic>();
        ...
        private void SetInitData()
        {
            ...
            this.ListArea = this.odo.GetQueryResult("SELECT * FROM B01_CtrlArea").ToList();
            #endregion            
        }
        ...
        private void SetQueryData()
        {
            ...
            sql = @"SELECT P.*,CtrlAreaName,EquNo,EquName,LastTime,CA.CtrlAreaNo FROM 
                    B01_CardExt A 
                        INNER JOIN V_PsnCard P ON A.CardID=P.CardID
                            INNER JOIN B01_EquData E ON LastDoorNo=EquNo 
                            INNER JOIN B01_CtrlArea CA ON A.CtrlAreaNo=CA.CtrlAreaNo WHERE (PsnName LIKE @PsnNo OR PsnNo LIKE @PsnNo ) AND LastTime>=@NowDate";
            this.txt_CardNo_PsnName = this.GetFormEqlValue("CardNoPsnNo");
            this.ListLog = this.odo.GetQueryResult<PsnExtData>(sql, new
            {
                PsnNo = this.GetFormEqlValue("CardNoPsnNo") + "%", NowDate=this.GetZoneTime().AddDays(-int.Parse(this.GetFormEqlValue("RangeDay")))
            }).ToList();            
        ...
        private void ExportExcel()
        {
            ...
            string CtrlName = "";
            if (!string.IsNullOrEmpty(this.GetFormEqlValue("CtrlArea")))
            {
                foreach(var o in this.odo.GetQueryResult("SELECT * FROM B01_CtrlArea WHERE CtrlAreaNo=@CtrlAreaNo", new { CtrlAreaNo = this.GetFormEqlValue("CtrlArea") }))
                {
                    CtrlName = string.Format("{1} ({0})", o.CtrlAreaNo, o.CtrlAreaName);
                }
            }
            if (this.ListLog.Count > 0)
            {
                ...
                {                  
                    sb.Append("<div style='width:100%;text-align:center;font-size:14pt'>管制區人員明細</div>");
                    sb.Append(string.Format("<table style='width:100%;font-size:10pt'><tr><td style='width:70%'>製表者：{0}/{1}</td><td style='width:30%'>製表時間：{2:yyyy/MM/dd HH:mm:ss}</td></tr></table>"
                        , Sa.Web.Fun.GetSessionStr(this, "UserID"), Sa.Web.Fun.GetSessionStr(this, "UserName"), DateTime.Now));
                    sb.Append(string.Format("<table style='width:100%;font-size:10pt'><tr><td style='width:25%'>分頁：{0}/{1}</td><td style='width:25%'>人員編號、姓名：{2}</td><td style='width:25%'>區間日：{4}</td><td style='width:25%'>管制區：{3}</td></tr></table>", 
                        p, PageData.PageCount, this.GetFormEqlValue("CardNoPsnNo"), CtrlName, this.GetFormEqlValue("RangeDay")));
                    ...
                    sb.Append("<td class='DataBar' style='width:15%'>最後讀卡時間</td></tr>");
            ...
            Response.AddHeader("content-disposition", "attachment;filename=管制區人員明細.pdf");
            Response.End();
        }//end method
