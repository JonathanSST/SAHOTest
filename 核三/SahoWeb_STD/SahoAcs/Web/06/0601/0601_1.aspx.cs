...
namespace SahoAcs
{
    public partial class _0601_1 : System.Web.UI.Page
    {
        ...
        {            
            ...
            ClientScript.RegisterClientScriptInclude("0601", "0601_1.js");//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetQueryData()
        {
            ...
            string sqlorgjoin = @"SELECT B01_EquData.EquNo, B01_EquData.EquName FROM B01_EquData
                ...
                WHERE B00_SysUserMgns.UserID = @UserID AND EquModel='FRS' GROUP BY B01_EquData.EquNo, B01_EquData.EquName";
        ...
        {
            System.Web.UI.WebControls.ListItem Item;
            string sql = "", wheresql = " EquModel='FRS' ";
            List<string> liSqlPara = new List<string>();
        ...
        void SetOneCardLog()
        {
            ...
            {
                var LogPic = this.odo.GetQueryResult("SELECT * FROM B02_FRSSCanLog WHERE CardNo=@CardNo AND PsnNo=@PsnNo AND CardLogTime=@CardTime", 
                    new { CardNo = logdata.CardNo, PsnNo = logdata.PsnNo, CardTime = logdata.CardTime.ToString("yyyy/MM/dd HH:mm:ss") });                
                foreach(var imagedata in LogPic)
                {
                    byte[] imageBytes = (byte[])imagedata.PicData;
                    PsnPicSource = Convert.ToBase64String(imageBytes);
                }
            }                        
            Response.Write(new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(
               new { message = "OK", card_log=logs.First(), error_msg = "",LogImage=PsnPicSource}));            
            Response.End();
        }
