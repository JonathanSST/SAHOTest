...
namespace SahoAcs
{
    public partial class QueryCardLog2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("QueryCardLog", "QueryCardLog2.js");//加入同一頁面所需的JavaScript檔案
        ...
        public void Query(string sMode, bool bMode, string SortExpression, string SortDire)
        {
            ...
            string sqlorgjoin = @"SELECT OrgData.OrgStrucID FROM OrgStrucAllData('Department') AS OrgData
            ...
            sql = @" SELECT *,OrgStrucID 
                FROM 
	                B01_CardLog CL
	                INNER JOIN B01_Person P ON CL.PsnNo=P.PsnNo ";
            if (orgdata.Count() > 0)
            {
                sql += " WHERE OrgStrucID IN @OrgStrucs ";
            }
            else
            {
                sql += " WHERE P.PsnNo=@UserID ";
            }
            ...
            if (sMode == "Normal")
            {
                ...
                this.cardlogs = this.odo.GetQueryResult<CardLogModel>(sql, new
                {
                    ...
                    OrgStrucs = orgdata.Select(i => i.OrgStrucID),
