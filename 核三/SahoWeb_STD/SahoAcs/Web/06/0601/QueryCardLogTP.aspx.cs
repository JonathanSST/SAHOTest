...
namespace SahoAcs
{
    public partial class QueryCardLogTP : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (Request["PageEvent"] != null && Request["PageEvent"] == "QueryCardLogStatus")
            {
                this.SetCardLogStatus();
            }
            else if (Request["PageEvent"] != null && Request["PageEvent"] == "UpdateCardLogStatus")
            {
                this.UpdateCardLogStatus();
            }
            else
            {
                this.SetInitData();
            ...
            ClientScript.RegisterClientScriptInclude("0601", "QueryCardLogTP.js");//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetInitData()
        {
            ...
            DateTime dtLastCardTime = oAcsDB.GetDateTimeScalar(@"SELECT MAX(CardTime) FROM B01_CardLog WHERE LogStatus=160 AND CardTime <= GETDATE()");
           ...
           this.ListCols.Add(new ColNameObj() { ColName = "CardVer", DataWidth = 64, TitleWidth = 60, TitleName = GetLocalResourceObject("ttCardVer").ToString() });
            ...
            this.ListCols.Add(new ColNameObj() { ColName = "LogTimeVal",DataRealName="LogTime", DataWidth = 84, TitleWidth = 84, TitleName = GetLocalResourceObject("ttLogTime").ToString() });
        ...
        private void SetQueryData()
        {
            ...
            sql = @"SELECT A.* FROM B01_CardLog A 	                    
                        WHERE (A.EquNo IN @EquList) AND LogStatus='160' ";
        ...
        private void ExportExcel()
        {
            ...
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment; filename=QueryCardLogTP.xlsx");
        ...
        #region Ceate dialog_List
        void SetCardLogStatus()
        {
            var CardLogStatus = this.odo.GetQueryResult("select code,StateDesc from B00_CardLogState where Code BETWEEN '48' AND '57' ");
            if (CardLogStatus.Count() > 0)
            {
                var CardLogStatusjson = JsonConvert.SerializeObject(CardLogStatus);
                Response.Write(new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(
                              new { message = "OK", card_log = CardLogStatusjson }));
                Response.End();
            }
            else
            {
                Response.Write(new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(
                              new { message = "NO", card_log = CardLogStatus }));
                Response.End();
            }
        }
        #endregion  Ceate dialog_List_End


        void UpdateCardLogStatus()
        {
            #region 取得客戶端IP位址
            ...
            SqlStr = @"update B01_CardLog set  LogStatus=@LogStatus,SyncMark3=0 where RecordID=@RecordID";
            odo.BeginTransaction();
            int IsUpdate = odo.Execute(SqlStr, new
            {
                LogStatus = SelectStatus,
                RecordID = recordID
            }) ;
            if (IsUpdate > -1)
            {
                ...
                var LogStatus = this.odo.GetQueryResult("select * from B01_CardLog where RecordID=@RecordID",new { RecordID = recordID });
                SysLogEntity _logObj = new SysLogEntity();
                var logstatus = this.odo.GetQueryResult("SELECT * FROM B00_CardLogState");
                var CurrentCardStatus = logstatus.Where(i => Convert.ToInt32(i.Code) == Convert.ToInt32(SelectStatus)).FirstOrDefault().StateDesc;
                _logObj.LogType = DB_Acs.Logtype.資料修改.ToString();
                _logObj.UserID = Session["UserID"].ToString();
                _logObj.UserName = Session["UserName"].ToString();
                _logObj.LogFrom = "0601";
                _logObj.LogIP = sIPAddress;
                _logObj.EquNo = LogStatus.First().EquNo;
                _logObj.EquName = LogStatus.First().EquName; ;
                _logObj.LogInfo =string.Format("卡號:{0} , 人員:{1} , 變更出勤狀況:{2} =>{3}", LogStatus.First().CardNo,LogStatus.First().PsnName, "未授權卡片", CurrentCardStatus) ;
                _logObj.LogDesc = DB_Acs.Logtype.資料修改.ToString(); ;
                odo.SetSysLogCreate(_logObj);
                #endregion 處理log_End
        ...
        {
            var logstatus = this.odo.GetQueryResult("SELECT * FROM B00_CardLogState");
            var CardLogStatus = this.odo.GetQueryResult("select code,StateDesc from B00_CardLogState where code = '160' ");
            var CurrentCardStatus = logstatus.Where(i => Convert.ToInt32(i.Code) == Convert.ToInt32(CardLogStatus.First().code)).FirstOrDefault().StateDesc;      
            this.ChkLogStatus.Value = Convert.ToString(CurrentCardStatus);
        }
    }//end page class
}//end namespace