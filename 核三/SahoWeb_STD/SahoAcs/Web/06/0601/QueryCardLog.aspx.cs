...
namespace SahoAcs
{
    public partial class QueryCardLog : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("0601", "QueryCardLog.js?" + Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetInitData()
        {
            ...
            #region 體溫設定值
            SetHeatVal = float.Parse(Pub.SetHeat);
            #endregion

            #region 是否顯示建檔照片
            ShowCreatePic = int.Parse(Pub.isShowCreatePic);
            #endregion
        }
        ...
        private void SetQueryData()
        {
            ...
            sql = @"SELECT * FROM V_CardLogOrg WHERE (EquNo IN @EquList)"; 
            ...
            foreach (DataRow r in this.DataResult.Rows)
            {
                ...
                if (r["HeatResult"].ToString() != "" && Request["PageEvent"] == "Query")
                {
                    if (float.Parse(r["HeatResult"].ToString()) >= SetHeatVal)
                    {
                        r["HeatResult"] = $"<span style='color:red'>{r["HeatResult"]}</span>";
                    }
                }
            }
        }
        ...
        private string GetConditionCmdStr()
        {
            ...
            if (Request["QueryMode"] == "2")
            {
                ...
                if (Request["DepNo"] != null && Request["DepNo"] != "")
                {
                    ...
                    sql += " AND ((oComID IN @oDepID) OR (oDepID IN @oDepID))";
                }
            }
            //一般查詢的方法
        ...
        private void ExportExcel()
        {
            ...
            for (int i = 0; i < this.DataResult.Rows.Count; i++)
            {
                for (int col = 0; col < this.ListCols.Count(); col++)
                {
                    ...
                    if (this.ListCols[col].ColName.ToString() == "HeatResult")
                    {
                        if (this.DataResult.Rows[i][this.ListCols[col].ColName].ToString() != "" && float.Parse(this.DataResult.Rows[i][this.ListCols[col].ColName].ToString()) >= SetHeatVal)
                        {
                            ws.Cells[i + 2, col + 1].Style.Font.Color.SetColor(Color.Red);
                        }
                    }
                }
            }
        ...
        private void CreateDropDownList_DepItem()
        {
            ...
            //         INNER JOIN B00_SysUserMgns AS SysUserMgns ON SysUserMgns.MgaID = MgnOrgStrucs.MgaID  ";
            //sqlouttable += "SELECT * FROM (";
            //sqlouttable += sql.Replace("@Type", "Company");
            //sqlouttable += " UNION ";
            //sqlouttable += sql.Replace("@Type", "Department");
            //sqlouttable += ") AS ResultTable ";
            //#region DataAuth
            ...
            else
            {
                ...
                DropDownList_Company.Items.Add(new ListItem() { Text="尚無資料", Value = ""});
            }
        }
        #endregion
        ...
        void SetOneCardLog()
        {
            ...
            Response.Clear();
            string PsnPicSource = "/Img/Default.png";
            string PsnImgData = "";
            string Url = "http://127.0.0.1:8080/";
            string tmpSuccessLogPicFolder = "";
            string tmpFailLogPicFolder = "";
            string LogPicFolder = "";

            var PsnPicSourceUrl = this.odo.GetQueryResult("SELECT * FROM B00_SysParameter WHERE ParaClass='Person' AND ParaNo in ('Url','LogPicFolder','FailLogPicFolder') ");
            foreach (var o in PsnPicSourceUrl)
            {
                switch (o.ParaNo)
                {
                    case "Url":
                        ...
                        break;
                    case "LogPicFolder":
                        tmpSuccessLogPicFolder = Convert.ToString(o.ParaValue);
                        break;
                    case "FailLogPicFolder":
                        tmpFailLogPicFolder = Convert.ToString(o.ParaValue);
                        break;
                }
            ...
            var logs = this.odo.GetQueryResult<CardLogModel>(@"declare @input varchar(max), @sep varchar(2)
                    ...
                    SELECT B01_CardLog.*,isnull(CamNo,'') AS CamNo,DATEADD(HOUR,-V.TimeZone,CardTime) AS CardTime FROM B01_CardLog INNER JOIN V_MCRInfo V  ON B01_CardLog.EquNo=V.EquNo
            ...
            LogPicFolder = tmpSuccessLogPicFolder;
            if (logs.First().LogStatus == "160")
            {
                LogPicFolder = tmpFailLogPicFolder;
            }

            string path = System.IO.Path.GetDirectoryName(this.Request.PhysicalPath);
            path = Request.Url.AbsoluteUri;
            path = path.Substring(0, path.ToLower().LastIndexOf(@"/web"));
            path = string.Concat(path, "/" + LogPicFolder + "/", logs.First().RecordID, ".", WebAppService.GetSysParaData("CardPic"));
            ...
            if (Pub.isShowCreatePic == "1")
            {
                if (Pub.isUserFaceImg == "0")
                {
                ...
                {
                    string sqlStr = @"
                    SELECT G.ImageData FROM (
                        SELECT  V_PsnCard.PsnNo, V_PsnCard.CardNo, BS3.ImageData FROM V_PsnCard 
                        LEFT OUTER JOIN B02_FaceImageDataBS3 AS BS3 ON V_PsnCard.CardNo = BS3.CardNo
                        UNION
                        SELECT  V_PsnCard.PsnNo, V_PsnCard.CardNo, F3.ImageData FROM V_PsnCard 
                        LEFT OUTER JOIN B02_FaceImageData3 AS F3 ON V_PsnCard.CardNo = F3.CardNo
                        UNION
                        SELECT  V_PsnCard.PsnNo, V_PsnCard.CardNo, F2.ImageData FROM V_PsnCard 
                        LEFT OUTER JOIN B02_FaceImageData2 AS F2 ON V_PsnCard.CardNo = F2.CardNo
                        UNION
                        SELECT  V_PsnCard.PsnNo, V_PsnCard.CardNo, F1.ImageData FROM V_PsnCard 
                        LEFT OUTER JOIN B02_FaceImageData AS F1 ON V_PsnCard.CardNo = F1.CardNo
                    ) AS G
                    WHERE G.PsnNo = @PsnNo AND ImageData IS NOT NULL
                    ";

                    var PsnFaceDatas = this.odo.GetQueryResult<byte[]>(sqlStr, new { PsnNo = logs.First().PsnNo });
                    if (PsnFaceDatas.Count() > 0)
                    {
                        PsnPicSource = "data:image/png;base64, " + Convert.ToBase64String(PsnFaceDatas.First());
                        PsnFaceDatas = null;
                    }
                }
            ...
            Response.Write(new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(
               new { message = this.odo.DbExceptionMessage, card_log = logs.First(), error_msg = "", PsnPicSource = PsnPicSource }));
            Response.End();
        }
