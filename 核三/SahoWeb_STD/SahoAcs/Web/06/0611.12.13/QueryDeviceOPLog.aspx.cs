...
namespace SahoAcs
{
    public partial class QueryDeviceOPLog : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("QueryDeviceOPLog", "QueryDeviceOPLog.js?"+Pub.GetNowTime);//加入同一頁面所需的JavaScript檔案
            ...
            if (!IsPostBack && !oScriptManager.IsInAsyncPostBack)
            {
                ...
                Input_LogTimeBegin.DateValue = this.GetZoneTime().ToString("yyyy/MM/dd") + " 00:00:00";
                Input_LogTimeEnd.DateValue = this.GetZoneTime().ToString("yyyy/MM/dd") + " 23:59:59";
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    #region 設定欄位寛度
                    int[] HeaderWidth = { 10, 160,120, 70, 100, 80, 110 };
                ...
                case DataControlRowType.DataRow:
                    ...
                    #region 設定欄位寛度
                    int[] DataWidth = { 10, 163,124, 74, 104, 84, 114 };
                    for (int i = 0; i < e.Row.Cells.Count - 1; i++)
                    ...
                    e.Row.Cells[4].Text = LimitText(e.Row.Cells[4].Text, 11, true);
                ...
                case DataControlRowType.Pager:
                    ...
                    for (int i = startIndex; i < endIndex; i++)
                    {
                        ...
                        if (i == pageIndex)
                        {
                            ...
                            lbtnPage.OnClientClick = "Block();return false;";
                        }
                        else
...
//                     INNER JOIN B00_SysUser AS SysUser ON SysUser.UserID = SysUserMgns.UserID ";
            ...
            sql = @" SELECT ROW_NUMBER() OVER(ORDER BY " + SortExpression + " " + SortDire + @") AS NewIDNum,
                     B00_SysDeviceOPLog.RecordID, B00_SysDeviceOPLog.CreateTime AS LogTime, B00_ItemList.ItemName AS DOPState, 
                      CASE DOPActive WHEN 'SetOnceCardData' THEN '單筆設碼'
					 WHEN 'CancleOnceCardData' THEN '消除所有設碼'
					 WHEN 'CardDataCountGet' THEN '設碼筆數取得'
                    WHEN 'SetAllCardDataEnd' THEN '單機設碼結束'
					 WHEN 'SetAllCardDataStart' THEN '單機設碼開始'
                     WHEN 'SetCardBlockData' THEN '批次設碼'
					 WHEN 'SetOnceCardData' THEN '單筆設碼'
                    WHEN 'CardModeSet' THEN '讀卡規則設定'
					 WHEN 'CtrlerTimeGet' THEN '取得控制器時間'
                    WHEN 'CtrlerTimeSet' THEN '設定控制器時間'
					 WHEN 'LimitOneControlSet' THEN '設定一進一出控制'
					 END AS DOPActiveName, 
                    B00_SysDeviceOPLog.EquNO,
            ...
            if (!string.IsNullOrEmpty(ViewState["query_LogTimeBegin"].ToString().Trim()))
            {
                ...
                liSqlPara.Add("D:" + DateTime.Parse(ViewState["query_LogTimeBegin"].ToString()).GetUtcTime(this).ToString().Trim());
            }
            ...
            if (!string.IsNullOrEmpty(ViewState["query_LogTimeEnd"].ToString().Trim()))
            {
                if (wheresql != "") wheresql += " AND ";
                wheresql += " (B00_SysDeviceOPLog.CreateTime <= ?) ";
                liSqlPara.Add("D:" + DateTime.Parse(ViewState["query_LogTimeEnd"].ToString()).GetUtcTime(this).ToString().Trim());
            }
            ...
            #endregion
            foreach(DataRow r in CardLogTable.Rows)
            {
                r["LogTime"] = Convert.ToDateTime(r["LogTime"]).GetZoneTime(this);
            }
        ...
        public static string[] LoadData(string SelectValue)
        {
            ...
            sql = @" SELECT
                     DeviceOPLog.CreateTime AS LogTime,
                     StateList.ItemName AS DOPState, DeviceOPLog.EquNO,
                     DeviceOPLog.CreateUserID AS UserID, DeviceOPLog.IPAddress AS UserIP, DeviceOPLog.ResultMsg,DOPActive
                     FROM B00_SysDeviceOPLog AS DeviceOPLog 
                     LEFT JOIN B00_ItemList AS StateList ON StateList.ItemClass = 'DOPState' AND StateList.ItemNo = DeviceOPLog.DOPState AND StateList.ItemInfo1 = DeviceOPLog.DOPType
            ...
            if (dr.Read())
            {
                ...
                for (int i = 0; i < dr.DataReader.FieldCount; i++)
                {
                    ...
                    {
                        string datezone = Convert.ToDateTime(dr.DataReader["LogTime"]).GetZoneTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                        TableData[i] = datezone;
                    }
                    //else if (i == dr.DataReader.FieldCount - 2)
                    //{
                    //    string resultmsg=dr.DataReader["ResultMsg"].ToString();                        
                    //    string dopactive = dr.DataReader["DOPActive"].ToString() == "SetOnceCardData" ? "設碼" : "消碼";
                    ...
                    //    }
                    //    else if(!dr.DataReader["DOPActive"].ToString().Contains("Card"))
                    //    {
                    //        TableData[i] ="設備 "+ dr.DataReader["EquNo"].ToString()+" 處理 "+dr.DataReader["DOPActive"].ToString()+ " 設碼 " + dopstate;
                    //    }
                    //    else
                    //    {
