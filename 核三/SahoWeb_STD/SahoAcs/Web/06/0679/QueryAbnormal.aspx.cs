...
{
    public partial class QueryAbnormal : System.Web.UI.Page
    {
        ...
        {            
            ...
            ClientScript.RegisterClientScriptInclude("IncludeScript", "QueryAbnormal.js");//加入同一頁面所需的JavaScript檔案
        }
        ...
        private void SetQueryData()
        {
            ...
            {                
                ...
                DateS = this.GetFormEqlValue("CardTimeS"),
                DateE = this.GetFormEqlValue("CardTimeE"),
            ...
            {
                //轉datatable                
                this.DataResult = OrmDataObject.IEnumerableToTable(this.CardLogList);
            }
            else
            {
                //轉datatable
                this.PagedList = this.CardLogList.ToPagedList(PageIndex, 100);
        ...
        private void ExportPdf()
        {
            ...
            if (this.CardLogList.Count > 0)
            {
                var PageData = this.CardLogList.ToPagedList(1, 18);
                for(int i=1;i<=PageData.PageCount;i++)
                {
                    sb.Append("<div style='width:100%;text-align:center;font-size:14pt'>打卡紀錄異常表</div>");
                    ...
                    foreach (var o in this.CardLogList.ToPagedList(i, 18))
                    {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=打卡紀錄異常表.pdf");
            Response.End();
        }
