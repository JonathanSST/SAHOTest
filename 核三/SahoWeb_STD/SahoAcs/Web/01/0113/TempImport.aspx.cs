...
namespace SahoAcs
{
    public partial class TempImport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("TempImport", "TempImport.js");//加入同一頁面所需的JavaScript檔案
        ...
        protected void PsnCard_ImportButton_Click(object sender, EventArgs e)
        {
            ...
            {
                #region 新增Psn資料(臨時人員不新增PERSON，只改姓名)
                //if (iRet > -1)
                //{
               ...
               #region 更新臨時人員姓名(person已存在，只更新每次不同臨時人員姓名)
               String sql = @"UPDATE B01_Person SET PsnName = a.PsnName FROM B01_Person INNER JOIN 
                        (select psnno, PsnName from B00_SyncTable_PersonCard group by psnno, PsnName) as a 
                        ON a.PsnNo = B01_Person.PsnNo";
                ...
                #region 新增Card資料(臨時人員不處理卡片，只改姓名)
                //       if (iRet > -1)
                //       {
                ...
                if (iRet > -1)
                {
                    sSql = @"DELETE FROM B01_ExamPerson 
                        ...
                        AND 'T' = (SELECT PsnType FROM B01_Person WHERE PersonNo = B01_Person.PsnNo)";
                    iRet = oAcsDB.SqlCommandExecute(sSql);
                }
        ...
        private bool CheckPsnCardCorrect(out int ReturnRowCount)
        {
            ...
            #region 檢查Person是否已存在系統中(臨時人員轉入不檢查，因為已存在是正常的)
            //if (ErrorCount == 0)
            //{
            //    sSql = @" SELECT COUNT(*) FROM B00_SyncTable_PersonCard AS SyncTable_PersonCard
            ...
            #region 檢查Card是否已存在系統中(臨時人員轉入不檢查，因為已存在是正常的)
            //if (ErrorCount == 0)
            //{
            //    sSql = @" SELECT COUNT(*) FROM B00_SyncTable_PersonCard AS SyncTable_PersonCard
