...
namespace SahoAcs
{
    public partial class TempCardRecordWithArea : System.Web.UI.Page
    {
        #region 一.宣告
        ...
        private static string strAreaList = "SELECT * FROM B00_ItemList WHERE ItemClass='AreaType' ORDER BY ItemOrder";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var result = this.odo.GetQueryResult(strAreaList);
        ...
        private void RegisterComponeAndScript()
        {
            ...
            ClientScript.RegisterClientScriptInclude("TempCardRecordWithArea", "TempCardRecordWithArea.js");
        }
        ...
        private void RegisterWindowsAndButton()
        {
            ...
            this.ddlCardArea.Attributes["OnChange"] = "SelectArea()";
            //註冊次作業畫面的按鈕動作
        ...
        public void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.Header:
                    ...
                    e.Row.Cells[3].Width = 100;      //原來卡片號碼
                    e.Row.Cells[4].Width = 100;      //借出卡片號碼
                    e.Row.Cells[5].Width = 80;          //廠區
                    e.Row.Cells[6].Width = 145;      //借出時間
                    e.Row.Cells[7].Width = 145;      //歸還時間
                ...
                case DataControlRowType.DataRow:
                    ...
                    e.Row.Cells[3].Width = 104;      //原來卡片號碼
                    e.Row.Cells[4].Width = 104;      //借出卡片號碼
                    e.Row.Cells[5].Width = 84;
                    e.Row.Cells[6].Width = 149;      //借出時間
                    e.Row.Cells[7].Width = 149;      //歸還時間
                    ...
                    if (!String.IsNullOrEmpty(e.Row.Cells[7].Text.Replace("&nbsp;", "").Trim()))
                        e.Row.Cells[7].Text = DateTime.Parse(e.Row.Cells[7].Text).ToString("yyyy/MM/dd HH:mm:ss");
        ...
        public void Query(string SortExpression, string SortDire)
        {
            ...
            sql = @" 
                SELECT TCR.*, P.PsnName,IL.ItemName AS AreaName
                FROM B01_TempCardRecord AS TCR 
                LEFT JOIN B01_Card  C ON TCR.CardNo=C.CardNo
                LEFT JOIN B00_ItemList AS IL ON IL.ItemNo=C.Rev02 AND ItemClass='AreaType'
        ...
        public int Query(string mode, string SortExpression, string SortDire)
        {
            ...
            sql = @" 
                SELECT 
                    TCR.*, P.PsnName,IL.ItemName AS AreaName
                FROM B01_TempCardRecord AS TCR 
                LEFT JOIN B01_Card  C ON TCR.CardNo=C.CardNo
                LEFT JOIN B00_ItemList AS IL ON IL.ItemNo=C.Rev02 AND ItemClass='AreaType'
                LEFT JOIN B01_Person AS P ON P.PsnNo = TCR.PsnNo ";
        ...
        public static string[] LoadData(string sRecordID, string sActFunc)
        {
            ...
            sql  = @" 
                ...
                LEFT JOIN B01_Card AS C ON TR.CardNo = C.CardNo
                WHERE TR.RecordID = ? ";
        ...
        public static object Insert(string sPsnNo, string sOrigCardNo, string sCardNo, string sBorrowTime, string sTempDesc)
        {
            ...
            oAcsDB.WriteLog(DB_Acs.Logtype.資料新增, strUserId, strUserName, "0107", "", "", string.Format("RecordID={0} PsnNo={1} OrigCardNo={2} CardNo={3} BorrowTime={4} TempDesc={5} CreateUserID={6} CreateTime={7}", sRecordID, sPsnNo.Trim(), sOrigCardNo.Trim(), sCardNo.Trim(), sBorrowTime, sTempDesc.Trim(), UserID.Trim(), sNowTime.Trim()), "B01_TempCardRecord");
            #endregion
        ...
        public static object Update(string SelectValue, string sReturnTime, string sTempDesc)
        {
            ...
            liSqlPara.Clear();
            liSqlPara.Add("S:" + DateTime.Parse(sReturnTime).ToString("yyyy-MM-dd HH:mm:ss.fff"));
            liSqlPara.Add("S:" + sTempDesc.Trim());
        ...
        public static object ExcuteCardAuthCopy(string sUserID, string sSourceCardNo, string[] sObjectCardNoArray)
        {
            ...
            sSql = @" 
                SELECT 
                    CardID, CardNo, CardAuthAllow, CardSTime, CardETime, PsnID 
                FROM B01_Card 
                WHERE CardID = ? ";
            ...
            {
                string sCardIDValue1 = "", sCardNoValue1 = "", sCardAuthAllow1 = "", sCardSTime1 = "", sCardETime1 = "", sPsnID = "";
        ...
        private void UpdateTempCardVer()
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            string strSQL = @"
                UPDATE B01_Card Set CardVer='', CardSerialNo='' 
