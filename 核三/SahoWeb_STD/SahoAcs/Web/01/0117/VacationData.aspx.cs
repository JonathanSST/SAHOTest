...
namespace SahoAcs
{
    public partial class VacationData : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("VacationData", "VacationData.js");  //加入同一頁面所需的JavaScript檔案
        ...
        private void Query(bool select_state, string SortExpression, string SortDire)
        {
            ...
            sql = " SELECT * FROM B00_VacationData ";
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] sNoAY, string sMode)
        {
            ...
            if (sMode == "Insert")
            {
                ...
                if (string.IsNullOrEmpty(sNoAY[0].Trim()))
                {
                    ...
                    sRetMsg.message += "假別代號 必須輸入";
                }

                if (sNoAY[0].Contains(" "))
                {
                    sRetMsg.result = false;
                    sRetMsg.message += "假別代號字串中不可有空白符號";
                }
            ...
            {
                case "Insert":
                    sSql = @" SELECT * FROM B00_VacationData WHERE VNo = ?; ";
                    sSqlParaLT.Add("S:" + sNoAY[0].Trim());
                    break;
                case "Update":
                    sSql = @" SELECT * FROM B00_VacationData WHERE VNo = ? AND VID<>?";
                    sSqlParaLT.Add("S:" + sNoAY[0].Trim());
                    sSqlParaLT.Add("S:" + sNoAY[2].ToString());
                    break;
            }
        ...
        protected void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.DataRow:
                    ...
                    GrRow.ID = "GV_Row" + oRow.Row["VID"].ToString();
                    #endregion

                    #region 設定欄位寛度
                    e.Row.Cells[0].Width = 50;
                    e.Row.Cells[1].Width = 83;
                    e.Row.Cells[2].Width = 110;
                    e.Row.Cells[3].Width = 110;
                    e.Row.Cells[4].Width = 174;
                    e.Row.Cells[5].Width = 110;
                    e.Row.Cells[6].Width = 165;
                    #endregion
                    ...
                    e.Row.Attributes.Add("OnClick", "SingleRowSelect(0, this, SelectValue,'" + oRow.Row["VID"].ToString() + "', '', '');");
        ...
        public static string[] LoadData(string sVID, string sUserID, string sMode)
        {
            string sSql = "";
            DB_Acs oAcsDB = null;
            DBReader oDbRdr = null;
            ...
            sSql = @" SELECT * FROM B00_VacationData WHERE VID = ? ";
            sSqlParaLT.Add("S:" + sVID.Trim());
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object Update(string sVID, string sVNo, string sVName, string sUserID, string sNowNo)
        {
            string sSql = "";
            DB_Acs oAcsDB = null;
            string[] sNoAY = new string[8];
            List<string> sSqlParaLT = new List<string>();
            Pub.MessageObject sRetMsg = new Pub.MessageObject();
            DataTable dt = new DataTable();
            sNoAY[0] = sVNo;
            ...
            if (sRetMsg.result)
            {
                #region Process String
                oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
                sSqlParaLT.Add("S:" + sNowNo);
                oAcsDB.GetDataTable("t1", "SELECT * FROM B03_PsnHoliday WHERE HoliNo=? ", sSqlParaLT, out dt);
                if (!sNowNo.Equals(sVNo) && dt.Rows.Count>0)
                {                  
                    sRetMsg.result = false;
                    sRetMsg.message = "此假別設定已在使用中，無法修改";
                }
                else
                {
                    sSqlParaLT.Clear();
                    sSql = @" UPDATE B00_VacationData SET VNo = ?, VName = ?, UpdateUserID = ?, UpdateTime = GETDATE() WHERE VID = ?; ";
                    sSqlParaLT.Add("S:" + sVNo.Trim());
                    sSqlParaLT.Add("S:" + sVName.Trim());
                    sSqlParaLT.Add("S:" + sUserID);
                    sSqlParaLT.Add("S:" + sVID);
                    #endregion
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object Insert(string sVNo, string sVName, string sUserID)
        {
            string sSql = "";
            DB_Acs oAcsDB = null;
            DBReader oDbRdr = null;
            ...
            if (oRetMsg.result)
            {
                ...
                sSql = @" INSERT INTO B00_VacationData(VNo, VName, CreateUserID) 
                         VALUES (?, ?, ?) ";
                sSqlParaLT.Add("S:" + sVNo.Trim());
                sSqlParaLT.Add("S:" + sVName.Trim());
                sSqlParaLT.Add("S:" + sUserID);
                ...
                if (oAcsDB.SqlCommandExecute(sSql, sSqlParaLT) > -1)
                {
                    ...
                    {
                        #region 取得新的資料ＩＤ
                        sSql = @" SELECT VID FROM B00_VacationData WHERE VNo = ?; ";
                        sSqlParaLT.Clear();
                        sSqlParaLT.Add("S:" + sVNo);
                        ...
                        if (oAcsDB.GetDataReader(sSql, sSqlParaLT, out oDbRdr))
                        {
                            ...
                            {
                                oDbRdr.Read();
                                oRetMsg.message = sVNo + "|" + oDbRdr.DataReader["VID"].ToString();
                            }
                        }
                        #endregion
                    }
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object Delete(string sVID, string sUserID)
        {
            string sSql = "";
            DB_Acs oAcsDB = null; ;
            List<string> sSqlParaLT = new List<string>();
            ...
            #region Process String
            oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));            
            sSql = @" DELETE FROM B00_VacationData WHERE VID = ? AND VNo NOT IN (SELECT DISTINCT HoliNo FROM B03_PsnHoliday); ";
            sSqlParaLT.Add("S:" + sVID.Trim());
            DataTable dt0 = new DataTable();
            oAcsDB.GetDataTable("t1", "SELECT * FROM B03_PsnHoliday WHERE HoliNo IN (SELECT VNo FROM B00_VacationData WHERE VID=?)", sSqlParaLT, out dt0);
            if (dt0.Rows.Count == 0)
            {
                oAcsDB.BeginTransaction();
                if (oAcsDB.SqlCommandExecute(sSql, sSqlParaLT) > -1)
            ...
            {
                oRetMsg.result = false;
                oRetMsg.message = "此假別設定已在使用中，無法刪除";
            }
