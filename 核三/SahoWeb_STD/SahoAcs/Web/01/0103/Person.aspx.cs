...
namespace SahoAcs
{
    public partial class Person : System.Web.UI.Page
    {
        ...
        private int years = 90;     //年份選單的預設年數
        private int yongyear = 10;  //成年人歲數
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person.js?jsdate=" + DateTime.Now.ToString("yyyyMMddHHmmss")); //加入同一頁面所需的JavaScript檔案
            ...
            Input_TrainingEDate.SetWidth(180);
        }
        #endregion
        ...
        private void RegisterObj()
        {
            ...
            #region 註冊主頁Button動作
            this.InputWord.Attributes["onkeydown"] = "CheckKeyUp()";
            ...
            btnMakeCard.Attributes["onClick"] = "CellMakeCard(); return false;";
            #endregion
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.MaxPerson.Value = DongleVaries.GetMaxPerson().ToString();
            try
            {
        ...
        private void Page_Init(object sender, EventArgs e)
        {
            ...
            #region Sam 20190131 增加組織架構異常處理
            oAcsDB.SqlCommandExecute(@"UPDATE B01_Person SET OrgStrucID=(SELECT MIN(OrgStrucID) FROM B01_OrgStruc) WHERE OrgStrucID IN
                                                                (SELECT OrgStrucID FROM B01_OrgStruc WHERE OrgIDList LIKE '%undefine%' OR OrgIDList LIKE '%null%')");
            oAcsDB.SqlCommandExecute(@"DELETE B01_OrgStruc WHERE OrgIDList  LIKE '%undefine%' OR OrgIDList LIKE '%null%'");
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    ...
                    while (drOrg.Read())
                    {
                        ...
                        if (sOrgClass == "Company") { dropCompany.Items.Add(oItem); }
                        else if (sOrgClass == "Department" || sOrgClass == "Unit") { dropDepartment.Items.Add(oItem); }
        ...
        private void SetCardItemInfo()
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Session["CardLen"] = oAcsDB.GetIntScalar(" SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'CardID' AND ParaNo = 'CardLen' ");
            Session["CardVer"] = oAcsDB.GetStrScalar(" SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'CardID' AND ParaNo = 'CardVer' ");
            ...
            this.popInput_CardPW.MaxLength = oAcsDB.GetIntScalar("SELECT ISNULL(MAX(ParaValue),4) FROM B00_SysParameter WHERE ParaClass='System' AND ParaNo='CardPwdLen' ");
            CardPwLength = this.popInput_CardPW.MaxLength;
            this.hCardPwdAlertMsg.Value = string.Format("卡片密碼長度必須為{0}碼", this.popInput_CardPW.MaxLength);
            popInput_CardVer.Enabled = Session["CardVer"].ToString() == "Y" ? true : false;
            Input_CardVer.Enabled = popInput_CardVer.Enabled;
            Input_CardNo.Enabled = false;
            //驗證卡號認證模式的資訊是否顯示            
        ...
        {
            DB_Acs oAcsDB = new DB_Acs(this.Page, Pub.GetConnectionString(Pub.sConnName));
            this.CurrentPerson.Value = oAcsDB.GetStrScalar("SELECT COUNT(*) AS Currents FROM B01_Card");
            string sql = "", wheresql = "";
            DataTable dt;
            DBReader dr = null;
            ...
            wheresql += " UserID=? ";
            liSqlPara.Add("S:" + Sa.Web.Fun.GetSessionStr(this, "UserID"));
            if (!string.IsNullOrEmpty(NowData))
            {
                if (wheresql != "")
                    wheresql += " AND ";
                wheresql += " (OrgNameList LIKE ? OR PsnNo LIKE ? OR PsnName LIKE ? OR (CardNo=? AND ISNULL(B01_Card.PsnID,0)!=0)) ";
            ...
            UpdatePanel1.Update();
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "CheckMax", "SetUserLevel('" + Session["FunAuthSet"].ToString() + "')", true);
            hSelectState.Value = "false";
        }
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string mode)
        {
            ...
            List<string> liSqlPara = new List<string>();
            Sa.DB.DBReader dr;
            int cardlen = int.Parse(oAcsDB.GetStrScalar("SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' AND ParaNo = 'CardDefaultLength'"));
            string isUseIDNum = oAcsDB.GetStrScalar("SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'Person' AND ParaNo = 'IDNumMustKeyIn'");
            int IDNumLen = int.Parse(oAcsDB.GetStrScalar("SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'Person' AND ParaNo = 'IDNumLen'"));
            ...
            if (mode == "Insert")
            {
                ...
                {
                    if (string.IsNullOrEmpty(NoArray[4].Trim()))
                    {
                        if (!string.IsNullOrEmpty(sRet.message)) sRet.message += "\n";
                        sRet.result = false;
                        sRet.message += "身份證號為必填";
                    }
                    else if (NoArray[4].Trim().Length != IDNumLen)
                    {
                        if (!string.IsNullOrEmpty(sRet.message)) sRet.message += "\n";
                        sRet.result = false;
                        sRet.message += string.Format("身份證號長度必須為{0}碼", IDNumLen);
                    }
                    else
                    {
                        System.Text.RegularExpressions.Regex regex = new System.Text.RegularExpressions.Regex(@"^([A-Z])([1-2]\d{8})$");
                        Match match = regex.Match(NoArray[4].Trim().ToUpper());
                        if (!match.Success)
                        {
                            if (!string.IsNullOrEmpty(sRet.message)) sRet.message += "\n";
                            sRet.result = false;
                            sRet.message += "身份證號格式有錯";
                        }
                    }
                }
        ...
        protected static string ConvertLettersToNumbers(string input)
        {
            char character = Convert.ToChar(input.ToUpper());
            if (Char.IsLetter(character))
            {
                int numericValue = character - 'A' + 1; // 將字母轉換為數字
                return numericValue.ToString("D2"); // 使用D2格式保證輸出兩位數字
            }
        ...
        public static ArrayList SingleData(String sPsnID, String UserID)
        {
            ...
            String sql = @"SELECT PsnID, PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, OrgStrucID,
                ...
                Text1, Text2, Text3, Text4, Text5, TrainingEDate FROM B01_Person WHERE PsnNo = '" + sPsnID + "'";
            oAcsDB.GetDataReader(sql, out dr);
            if (dr.HasRows)
            {
                EditData = new string[dr.DataReader.FieldCount + 4];//基本資訊(22) + 額外資訊(4) 
                    ...
                    for (int i = 0; i < dr.DataReader.FieldCount; i++)
                    {
                        ...
                        else if (i == 11 || i == 12 || i == 21)
                        {
                            if (!Convert.IsDBNull(dr.DataReader[i]))
                            {
                                ...
                                EditData[i] = DateTime.Parse(dr.DataReader[i].ToString()).GetZoneTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                            }
                        }
                        else
                        {
            ...
            if (EditData.Length > 2)
            {
                ...
                #region 卡片資訊
                dr = null;
                sql = @"SELECT B01_Card.CardID, B01_Card.CardNo, B00_ItemList.ItemName, B01_Card.CardRule, B01_Card.CardLock
                    FROM B01_Card INNER JOIN B00_ItemList ON B01_Card.CardType = B00_ItemList.ItemNo
                    WHERE ItemClass = 'CardType' AND PsnID = '" + EditData[0] + "' AND CardType NOT IN ('R','TEMP') ";
                oAcsDB.GetDataReader(sql, out dr);
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        EditData[EditData.Length - 2] += dr.DataReader["CardNo"].ToString() + "|" +
                            ...
                            dr.DataReader["CardRule"].ToString() + "|" +
                            dr.DataReader["CardLock"].ToString() + "|";
                    }
        ...
        public static String[] DefaultData(String UserID, String PsnID)
        {
            ...
            var page = HttpContext.Current.Handler as Page;
            EditData[1] = page.GetZoneTime().ToString("yyyy/MM/dd HH:mm:ss");
            #endregion
        ...
        public static object CheckOrgStruc(string sUserID, string sBirthday, string sOrgIDList, string sOrgNoList)
        {
            ...
            //若OrgIDList 有例外格式，則不予處理
            if (sOrgIDList.Contains("undefined") || sOrgIDList.Contains("null"))
            {
                NoArray[4] = "2";
            }
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
        ...
        public static object Insert(string sPsnNo, string sPsnName, string sPsnEName, string sPsnType, string sPsnIDNum, string sBirthday, string sOrgIDList, string sOrgNoList,
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sCardVer, string sCardRule, string sCardLock, string sTrainingEDate)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Pub.MessageObject sRet = new Pub.MessageObject();
            string sql = "";
            ...
            //將身份證字號轉卡號
            if (string.IsNullOrEmpty(sCardNo) && !string.IsNullOrEmpty(sPsnIDNum))
            {
                sCardNo = ConvertLettersToNumbers(sPsnIDNum.Substring(0, 1)) + sPsnIDNum.Substring(1, sPsnIDNum.Length - 1) + "0";
            }

            string[] NoArray = { sPsnNo, sPsnName, sCardNo, sPsnAccount, sPsnIDNum };
            String DefaultETime = "";
            String DefaultTrainingEDate = "";
            ...
            if (sRet.result && !arrOrgStrucID[0].Equals(""))
            {
                ...
                sql = @"INSERT INTO B01_Person (PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, OrgStrucID,
                    ...
                    Text4, Text5, CreateUserID, UpdateUserID, UpdateTime, TrainingEDate ";
                if (verifi_cnt > 0)
                {
                    sql += ",VerifiMode";
                }
                ...
                if (page != null)
                {
                    sPsnSTime = DateTime.Parse(sPsnSTime).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                    DefaultETime = DateTime.Parse(DefaultETime).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                }
                liSqlPara.Add("S:" + sPsnSTime);
                //DefaultETime Convert UTC
                ...
                if (sTrainingEDate != "")
                {
                    DefaultTrainingEDate = sTrainingEDate;
                }
                else
                {
                    DefaultTrainingEDate = oAcsDB.GetStrScalar("SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' AND ParaNo = 'DefaultEndDateTime' ");
                }
                if (page != null)
                {
                    DefaultTrainingEDate = DateTime.Parse(DefaultTrainingEDate).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                }
                liSqlPara.Add("S:" + DefaultTrainingEDate);
                ...
                if (istat > -1)
                {
                    if (oAcsDB.Commit())
                    {
                        ...
                        log.LogInfo = string.Format("Add Person {0} {1}", sPsnNo, sPsnName);
                        log.LogDesc = "Add Person";
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                //核三 卡片類型跟著人員類型走
                                string CardPwd = "0".PadLeft(CardPwLength, '0');
                                CardInsert_ForPerson(oReader.DataReader["PsnID"].ToString(), sCardNo.Trim(), sCardVer, CardPwd, "", "", sPsnType, sPsnAuthAllow,
                                    DateTime.Parse(oReader.DataReader["PsnSTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), DefaultETime, "", UserID, sCardRule, sCardLock);
                                liSqlPara.Clear();
                                #endregion
        ...
        public static bool CardInsert_ForPerson(string SelectValue, string sCardNo, string sCardVer, string sCardPW, string sCardSerialNo, string sCardNum, string sCardType, string sCardAuthAllow, string sCardSTime, string sCardETime, string sCardDesc, String UserID, string sCardRule, string sCardLock)
        {
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    sql = @" INSERT INTO B01_Card(CardNo, CardVer, CardPW, CardSerialNo, CardNum,
                        ...
                        UpdateUserID, UpdateTime, CardRule, CardLock) VALUES (?, ?, ?, ?,?,?
                        , ?, ?, ?, ?, ?, ?, ?, GETDATE(), ?, ?); ";
                    System.Web.HttpContext context = System.Web.HttpContext.Current;
                    ...
                    liSqlPara.Add("S:" + UserID);
                    liSqlPara.Add("S:" + UserID);
                    liSqlPara.Add("S:" + sCardRule);
                    liSqlPara.Add("S:" + sCardLock);
                    #endregion
                ...
                if (istat > -1)
                {
                    ...
                    var result = odo.GetQueryResult(@"SELECT B01_Card.CardID, B01_Card.CardNo, B00_ItemList.ItemName FROM B01_Card
                        ...
                        WHERE ItemClass = 'CardType' AND PsnID = @PsnID AND CardType NOT IN ('R','TEMP')", new { PsnID = SelectValue });
                    string sCardID = "";
        ...
        public static object Update(string SelectValue, string sPsnOldNo, string sPsnNo, string sPsnName, string sPsnEName, string sPsnType, string sPsnIDNum, string sBirthday,
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sCardRule, string sCardLock, string sTrainingEDate)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            OrmDataObject odo = new OrmDataObject("MsSql", Pub.GetDapperConnString());
            Pub.MessageObject sRet = new Pub.MessageObject();
            ...
            if (sRet.result && !arrOrgStrucID[0].Equals(""))
            {
                ...
                sql = @"UPDATE B01_Person SET PsnNo = @PsnNo, PsnName = @PsnName, PsnEName = @PsnEName, PsnType = @PsnType, IDNum = @IDNum,
                    ...
                    PsnETime = @PsnETime, PsnPicSource = @PsnPicSource, Remark = @Remark,UpdateUserID=@UpdateUserID,UpdateTime=GETDATE(), TrainingEDate=@TrainingEDate ";
                ...
                var DefaultEndTime = sPsnETime != "" ? sPsnETime : oAcsDB.GetStrScalar(@"SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' AND ParaNo = 'DefaultEndDateTime'");
                var DefaultTrainingEDate = sTrainingEDate != "" ? sTrainingEDate : oAcsDB.GetStrScalar(@"SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' AND ParaNo = 'DefaultEndDateTime'");
                #endregion
                var page = HttpContext.Current.Handler as Page;
                if (page != null)
                {
                    sPsnSTime = DateTime.Parse(sPsnSTime).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                    DefaultEndTime = DateTime.Parse(DefaultEndTime).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                }
                ...
                int istat = odo.Execute(sql, new
                {
                    ...
                    Remark = sRemark.Trim(),
                    UpdateUserID = UserID,
                    TrainingEDate = DefaultTrainingEDate,
                    Text1 = sText1.Trim(),
                    Text2 = sText2.Trim(),
                    Text3 = sText3.Trim(),
                    Text4 = sText4.Trim(),
                ...
                if (istat > -1)
                {
                    ...
                    oAcsDB.GetDataTable("CardID", "SELECT CardID, CardNo FROM B01_Card WHERE CardType NOT IN ('R','TEMP') AND PsnID = '" + SelectValue + "'", out dtCardID); ;
                    if (dtCardID.Rows.Count > 0)
                    {
                        ...
                        #region 修改卡片遞迴
                        sql = "UPDATE B01_Card SET CardRule = @CardRule, CardLock = @CardLock, UpdateUserID = @UpdateUserID, UpdateTime = GETDATE() WHERE CardID = @CardID ;";
                        odo.Execute(sql,
                            new { CardRule = sCardRule.Trim(), CardLock = sCardLock.Trim(), UpdateUserID = UserID, CardID = sCardID });
                        #endregion
        ...
        public static object Delete(string SelectValue, string sPsnNo, String UserID)
        {
            ...
            DBReader dr = null;
            List<string> liSqlPara = new List<string>();
            sql = @" SELECT CardID FROM B01_Card WHERE PsnID = '" + SelectValue.Trim() + "' AND CardType NOT IN ('R','TEMP'); ";
            oAcsDB.GetDataReader(sql, out dr);
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    while (CardIDList.Count > 0)
                    {
                        ...
                        sql += @" UPDATE B01_CardAuth SET OpMode = 'Del',OpStatus='',ErrCnt=0 WHERE CardNo = '" + sCardNo + "' ; ";
        ...
        public static object SingleCardData(string CardID)
        {
            ...
            if (CardID != "")
            {
                ...
                if (dr.HasRows)
                {
                    ...
                    var CardTypes = DongleVaries.GetCardType();
                    EditData[EditData.Length - 3] = "";
                    if (CardTypes.Count > 0)
                    {
                        ...
                        CardTypes.ForEach(cardtype =>
                        {
                            ...
                            EditData[EditData.Length - 3] += cardtype.ItemNo + "|" + cardtype.ItemName;
                        });
                    }
                    else
                    {
        ...
        public static object DefaultCardData()
        {
            ...
            #region 卡片類型
            String sql = @" SELECT ItemNo, ItemName FROM B00_ItemList
                WHERE ItemClass = 'CardType' AND ItemNo NOT IN ('R') AND ItemInfo1 = 'Default' ORDER BY ItemOrder ";
            var CardTypes = DongleVaries.GetCardType();
            if (CardTypes.Count > 0)
            {
                foreach (var cardtype in CardTypes)
                {
                    EditData[0] += cardtype.ItemNo + "|" + cardtype.ItemName + "|";
                }
                if (EditData[0] != "")
                {
                    EditData[0] = EditData[0].Substring(0, EditData[0].Length - 1);
            ...
            #region 啟用時間
            var page = HttpContext.Current.Handler as Page;
            EditData[2] = page.GetZoneTime().ToString("yyyy/MM/dd HH:mm:ss");
            #endregion
        ...
        public static object CardInsert(string SelectValue, string sCardNo, string sCardVer, string sCardPW, string sCardSerialNo, string sCardNum, string sCardType, string sCardAuthAllow, string sCardSTime, string sCardETime, string sCardDesc, String UserID)
        {
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    {
                        //liSqlPara.Add("S:" + sCardETime);
                        DefaultETime = sCardETime;
                    }
                    sCardSTime = DateTime.Parse(sCardSTime).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                    DefaultETime = DateTime.Parse(DefaultETime).GetUtcTime(page).ToString("yyyy/MM/dd HH:mm:ss");
                    liSqlPara.Add("S:" + sCardSTime);
                    liSqlPara.Add("S:" + DefaultETime);
                    liSqlPara.Add("S:" + sCardDesc);
                ...
                if (istat > -1)
                {
                    ...
                    if (sCardType == "ETAG")
                    {
                        odo.Execute("UPDATE B01_Person SET Text2=@CardNo WHERE PsnID=@PsnID ", new { CardNo = sCardNo, PsnID = SelectValue });
                    }
                    #endregion
            ...
            sRet.act = "Add";
            sRet.CurrentData = DongleVaries.GetCurrentCard();
            return sRet;
        }
        #endregion
        ...
        public static object SetCardVerifiMode(string SelectValue, string VerifiMode, string userid)
        {
            ...
            odo.BeginTransaction();
            var page = HttpContext.Current.Handler as Page;
            odo.Execute(sql_cmd, new { EquList = page.VerifiEquList(), PsnID = SelectValue, VerifiMode = VerifiMode, UserID = userid });
            if (istat > -1)
            {
                odo.Commit();
            ...
            , string sCardAuthAllow, string sCardSTime, string sCardETime, string sCardDesc, string adm100setting, String UserID)
        {
            ...
            if (sRet.result)
            {
                ...
                if (istat > -1)
                {
                    ...
                    if (sCardType == "ETAG")
                    {
                        odo.Execute("UPDATE B01_Person SET Text2=@CardNo WHERE PsnID=@PsnID ", new { CardNo = sCardNo.Trim(), PsnID = SelectValue });
                    }
                    #endregion
                }
                else
        ...
        public static object CardDelete(bool IsConfirm, string SelectValue, string SelectCardValue, String UserID)
        {
            ...
            if (IsConfirm)
            {
                if (sRet.result)
                {
                    ...
                    {
                        #region Process String
                        sql = @" UPDATE B01_Card SET PsnID=NULL WHERE CardID = ?  AND CardType IN ('R','TEMP') ; ";
                        //ETAG
                        sql += @"UPDATE B01_Person SET Text2='' WHERE PsnID IN (select PsnID from B01_Card where CardType='ETAG' AND CardID = ? );";

                        sql += @" DELETE FROM B01_Card WHERE CardID = ?  AND CardType NOT IN ('R','TEMP') ; ";
                    ...
                    if (istat > -1)
                    {
                        oAcsDB.Commit();
                        OrmDataObject odo = new OrmDataObject("MsSql", Pub.GetDapperConnString());
                        odo.SetPsnDelete(sCardNo);      //刪除B02_xxx 相關檔案
                        var log = SysLogService.GetSysLogEntityPackage(DB_Acs.Logtype.人員權限設定, UserID, UserName, "0103");
        ...
        # region CellMakeCard 製卡
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object CellMakeCard(string SelectValue)
        {
            Pub.MessageObject sRet = new Pub.MessageObject();
            ...
            sRet = Check_Input_DB(NoArray, "MakeCard");

            if (sRet.result)
            {
                DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
                sSql = @"SELECT CardID FROM B01_Card WHERE PsnID = ? ";
                liSqlPara.Clear();
                liSqlPara.Add("S:" + SelectValue.Trim());
                string sCardID = oAcsDB.GetStrScalar(sSql, liSqlPara);

                //string[] exePath = Pub.PCardPath.ToString().Trim().Split('\\');
                //string exeName = exePath[exePath.Length - 1].ToString().Replace(".exe", "");

                try
                {
                    System.Diagnostics.Process p = System.Diagnostics.Process.Start(Pub.PCardPath.ToString().Trim(), sCardID);
                }
                catch(Exception ex)
                {
                    sRet.result = false;
                    sRet.message += "製卡程式開啟失敗。";
                }
            }
