...
namespace SahoAcs.Web
{
    ...
    {        
        ...
        private void SetInitData()
        {
            ...
            //查詢人員文字一~五
            this.TextInfo = this.odo.GetQueryResult("SELECT ParaName,ParaNo,'' AS ParaValue FROM B00_SysParameter WHERE ParaClass='InputText' AND ParaValue='Y' ").ToList();
            //認證模式
            ...
            this.BirthDay.DateValue = DateTime.Now.AddYears(-18).ToString("yyyy/MM/dd");
        ...
        private void DoInsert()
        {
            this.odo.Execute(@"INSERT INTO B01_Person 
                    (PsnPicSource,PsnNo,PsnName,PsnEName,PsnType,IDNum,Birthday,OrgStrucID,PsnAccount,PsnPW,PsnAuthAllow,PsnSTime,PsnETime,Remark,VerifiMode,CreateTime,CreateUserID,UpdateTime,UpdateUserID,Text1,Text2,Text3,Text4,Text5) 
                    VALUES ('',@PsnNo,@PsnName,@PsnEName,@PsnType,@IDNum,@BirthDay,@OrgStrucID,@PsnAccount,@PsnPW
                    ,@PsnAuthAllow,@PsnSTime,@PsnETime,@Remark,@VerifiMode,GETDATE(),@CreateUserID,GETDATE(),@UpdateUserID,@Text1,@Text2,@Text3,@Text4,@Text5) ", this.PersonObj);
        ...
        {
            string UpdateCmd = @"UPDATE B01_Person SET 
        ...
        {
            //這裡要取得 MgaID，若MgaID 是第二區的
            var MgaDataList = this.odo.GetQueryResult("SELECT * FROM B00_SysMenu WHERE UserID=@UserID ",new {UserID=Sa.Web.Fun.GetSessionStr(this,"UserID") }).ToList();
        ...
        private bool GetDblCardNo()
        {
            ...
            if (this.odo.GetQueryResult("SELECT * FROM B01_Card WHERE CardNo=@CardNo AND PsnID<>@PsnID", this.PersonObj).Count() > 0)
            {
                return true;
            }
            return false;
