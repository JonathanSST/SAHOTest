...
namespace SahoAcs
{
    public partial class Person_K3 : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person_K3.js?jsdate="+DateTime.Now.ToString("yyyyMMddHHmmss")); //加入同一頁面所需的JavaScript檔案
        ...
        {            
            ...
            if (dr.HasRows)
            {
                EditData = new string[dr.DataReader.FieldCount + 4 + 2];//基本資訊(21) + 額外資訊(4) + 遞迴規則(2)
            ...
            if (EditData.Length > 2)
            {
                ...
                {
                    dr.Read();
                    EditData[EditData.Length - 6] = dr.DataReader["OrgIDList"].ToString();
                    EditData[EditData.Length - 5] = dr.DataReader["OrgNoList"].ToString();
                }
                else
                {
                    EditData[EditData.Length - 6] = "";
                    EditData[EditData.Length - 5] = "";
                }
                #endregion
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        EditData[EditData.Length - 4] += dr.DataReader["CardNo"].ToString() + "|" +
                            dr.DataReader["ItemName"].ToString() + "|" +
                            dr.DataReader["CardID"].ToString() + "|";

                        EditData[EditData.Length - 3] = dr.DataReader["CardRule"].ToString();
                        EditData[EditData.Length - 2] = dr.DataReader["CardLock"].ToString();
                    }

                    EditData[EditData.Length - 4] = EditData[EditData.Length - 4].Substring(0, EditData[EditData.Length - 4].Length - 1);
                }
                else
                {
                    EditData[EditData.Length - 4] = "";
                    EditData[EditData.Length - 3] = "0";
                    EditData[EditData.Length - 2] = "0";
                }
                #endregion
        ...
        public static object Insert(string sPsnNo, string sPsnName, string sPsnEName, string sPsnType, string sPsnIDNum, string sBirthday, string sOrgIDList, string sOrgNoList, 
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sCardVer, string sPsnRule, string sPsnLock)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Pub.MessageObject sRet = new Pub.MessageObject();
            string sql = "";
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                CardInsert_ForPerson(oReader.DataReader["PsnID"].ToString(), sCardNo.Trim(), sCardVer, CardPwd, "", "", sPsnType, sPsnAuthAllow, 
                                    DateTime.Parse(oReader.DataReader["PsnSTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), DefaultETime, "", UserID, sPsnRule, sPsnLock);
                                liSqlPara.Clear();
                                #endregion
        ...
        public static bool CardInsert_ForPerson(string SelectValue, string sCardNo, string sCardVer, string sCardPW, string sCardSerialNo, string sCardNum, string sCardType, string sCardAuthAllow, string sCardSTime, string sCardETime, string sCardDesc, String UserID, String sCardRule, String sCardLock)
        {
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    liSqlPara.Add("I:" + sCardRule);
                    liSqlPara.Add("I:" + sCardLock);
        ...
        public static object Update(string SelectValue, string sPsnOldNo, string sPsnNo, string sPsnName, string sPsnEName, string sPsnType, string sPsnIDNum, string sBirthday,
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sPsnRule, string sPsnLock)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            OrmDataObject odo = new OrmDataObject("MsSql",Pub.GetDapperConnString());
            Pub.MessageObject sRet = new Pub.MessageObject();
            ...
            if (sRet.result && !arrOrgStrucID[0].Equals(""))
            {
                ...
                if (istat > -1)
                {
                    ...
                    if (dtCardID.Rows.Count > 0)
                    {
                        ...
                        odo.Execute("UPDATE B01_Card SET CardRule=@CardRule, CardLock=@CardLock WHERE PsnID=@PsnID ",
                           new { CardRule = sPsnRule, CardLock = sPsnLock, PsnID = SelectValue });
        ...
        public static object SingleCardData(string CardID)
        {
            ...
            if (CardID != "")
            {
                ...
                String sql = @" SELECT CardID, CardNo, CardVer, CardPW, CardSerialNo, CardNum,
                    CardType, PsnID, CardAuthAllow, CardSTime, CardETime, CardDesc, CardRule, CardLock FROM B01_Card
                    WHERE CardID = ? ; ";
