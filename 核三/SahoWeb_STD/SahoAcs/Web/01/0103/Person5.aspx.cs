...
namespace SahoAcs
{
    public partial class Person5 : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person5.js?"+DateTime.Now.ToString("yyyyMMddHmmss")); //加入同一頁面所需的JavaScript檔案
        ...
        private void GetCardItemInfo()
        {
            ...
            Session["CardVer"] = oAcsDB.GetStrScalar(" SELECT ParaValue FROM B00_SysParameter WHERE ParaNo = 'CardVer' ");
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string mode)
        {
            ...
            if (mode == "Insert")
            {
                ...
                if (!Sa.Check.CheckIDNumber(NoArray[0]))
                {
                    sRet.result = false;
                    sRet.message += "人員編號格式錯誤!!";
                }
                    ...
                    if (oAcsDB.Commit())
                    {
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                string CardNo = Pub.GetIDToCard(sPsnNo);
