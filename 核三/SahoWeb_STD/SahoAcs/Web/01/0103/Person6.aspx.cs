...
namespace SahoAcs
{
    public partial class Person6 : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person6.js?"+DateTime.Now.ToString("yyyyMMddHmmss")); //加入同一頁面所需的JavaScript檔案
        ...
        private void Query(bool select_state, string SortExpression, string SortDire)
        {
            ...
            if (!string.IsNullOrEmpty(NowData))
            {
                ...
                wheresql += " (IDNum=? OR PsnNo LIKE ? OR PsnName LIKE ? OR B01_Person.PsnID IN (SELECT PsnID FROM B01_Card WHERE CardNo=? AND PsnID IS NOT NULL ) ) ";
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string mode)
        {
            ...
            if (mode == "Insert")
            {
                ...
                if (!Sa.Check.CheckIDNumber(NoArray[4]))
                {
                    sRet.result = false;
                    sRet.message += "身份證格式錯誤!!";
                }
        ...
        protected void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.DataRow:
                    ...
                    #region 設定欄位寛度
                    e.Row.Cells[1].Width = 114;
                    e.Row.Cells[2].Width = 104;
                    #endregion
        ...
        #region Person PwdChange 重設密碼
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object PwdChangeProc(string sPsnNo, string sPsnID)
        {
            Pub.MessageObject sRet = new Pub.MessageObject();
            OrmDataObject odo = new OrmDataObject("MsSql", Pub.GetDapperConnString());
            odo.Execute("UPDATE B01_Person SET PsnPW='AbcdEfgh1234', ErrCnt=0 WHERE PsnID=@PsnID AND ErrCnt>=3",new {PsnID=sPsnID});
            sRet.act = "Edit";
            sRet.CurrentData = 1;
            sRet.message = "AbcdEfgh1234";
            sRet.result = true;
            return sRet;
        }
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sCardVer)
        {
            ...
            if (sRet.result && !arrOrgStrucID[0].Equals(""))
            {
                ...
                liSqlPara.Add("S:" + DateTime.Now.ToString("yyyy/MM/dd"));
                    ...
                    if (oAcsDB.Commit())
                    {
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                string CardNo = Pub.GetIDToAscCard(sPsnIDNum);      //取得卡號
                                CardInsert_ForPerson(oReader.DataReader["PsnID"].ToString(), CardNo, sCardVer, "0000", "", "", sPsnType, sPsnAuthAllow, DateTime.Parse(oReader.DataReader["PsnSTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), DefaultETime, "", UserID);
                                liSqlPara.Clear();
                                #endregion
        ...
        {            
            ...
            List<string> liSqlPara = new List<string>();
            OrmDataObject odo = new OrmDataObject("MsSql",Pub.GetDapperConnString());
            var NewCardNo = sCardNo;
            var NewVerNo = sCardVer;
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    if (odo.GetQueryResult("SELECT * FROM B01_Card WHERE CardNo=@CardNo AND CardVer=@CardVer", new { CardNo = sCardNo, CardVer = sCardVer }).Count() == 0)
                    {
                        odo.Execute("UPDATE B01_Card SET CardAuthAllow=0 WHERE CardID=@CardID", new { CardID = SelectCardValue });
                        odo.Execute("EXEC CardAuth_Update @sCardNo = @CardNo, @sUserID = @UserID, @sFromProc = 'Person', @sFromIP = '', @sOpDesc = '卡片資料內容更新' ; "
                        , new { CardNo = sCardNo, UserID = UserID });
                        //記錄換卡版次
                        odo.Execute("UPDATE B01_Person SET Text1=CONVERT(VARCHAR(30),GETDATE(),120)  WHERE PsnID=@PsnID", new { PsnID = SelectValue });
                    }
                    sql = @" UPDATE B01_Card SET CardVer = ?, CardPW = ?, CardSerialNo = ?,
                        CardNum = ?, CardType = ?, CardAuthAllow = ?, CardSTime = ?, CardETime = ?,
                        CardDesc = ?, UpdateUserID = ?, UpdateTime = GETDATE() WHERE CardID = ?; ";                    
                    //liSqlPara.Add("S:" + NewCardNo.Trim());
                    liSqlPara.Add("S:" + sCardVer.Trim());
                    liSqlPara.Add("S:" + sCardPW.Trim());
                    liSqlPara.Add("S:" + sCardSerialNo);
                    liSqlPara.Add("S:" + sCardNum.Trim());
                ...
                if (istat > -1)
                {
                    ...
                    log.LogInfo = string.Format("Update Card CardNo='{0}',CardSTime='{1}',CardETime='{2}'", NewCardNo, sCardSTime, sCardETime);
                    log.LogDesc = "修改卡片權限";
                    odo.SetSysLogCreate(log);
                    //新的卡片預設權限設定    20170524 update by Sam
                    sRet.message += odo.SetOrgEquData(NewCardNo, int.Parse(SelectValue), UserID, gms);
                }
                else
                {
                    oAcsDB.Rollback();
