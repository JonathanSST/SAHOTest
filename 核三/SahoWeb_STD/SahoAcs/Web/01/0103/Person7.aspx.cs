...
namespace SahoAcs
{
    public partial class Person7 : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person7.js"); //加入同一頁面所需的JavaScript檔案
                    ...
                    if (oAcsDB.Commit())
                    {
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                string CardNo = sPsnNo.PadLeft(8, '0');
            ...
            if (IsConfirm)
            {
                if (sRet.result)
                {
                    ...
                    if (istat > -1)
                    {
                        oAcsDB.Commit();
                        OrmDataObject odo = new OrmDataObject("MsSql", Pub.GetDapperConnString());
                        odo.SetPsnDelete(sCardNo);      //刪除特徵檔案
                        var log = SysLogService.GetSysLogEntityPackage(DB_Acs.Logtype.人員權限設定, UserID, UserName, "0103");
