...
namespace SahoAcs
{
    public partial class PersonTc : System.Web.UI.Page
    {
        ...
        private void SetCardItemInfo()
        {
            ...
            Session["CardLen"] = oAcsDB.GetIntScalar(" SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' AND ParaNo = 'CardDefaultLength' ");
                    ...
                    if (oAcsDB.Commit())
                    {
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                CardInsert_ForPerson(oReader.DataReader["PsnID"].ToString(), sCardNo.Trim(), sCardVer, CardPwd, "", "", sPsnType, sPsnAuthAllow, DateTime.Parse(oReader.DataReader["PsnSTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), DefaultETime, "", UserID);
                                liSqlPara.Clear();
                                #endregion
        ...
        public static object Delete(string SelectValue, string sPsnNo, String UserID)
        {
            ...
            if (sRet.result)
            {
                ...
                if (istat > -1)
                {
                    ...
                    try
                    {
                        oAcsDB.SqlCommandExecute(organism_sql, liSqlPara);
                    }
                    catch (Exception ex)
                    {
