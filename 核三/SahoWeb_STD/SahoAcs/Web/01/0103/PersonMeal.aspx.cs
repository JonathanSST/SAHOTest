...
namespace SahoAcs
{
    public partial class PersonMeal : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "PersonMeal.js?jsdate="+DateTime.Now.ToString("yyyyMMddHHmmss")); //加入同一頁面所需的JavaScript檔案
        ...
        protected void GridView1_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.DataRow:
                    ...
                    if (oRow["EquModel"].ToString() == "ADM100FP" && this.dtAuthMode.Rows.Count>0)
                    {
        ...
        {            
            ...
            String sql = @"SELECT PsnID, PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, OrgStrucID,
                ...
                , MealFood 
                FROM B01_Person WHERE PsnNo = '" + sPsnID + "'";
            oAcsDB.GetDataReader(sql, out dr);
            if (dr.HasRows)
            {
        ...
        public static object Insert(string sPsnNo, string sPsnName, string sPsnEName, string sPsnType, string sPsnIDNum, string sBirthday, string sOrgIDList, string sOrgNoList, 
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sCardVer, string sMealFood)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Pub.MessageObject sRet = new Pub.MessageObject();
            string sql = "";
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                CardInsert_ForPerson(oReader.DataReader["PsnID"].ToString(), sCardNo.Trim(), sCardVer, CardPwd, "", "", sPsnType, sPsnAuthAllow, 
                                    DateTime.Parse(oReader.DataReader["PsnSTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), DefaultETime, "", UserID);
                                liSqlPara.Clear();
                                #endregion
        ...
        public static object Update(string SelectValue, string sPsnOldNo, string sPsnNo, string sPsnName, string sPsnEName, string sPsnType, string sPsnIDNum, string sBirthday,
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sMealFood)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            OrmDataObject odo = new OrmDataObject("MsSql",Pub.GetDapperConnString());
            Pub.MessageObject sRet = new Pub.MessageObject();
