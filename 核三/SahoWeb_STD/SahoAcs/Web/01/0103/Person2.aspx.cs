...
namespace SahoAcs
{
    public partial class Person2 : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person2.js"); //加入同一頁面所需的JavaScript檔案
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (ConfigurationManager.AppSettings["ShowEquList"].ToString().Equals("2"))
            {
                //this.GridView1.Visible = false;
                //this.td_showGridView1.Visible = false;
        ...
        private void Query(bool select_state, string SortExpression, string SortDire)
        {
            ...
            if (!string.IsNullOrEmpty(NowData))
            {
                ...
                wheresql += " ( PsnNo LIKE ? OR PsnName LIKE ? OR B01_Person.PsnID IN (SELECT PsnID FROM B01_Card WHERE CardNo=? AND PsnID IS NOT NULL ) ) ";
        ...
        public static ArrayList SingleData(String sPsnID, String UserID)
        {
            ...
            if (EditData.Length > 2)
            {
                ...
                sql = @"SELECT B01_Card.CardID, B01_Card.CardNo, B00_ItemList.ItemName
                    ...
                    WHERE ItemClass = 'CardType' AND PsnID = '" + EditData[0] + "' AND CardType<>'R' ";
                oAcsDB.GetDataReader(sql, out dr);
        ...
        public static String[] DefaultData(String UserID, String PsnID)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            DBReader dr = null;
            string[] EditData = new string[4];//0：人員類型、1：起始日
            int cardlen = oAcsDB.GetIntScalar("SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' AND ParaNo = 'CardDefaultLength'");
            string PsnNo = Pub.GetPsnNo();
            string CardNo = Pub.AddZero(PsnNo, cardlen);
            #region 人員類型
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    sql = @" INSERT INTO B01_Card(CardNo, CardVer, CardPW, CardSerialNo, CardNum,
                       ...
                       (SELECT TOP 1 ItemNo FROM B00_ItemList WHERE ItemClass='CardType' ORDER BY ItemOrder )
                        , ?, ?, ?, ?, ?, ?, ?, GETDATE()); ";
                    liSqlPara.Add("S:" + Pub.SetToNarrow(sCardNo.Trim()));
                ...
                if (istat > -1)
                {
                    ...
                    if (result.Count() > 0)
                    {
                        ...
                        #region 預設群組權限
                        h_EquGrIDList = ResetDefaultAuth(SelectValue);

                        if (h_EquGrIDList.Count > 0 && sCardID.Trim() != "")
                        {
                            var para = oAcsDB.GetParaPackage(DB_Acs.Logtype.預設卡片權限, UserID, UserName, "0103", "", "", "", "");

                            sql = @" DELETE FROM B01_CardEquGroup WHERE CardID = ?; ";
                            liSqlPara.Add("I:" + sCardID);
                            foreach (var obj in h_EquGrIDList)
                            {
                                sql += @" INSERT INTO B01_CardEquGroup(CardID, EquGrpID, CreateUserID)
                                    VALUES (?, ?, ?); ";
                                liSqlPara.Add("I:" + sCardID);
                                liSqlPara.Add("I:" + obj.Value.ToString());
                                liSqlPara.Add("S:" + UserID);
                            }
                            oAcsDB.BeginTransaction();
                            istat = oAcsDB.SqlCommandExecute(sql, liSqlPara);
                            if (istat > 0)
                            {
                                istat = oAcsDB.SqlCommandExecute(" EXEC CardAuth_Update @sCardNo = '" + sCardNo + "',@sUserID = '" + UserID + "',@sFromProc = 'Person',@sFromIP = '" + sIPAddress + "',@sOpDesc = '新卡寫入預設權限' ; ");
                                oAcsDB.Commit();
                                odo.Execute(Pub.StrInsertSysLog,
                                    from i in h_EquGrIDList
                                    select new
                                    {
                                        p1 = para[0],
                                        p2 = para[1],
                                        ...
                                        p6 = "",
                                        p7 = "",
                                        p8 = string.Format("{0} Add CardEquGroup {1}", sCardNo, i.Value),
                                        p9 = "建立預設權限"
                                    });
                            }
                            else
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID)
        {
            ...
            if (sRet.result && !arrOrgStrucID[0].Equals(""))
            {
                int verifi_cnt = AuthModelCount();
                #region Process String
                sql = @"UPDATE B01_Person SET PsnNo = ?, PsnName = ?, PsnEName = ?, PsnType = ?, IDNum = ?,
                    Birthday = ?, OrgStrucID = ?, PsnAccount = ?, PsnPW = ?, PsnAuthAllow = ?, PsnSTime = ?,
                    PsnETime = ?, PsnPicSource = ?, Remark = ?, Text1 = ?, Text2 = ?, Text3 = ?, Text4 = ?,
                    Text5 = ? ";
                if (verifi_cnt > 0)
                {
                    sql += " ,VerifiMode=? ";
                }
                sql += " ,UpdateUserID = ?, UpdateTime = GETDATE() WHERE PsnID = ?;";
                liSqlPara.Add("S:" + sPsnNo.Trim());
                liSqlPara.Add("S:" + sPsnName.Trim());
                liSqlPara.Add("S:" + sPsnEName.Trim());
                liSqlPara.Add("S:" + sPsnType);
                ...
                {
                    String DefaultETime = oAcsDB.GetStrScalar(@"SELECT ParaValue FROM B00_SysParameter WHERE ParaClass = 'System' 
                    AND ParaNo = 'DefaultEndDateTime'");
                    liSqlPara.Add("S:" + DefaultETime);
                }
                liSqlPara.Add("S:" + sPsnPicSource.Trim());
                liSqlPara.Add("S:" + sRemark);
                liSqlPara.Add("S:" + sText1.Trim());
                liSqlPara.Add("S:" + sText2.Trim());
                liSqlPara.Add("S:" + sText3.Trim());
                liSqlPara.Add("S:" + sText4.Trim());
                ...
                if (verifi_cnt > 0)
                {
                    liSqlPara.Add("S:" + verifiMode.Trim());
                }
                liSqlPara.Add("S:" + UserID);
                liSqlPara.Add("I:" + SelectValue);
                ...
                if (istat > -1)
                {
                    ...
                    oAcsDB.GetDataTable("CardID", "SELECT CardID, CardNo FROM B01_Card WHERE CardType<>'TEMP' AND PsnID = '" + SelectValue + "'", out dtCardID); ;
                    if (dtCardID.Rows.Count > 0)
                    {
                        ...
                        h_EquGrIDList = ResetDefaultAuth(SelectValue);//取得組織預設權限                                                
                        if (h_EquGrIDList.Count > 0)
                        {
                            sql = "";
                            foreach (var obj in h_EquGrIDList)
                            {
                        ...
                        sql = @" EXEC CardAuth_Update @sCardNo = '" + sCardNo + "',@sUserID = '" + UserID + "',@sFromProc = 'Person',@sFromIP = '" + IPAddress() + "',@sOpDesc = '更新重整預設權限' ; ";
                        oAcsDB.SqlCommandExecute(sql);
                        if (arrOrgStrucID[1].Equals("true"))
                        {
                            foreach (var o in h_EquGrIDList)
                            {
                                log.LogType = DB_Acs.Logtype.卡片權限調整.ToString();
                                log.LogInfo = string.Format("Action:'Add',EquGrpNo:'',EquGrpID:'{0}',CardNo:'{1}'", o.Value, sCardNo);
                                odo.SetSysLogCreate(log);
                            }
                        }
                    }
        ...
        public static object Delete(string SelectValue, string sPsnNo, String UserID)
        {
            ...
            DBReader dr = null;
            List<string> liSqlPara = new List<string>();
            sql = @" SELECT CardID FROM B01_Card WHERE PsnID = '" + SelectValue.Trim() + "' AND CardType<>'TEMP'; ";
            oAcsDB.GetDataReader(sql, out dr);
            ...
            if (sRet.result)
            {
                ...
                if (istat > -1)
                {
                    ...
                    while (CardIDList.Count > 0)
                    {
                        ...
                        sql += @" UPDATE B01_CardAuth SET OpMode = 'Del' WHERE CardNo = '" + sCardNo + "' ; ";
            ...
            , string sCardAuthAllow, string sCardSTime, string sCardETime, string sCardDesc, string adm100setting, String UserID)
        {
            ...
            OrmDataObject odo = new OrmDataObject("MsSql"
            //, string.Format(Pub.db_connection_template, Pub.db_source_ip, Pub.db_data_name, Pub.db_user_id, Pub.db_pwd)
            , Pub.GetDapperConnString());

            #region 設定ADM100FP 認證模式修改

            if (AuthModelCount() > 0 && adm100setting.Length > 0)
            {
                sql = @" SELECT A.EquID,A.VerifiMode FROM B01_CardAuth A INNER JOIN B01_EquData B ON A.EquID=B.EquID WHERE CardNo = ? ; ";
                liSqlPara.Add("S:" + sCardNo);
                oAcsDB.GetDataTable("Table1", sql, liSqlPara, out dt);
                string[] adm100FP_arr = adm100setting.Substring(1, adm100setting.Length - 1).Split('|');
                List<SahoAcs.DBModel.EquData> list_equ = new List<DBModel.EquData>();
                foreach (string s in adm100setting.Substring(1, adm100setting.Length - 1).Split('|'))
                {
                    list_equ.Add(new DBModel.EquData
                    {
                        EquID = int.Parse(s.Split('%').FirstOrDefault()),
                        VerifiMode = s.Split('%').LastOrDefault()
                    });
                }
                liSqlPara.Clear();
                sql = "";
                ...
                {
                    if (list_equ.Where(i => i.EquID == Convert.ToInt32(r["EquID"]) && !i.VerifiMode.Equals(Convert.ToString(r["VerifiMode"]))).Count() > 0)
                    {
                        sql += @"UPDATE B01_CardAuth SET OpMode='Reset',OpStatus='',ErrCnt=0,VerifiMode=?,UpdateTime=GETDATE(),UpdateUserID=?
                                       WHERE CardNo=? AND EquID=? AND OpMode<>'Del' ; ";
                        liSqlPara.Add("S:" + list_equ.Where(i => i.EquID == Convert.ToInt32(r["EquID"])).FirstOrDefault().VerifiMode);
                        liSqlPara.Add("S:" + UserID);
                        liSqlPara.Add("S:" + sCardNo);
                        liSqlPara.Add("S:" + Convert.ToString(r["EquID"]));
                    }
                }
                if (sql.Length > 0)
                {
            ...
            if (sRet.result)
            {
                ...
                if (istat > -1)
                {
                    ...
                    if (h_EquGrIDList.Count > 0)
                    {
                        sRet.message = gms.MsgUpdateSuccess + "|";
                    }
                    else
                    {
                        sRet.message = gms.MsgUpdateSuccess1 + "|";
                    }
                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            sRet.message += dr.DataReader["CardNo"].ToString() + "|" +
                                dr.DataReader["ItemName"].ToString() + "|" +
                                dr.DataReader["CardID"].ToString() + "|";
                        }
                        ...
                        //if (h_EquGrIDList.Count > 0)
                        //{
                        string sCardID = oAcsDB.GetStrScalar("SELECT CardID FROM B01_Card WHERE CardNo = '" + sCardNo + "'");
                        //sql = @" DELETE FROM B01_CardEquGroup WHERE CardID = ?; ";
                        //liSqlPara.Add("I:" + sCardID);                            
                        foreach (var obj in h_EquGrIDList)
                        {
                            sql += @" INSERT INTO B01_CardEquGroup(CardID, EquGrpID, CreateUserID) VALUES (?, ?, ?); ";
                            liSqlPara.Add("I:" + sCardID);
                            liSqlPara.Add("I:" + obj.Value.ToString());
                            liSqlPara.Add("S:" + UserID);
                        }

                        sql += @" UPDATE B01_CardExt SET CardBorrow = '0' WHERE CardID = ?; ";
                        liSqlPara.Add("I:" + sCardID);

                        //修改卡片起迄時間及讀卡規則
                        //sql += @" UPDATE dbo.B01_CardEquAdj SET BeginTime = ?,EndTime = ? WHERE CardID = ? AND OpMode<>'*' ";
                        //liSqlPara.Add("S:" + sCardSTime);
                        //liSqlPara.Add("S:" + sCardETime);
                        //liSqlPara.Add("I:" + sCardID);
                        #region 設定讀卡規則定義
                        List<CardEquRule> rules = new List<DBModel.CardEquRule>();

                        odo.Execute(@"EXEC CardEquAdj_Update 
                                                    @CardID=@CardID,@EquID=@EquID,@OpMode='+',@CardExtData='',
                                                    @CardRule=@RuleID,@BeginTime=@BeginTime,@EndTime=@EndTime,
                                                    @CreateUserID=@UserID,@CreateTime='" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "'", rules.Where(i => i.RuleID > 0));
                        #endregion


                        //20151202 增加卡片修改時，變更卡片權限起迄時間
                        sql += @" UPDATE dbo.B01_CardAuth SET OpMode='Reset',BeginTime = ?,EndTime = ? WHERE CardNo = ? AND OpMode<>'Del'; ";
                        liSqlPara.Add("S:" + sCardSTime);
                        liSqlPara.Add("S:" + sCardETime);
                        liSqlPara.Add("S:" + sCardNo);
                        ...
                        if (istat > 0)
                        {
                            istat = oAcsDB.SqlCommandExecute(" EXEC CardAuth_Update @sCardNo = '" + sCardNo + "',@sUserID = '" + UserID + "',@sFromProc = 'Person',@sFromIP = '" + sIPAddress + "',@sOpDesc = '卡片資料內容更新' ; ");
                            oAcsDB.Commit();
        ...
        public static object CardDelete(bool IsConfirm, string SelectValue, string SelectCardValue, String UserID)
        {
            ...
            if (IsConfirm)
            {
                if (sRet.result)
                {
                    ...
                    {
                        #region Process String
                        sql = @" UPDATE B01_Card SET PsnID=NULL WHERE CardID = ?  AND CardType='R' ; ";
                        sql += @" DELETE FROM B01_Card WHERE CardID = ?  AND CardType<>'R' ; ";
                        sql += @" DELETE FROM B01_CardEquGroup WHERE CardID = ? ; ";
