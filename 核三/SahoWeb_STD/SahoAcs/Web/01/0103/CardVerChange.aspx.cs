...
{
    public partial class CardVerChange : Sa.BasePage
    {
        ...
        {
            var result=this.odo.GetQueryResult("SELECT * FROM B01_Card WHERE CardNo=@CardNo AND CardVer=@NewCardVer", new { CardNo = Request["CardNo"], NewCardVer = Request["NewCardVer"] });
            ...
            if (result.Count() == 0)
            {
                string sNewCardNo = Request["CardNo"].Substring(0, Request["CardNo"].Length - 1) + Request["NewCardVer"];
                ...
                this.odo.Execute(@"UPDATE B01_Card SET 
                                    UpdateTime=GETDATE(),UpdateUserID=@UserID,CardAuthAllow=1,CardNo=@CardNo, CardVer=@CardVer WHERE CardID=@CardID", 
                                    new {CardNo= sNewCardNo, CardID = Request["CardID"],UserID=Session["UserID"], CardVer= Request["NewCardVer"] });
                this.odo.Execute(@"UPDATE B02_FPData SET 
                                    CardNo=@NewCardNo WHERE CardNo=@CardNo",
                                    new { NewCardNo = sNewCardNo, CardNo=Request["CardNo"] });
                ...
                if (CardType == "ETAG")
                {
                  ...
                  new { NewCardNo = sNewCardNo, PsnNo = PsnNo });
                }


                this.odo.Execute(@"EXEC CardAuth_Update @sCardNo = @CardNo,@sUserID = @UserID,
                                                        @sFromProc = @UserID,@sFromIP = '127.0.0.1',@sOpDesc = '一般權限重整'", new { CardNo = sNewCardNo, UserID = Session["UserId"].ToString() });
                syslog_dto.LogInfo = string.Format("人員卡片變更(人員:{0}_{1}_{2}_啟用門禁權限", PsnNo, PsnName, sNewCardNo);
                this.odo.Execute(this.SysLogCmd, syslog_dto);
                syslog_dto.LogType = DB_Acs.Logtype.卡號變更.ToString();
                syslog_dto.LogInfo = string.Format("PsnNo={0},PsnName={1},CardNo={2},NewCardNo={3}", PsnNo, PsnName, Request["CardNo"], sNewCardNo);
                this.odo.Execute(this.SysLogCmd, syslog_dto);
            }
            else
            {
                this.ErrorMessage = "版次重複!!";
            }
        }
