...
namespace SahoAcs
{
    public partial class Person3 : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            ClientScript.RegisterClientScriptInclude("Person", "Person3.js"); //加入同一頁面所需的JavaScript檔案
        ...
        private int Query(string mode, string SortExpression, string SortDire)
        {
            ...
            {
                if (wheresql != "") wheresql += " AND ";
                wheresql += " ( PsnNo LIKE ? OR PsnName LIKE ? OR Text1 LIKE ? OR Text2 LIKE ? OR CardNo LIKE ? ) ";
                liSqlPara.Add("S:" + '%' + NowData + '%');
                liSqlPara.Add("S:" + '%' + NowData + '%');
                liSqlPara.Add("S:" + '%' + NowData + '%');
                liSqlPara.Add("S:" + '%' + NowData + '%');
            ...
            string NowCompanyNo = this.dropCompany.SelectedValue;
            string NowDepartmentNo = this.dropDepartment.SelectedValue;
            string NowPsnTypeNo = this.dropPsnType.SelectedValue;
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string mode)
        {
            ...
            if (mode == "CardInsert")
            {
                ...
                //    sRet.message += string.Format(MsgCardSize, cardlen);
                //}
                NoArray[0] = Pub.AddZero(NoArray[0], cardlen);
            ...
            string sText1, string sText2, string sText3, string sText4, string sText5, string verifiMode, string UserID, string sCardVer)
        {
            ...
            if (sCardNo.Trim() != "")
            {
                sCardNo = Pub.AddZero(sCardNo, 8);
                sPsnNo = Pub.AddZero(sCardNo, 8);
            }
            ...
            if (IsConfirm)
            {
                if (sRet.result)
                {
                    ...
                    if (istat > -1)
                    {
                        ...
                        try
                        {
                            oAcsDB.SqlCommandExecute(organism_sql);
                        }
                        catch (Exception e)
                        {
