...
namespace SahoAcs
{
    public partial class OrgData : System.Web.UI.Page
    {
        ...
        private void Query(bool select_state, string SortExpression, string SortDire)
        {
            ...
            String NowCondition = "", NowClass = "", wheresql = "";
            ...
            sql = " SELECT * FROM B01_OrgData WHERE OrgNo > '' ";
            ...
            if (!string.IsNullOrEmpty(NowCondition))
            {
                ...
                for (int i = 0; i <= strArray.Length - 1; i++)
                {
                    if (strArray[i].Trim() != "")
                    {
                        wheresql += " AND (OrgNo + '__' + ISNULL(OrgName,'')+ '__' + ISNULL(OrgEName,'')) LIKE ? ";
            ...
            dt.Columns.Add(new DataColumn("OrgNameEName"));
            foreach (DataRow r in dt.Rows)
            {
                r["OrgNameEName"] = r["OrgName"].ToString().Trim();
                if (!string.IsNullOrEmpty(r["OrgEName"].ToString().Trim()))
                {
                    r["OrgNameEName"] = r["OrgName"].ToString().Trim() + "(" + r["OrgEName"].ToString().Trim() + ")";
                }
            }
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string mode)
        {
            ...
            {
                case "Insert":
                    sql = @" SELECT * FROM B01_OrgData WHERE (OrgNo = ? OR OrgName=?) AND OrgClass = ? ";
                    liSqlPara.Add("S:" + NoArray[0].Trim());
                    liSqlPara.Add("S:" + NoArray[1].Trim());
                    liSqlPara.Add("S:" + NoArray[2].Trim());
                    break;
                case "Update":
                    sql = @" SELECT * FROM B01_OrgData WHERE (OrgNo = ? OR OrgName=?) AND OrgNo <> ? AND OrgClass = ? ";
                    liSqlPara.Add("S:" + NoArray[0].Trim() + NoArray[1].Trim());
                    liSqlPara.Add("S:" + NoArray[4].Trim());
                    liSqlPara.Add("S:" + NoArray[2].Trim());
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object Insert(string Org_No, string Org_Name, string Org_Class, String UserID, string Org_EName)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Pub.MessageObject sRet = new Pub.MessageObject();
            string sql = "";
            ...
            {
                #region Process String
                sql = @" INSERT INTO B01_OrgData(OrgClass ,OrgNo ,OrgName ,CreateUserID ,UpdateUserID ,UpdateTime, OrgEName) 
                         VALUES (?,?,?,?,?,GETDATE(),?) ";
                ...
                liSqlPara.Add("S:" + UserID);
                liSqlPara.Add("S:" + Org_EName.Trim());
                #endregion
        ...
        [System.Web.Services.WebMethod()]
        [System.Web.Script.Services.ScriptMethod()]
        public static object Update(String Org_ID, String Old_Org_No, string New_Org_Title, string New_Org_No, string Org_Name, string Org_Class, String UserID, string Org_EName)
        {
            DB_Acs oAcsDB = new DB_Acs(Pub.GetConnectionString(Pub.sConnName));
            Pub.MessageObject sRet = new Pub.MessageObject();
            string sql = "";
            ...
            if (sRet.result)
            {
                ...
                {
                    #region Process String
                    sql = @" UPDATE B01_OrgData SET OrgClass = ? ,OrgNo = ? ,OrgName = ? ,UpdateUserID = ?, OrgEName = ? ,UpdateTime = GETDATE() WHERE OrgID = ? ; ";
