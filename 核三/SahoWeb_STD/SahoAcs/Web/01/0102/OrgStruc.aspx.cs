...
namespace SahoAcs
{
    public partial class OrgStruc : System.Web.UI.Page
    {
        ...
        private void LoadProcess()
        {
            ...
            js += "<script src=\"OrgGroupEdit.js\" Type=\"text/javascript\"></script>\n";
        ...
        private void Query(bool select_state, string SortExpression, string SortDire)
        {
            ...
            sql = " SELECT * FROM OrgStrucAllData('') WHERE OrgStrucNo > '' ";
            ...
            if (!string.IsNullOrEmpty(NowCondition))
            {
                ...
                for (int i = 0; i <= strArray.Length - 1; i++)
                {
                    if (strArray[i].Trim() != "")
                    {
                        wheresql += "AND (ISNULL(OrgStrucNo,'') + '__' + ISNULL(OrgNoList,'') + '__' + ISNULL(OrgNameList,'')) LIKE ? ";
            ...
            /* 
                ...
                以免資料量大時反覆讀取速度過慢
                這個網頁採用 B01_OrgData、B01_OrgStruc 的'總數字串'相加作為標準
                當 B01_OrgData、B01_OrgStruc 的總數發生異動時，才重新讀取資料庫
            */

            if (Session["tmpList"] == null) Session["tmpList"] = new List<string>();
            if (Session["tmpDatatable"] == null) Session["tmpDatatable"] = new DataTable();

            string strC1 = oAcsDB.GetStrScalar("SELECT COUNT(*) FROM B01_OrgData");
            string strC2 = oAcsDB.GetStrScalar("SELECT COUNT(*) FROM B01_OrgStruc");
            string strCount = "";

            if (strC1 != null && strC2 != null) strCount = strC1 + strC2;

            if (liSqlPara.SequenceEqual(Session["tmpList"] as List<string>)
                && hFlagRowCount.Value == strCount)
            {
        ...
        private int Query(string mode, string SortExpression, string SortDire)
        {
            ...
            int intRet = (find / _pagesize) + 1;
            if (intRet > 0) intRet = intRet - 1;
            MainGridView.PageIndex = intRet;
            GirdViewDataBind(this.MainGridView, dt);
            UpdatePanel1.Update();
        ...
        protected static Pub.MessageObject Check_Input_DB(string[] NoArray, string mode)
        {
            ...
            {
                case "Insert":
                    sql = @" SELECT * FROM B01_OrgStruc WHERE OrgStrucNo = ? OR OrgIDList=? ";
        ...
        protected void GridView_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            switch (e.Row.RowType)
            {
                ...
                case DataControlRowType.DataRow:
                    ...
                    e.Row.Cells[2].Width = 154;
                    e.Row.Cells[3].Width = 374;
                    #endregion
                    ...
                    e.Row.Cells[1].Text = LimitText(e.Row.Cells[1].Text, 30, true);
                    e.Row.Cells[2].Text = LimitText(e.Row.Cells[2].Text, 30, true);
                    e.Row.Cells[3].Text = LimitText(e.Row.Cells[3].Text, 56, true);
            ...
            if (sRet.result)
            {
                ...
                if (istat > -1)
                {
                    if (oAcsDB.Commit())
                    {
                        #region 取得新資料ID
                        #region Process String
                        sql = " SELECT OrgStrucID,OrgNoList FROM OrgStrucAllData('') WHERE OrgStrucNo = ?";
                        liSqlPara.Clear();
                        liSqlPara.Add("S:" + sOrgStrucNo.Trim());                       
                        #endregion
                        ...
                        if (oAcsDB.GetDataReader(sql, liSqlPara, out oReader))
                        {
                            if (oReader.HasRows)
                            {
                                ...
                                sRet.message = sOrgStrucNo.Trim() + "|" + oReader.DataReader["OrgStrucID"].ToString()
                                    + "|" + oReader.DataReader["OrgNoList"].ToString();
        ...
        public static object Delete(string OrgStruc_ID, string ChangeOrgStrucID, String UserID)
        {
            ...
            if (sRet.result)
            {
                if (istat > -1)
                {
                    ...
                    if (ChangeOrgStrucID != "")
                    {
                        OrmDataObject odb = new OrmDataObject("MsSql", Pub.GetDapperConnString());
                        var para_data =new { OldOrgStruc = OrgStruc_ID,NewOrgStruc=ChangeOrgStrucID};
                        sql = @"UPDATE B01_Person SET OrgStrucID=@NewOrgStruc WHERE OrgStrucID=@OldOrgStruc ";
                        odb.Execute(sql,para_data);
                        sql = @"INSERT INTO B01_MgnOrgStrucs (MgaID,OrgStrucID,CreateUserID,CreateTime)
                                        SELECT MgaID,@NewOrgStruc,'Saho',GETDATE() FROM B01_MgnOrgStrucs WHERE 
	                                            OrgStrucID=@OldOrgStruc
	                                            AND MgaID NOT IN (SELECT MgaID FROM B01_MgnOrgStrucs WHERE OrgStrucID=@NewOrgStruc)";
                        odb.Execute(sql, para_data);
                        sql = @"INSERT INTO B01_OrgEquGroup (OrgStrucID,EquGrpID,CreateUserID,CreateTime)
                                        SELECT EquGrpID,@NewOrgStruc,'Saho',GETDATE() FROM B01_OrgEquGroup WHERE 
	                                            OrgStrucID=@OldOrgStruc
	                                            AND EquGrpID NOT IN (SELECT EquGrpID FROM B01_OrgEquGroup WHERE OrgStrucID=@NewOrgStruc)";
                        odb.Execute(sql, para_data);
                    }
        ...
        public static string[] LoadData(String Ctrl_ddl, String Ctrl_List, string OrgStruc_ID, String UserID, String mode)
        {
            ...
            #region Process String
            sql = @"SELECT O1.*,O2.OrgNoList FROM B01_OrgStruc O1
	                        INNER JOIN OrgStrucAllData('') O2 ON O1.OrgStrucID=O2.OrgStrucID WHERE O1.OrgStrucID = ? ";
            liSqlPara.Add("S:" + OrgStruc_ID.Trim());
            oAcsDB.GetDataReader(sql, liSqlPara, out dr);
            if (dr.Read())
            {
            ...
            if (mode == "Delete" && OnceState)
            {
                liSqlPara.Clear();
                dr = null;
                sql = @" SELECT * FROM OrgStrucAllData('') WHERE OrgStrucID <> ? ";
                liSqlPara.Add("S:" + EditData[0]);
                oAcsDB.GetDataReader(sql, liSqlPara, out dr);
                if (dr.HasRows)
                {
                    while (dr.Read())
                        EditData[EditData.Length - 2] += dr.DataReader[0].ToString() + "|(" + dr.DataReader[1].ToString() + ")"+dr.DataReader["OrgNameList"].ToString()+"|";
                    EditData[EditData.Length - 2] = EditData[EditData.Length - 2].Substring(0, EditData[EditData.Length - 2].Length - 1);
                }
            }
            #endregion
