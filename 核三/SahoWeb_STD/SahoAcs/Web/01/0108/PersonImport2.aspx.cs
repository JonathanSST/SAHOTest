...
namespace SahoAcs
{
    public partial class PersonImport2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ClientScript.RegisterClientScriptInclude("PersonImport", "PersonImport2.js");//加入同一頁面所需的JavaScript檔案
            ...
            if (IsSuccess)
            {
                for (int i = 0; i < PsnCard_set.Count; i++)
                {
                    ...
                    sSql += @" INSERT dbo.B00_SyncTable_PersonCard
                               (PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, Company, Unit, Department, Title, PsnAccount, PsnPW, PsnAuthAllow, PsnSTime, PsnETime, PsnPicSource, CardNo, CardVer, CardPW, CardSerialNo, Text1, Text2)
                               VALUES (" + sParaStr + ") ";
            ...
            if (PsnCardCorrectFlag && PsnTableCount > 0)
            {
                ...
                if (iRet > -1)
                {
                    sSql = @" INSERT INTO B01_Person(PsnNo, PsnName, PsnEName, PsnType, IDNum, Birthday, 
                                OrgStrucID, PsnAccount, PsnPW, PsnAuthAllow, PsnSTime, PsnETime, PsnPicSource, CreateUserID, CreateTime, Text1, Text2)
                              SELECT 
