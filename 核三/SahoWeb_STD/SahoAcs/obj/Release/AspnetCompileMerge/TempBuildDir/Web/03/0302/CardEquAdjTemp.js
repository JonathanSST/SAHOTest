...
function SelectAllCheckboxes(spanChk) {
    ...
    switch (spanChk) {
            ...
            for (var i = 0; i < elm.length; i++) {
                if (elm[i].id != spanChk1.id) {
                    ...
                    if (str[4] == "RowCheckState1") {
                        if (elm[i].checked != spanChk1.checked)
                            elm[i].click();
                    }
                }
            }
        ...
        case spanChk2:
            for (var i = 0; i < elm.length; i++) {
                if (elm[i].id != spanChk2.id) {
                    var str = elm[i].name.split('$');
                    if (str[4] == "RowCheckState2") {
                        if (elm[i].checked != spanChk2.checked)
                            elm[i].click();
                    }
                }
            }
        ...
        case spanChk3:
            for (var i = 0; i < elm.length; i++) {
                if (elm[i].id != spanChk3.id) {
                    var str = elm[i].name.split('$');
                    if (str[4] == "RowCheckState3") {
                        if (elm[i].checked != spanChk3.checked)
                            elm[i].click();
                    }
                }
            }
        ...
        case spanChk4:
            for (var i = 0; i < elm.length; i++) {
                if (elm[i].id != spanChk4.id) {
                    var str = elm[i].name.split('$');
                    if (str[4] == "RowCheckState4") {
                        if (elm[i].checked != spanChk4.checked)
                            elm[i].click();
                    }
                }
            }
...
function SetUI(DataArray) {
    if (DataArray[0] != 'Saho_SysErrorMassage') {
        ...
        //依Type判斷要取那個HiddenField
        var arrType;

        switch (DataArray[0]) {
            case "Card":
                arrType = hCard.value.split(",");
                break;
            case "Org":
                arrType = hOrg.value.split(",");
                break;
            case "No":
                arrType = hNo.value.split(",");
                break;
            case "Name":
                arrType = hName.value.split(",");
                break;
        }

        if (arrType.length > 0) {
            for (var i = 0, k = arrType.length; i < k; i++) {
                var data = arrType[i].split("|");
                var option = null;
                var state = 0;
                option = document.createElement("option");
                option.text = data[0];
...
function LoadCardData() {
    ...
    lblDataList.innerHTML = "資料列表：(共" + DataList.length + "筆)";
}
...
function ExecProc() {
    var sTime = Input_STime.value;
    var eTime = Input_ETime.value;
    ...
    PageMethods.ExecCardProc(sTime, eTime, hFinalEquData.value, datalist, hddlState.value, hFinalFloorData.value, hUserId.value, hSelectState.value, equtype, ShowMsg, onPageMethodsError);
...
function GridCount(NotAddCount, AddedCount){
    //lblNotAdd.innerHTML = "待加入(共" + NotAddCount + "筆)";
    //lblAdded.innerHTML = "已加入(共" + AddedCount + "筆)";
}
