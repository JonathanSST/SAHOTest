...
function ResetPwd() {
    hPsnId.value = SelectValue.value;
    PageMethods.PwdChangeProc(Input_PsnNo.value, hPsnId.value, function (sRet) {
        if (sRet.result) {
            alert("更新完成!!");
            Input_PsnPW.value = sRet.message;
        } else {
            alert(sRet.message);
        }
...
function SaveData(UserId) {
    ...
    if ($('[name$="Input_PsnAccount"]').val() === $('input[name*="IDNum"]').val()) {
        alert('帳號不得與身份證號相同');
        return false;
    }
    //帳密為必要輸入欄位，且要符合密碼原則
    if ($('[name$="Input_PsnAccount"]').val() === "" || $('[name$="Input_PsnPW"]').val().length < 12) {
        alert('帳密為必要輸入欄位，密碼長度必須為12碼以上大小寫英文、數字');
        return false;
    }
    var re = new RegExp("^((?=.{8,}$)(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).*|(?=.{8,}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!\u0022#$%&'()*+,./:;<=>?@[\]\^_`{|}~-]).*)");
    if (!re.test($('[name$="Input_PsnPW"]').val())) {
        alert('密碼長度必須為12碼以上大小寫英文、數字');
        return false;
    }
...
function SaveDataCheck(CheckData) {
    ...
    var re = /^[A-F0-9]+$/;
    if (re.test($('input[name*="Input_CardVer"]').val()) == false) {
        //版次為必要欄位
        alert("卡片版次為必要輸入欄位!!\n格式必須為數字!!");
        $('input[name*="Input_CardVer"]').focus();
        return false;
    }
    ...
    if (CheckData[4] === "false") {
        if (confirm("該組織架構不存在，該人員無預設權限，是否繼續處理?")) {
            ...
            if (SelectValue.value == '') {                
                    ...
                    OrgIDList, OrgNoList, "", Input_PsnAccount.value.trim(), Input_PsnPW.value.trim(),
                    ...
                    ddlVerifiMode.value, UserId, Input_CardVer.value.trim(), Excutecomplete, onPageMethodsError);
            }
            else {
                PageMethods.Update(SelectValue.value, SelectNowNo.value, Input_PsnNo.value.trim(), Input_PsnName.value.trim(),
                    ...
                    txt1, txt2, txt3, txt4, txt5,
                    ddlVerifiMode.value, UserId, Excutecomplete, onPageMethodsError);
            }
        }
    }
    else {
...
function SaveCardData(UserId) {
    var adm100setting = "";
    var re = /^[A-F0-9]+$/;
    if (re.test($('input[name*="popInput_CardVer"]').val()) == false) {
        //版次為必要欄位
        alert("卡片版次為必要輸入欄位!!\n格式必須為數字!!");
        $('input[name*="popInput_CardVer"]').focus();
        return false;
    }
    $("#ContentPlaceHolder1_GridView1").find("tr").each(function () {
    ...
    if (SelectCardValue.value == '') {
        ...
        PageMethods.CardUpdate(SelectValue.value, SelectCardValue.value, popInput_CardNo.value,
