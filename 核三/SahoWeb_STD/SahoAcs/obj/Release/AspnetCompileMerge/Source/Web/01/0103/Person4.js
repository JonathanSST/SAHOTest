...
function ShowPsnData(ResultArray) {    
    ...
    if (DataArray[0] != 'Saho_SysErrorMassage') {
        ...
        Input_Text1.value = DataArray[16];//文字一
        Input_Text2.value = DataArray[17];//文字二
        Input_Text3.value = DataArray[18];//文字三
        Input_Text4.value = DataArray[19];//文字四
        Input_Text5.value = DataArray[20];//文字五
...
function SaveDataCheck(CheckData) {
    ...
    if ($('input[name*="Input_PsnNo"]').val().length != 10) {
        alert("人員編號欄位必須為身份證字號!!");
        $('input[name*="Input_PsnNo"]').focus();
        return false;
    }
    if (CheckData[4] === "false") {
        if (confirm("該組織架構不存在，該人員無預設權限，是否繼續處理?")) {
            ...
            if (SelectValue.value == '') {                
                    ...
                    Input_Text1.value, Input_Text2.value.trim(), Input_Text3.value.trim(), Input_Text4.value.trim(),
                    Input_Text5.value.trim(), ddlVerifiMode.value, UserId, Input_CardVer.value.trim(), Excutecomplete, onPageMethodsError);                
            }
            else {
                PageMethods.Update(SelectValue.value, SelectNowNo.value, Input_PsnNo.value.trim(), Input_PsnName.value.trim(),
                    ...
                    Input_Text1.value, Input_Text2.value.trim(), Input_Text3.value.trim(), Input_Text4.value.trim(),
                    Input_Text5.value.trim(), ddlVerifiMode.value, UserId, Excutecomplete, onPageMethodsError);
            }
        }
    }
    else {
