...
function SetShowOneLog(recordid,table_type) {
    $.ajax({
        type: "POST",
        url: "QueryMeal0601.aspx",
        data: { "RecordID": recordid, "PageEvent": "QueryOneLog","TableType":table_type },
        dataType: "json",
...
function SetMode(mode) {
    ...
    if (mode == 1) {
        ...
        if ($('[name*="DropMgaList"]').val() != "") {
            $("#MgaName").val($('[name*="DropMgaList"] option:selected').text());
        } else {
            $("#MgaName").val("");
        }
        if ($('[name*="DropEquNo"]').val() != "") {
            $("#EquNo").val($('[name*="DropEquNo"]').val());
        } else {
            $("#EquNo").val("");
        }
    } else {
