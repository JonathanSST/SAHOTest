...
function SaveCardLog() {
    $.post("AddLog.aspx",
            { PageEvent: "Save", CardID: $('#CardID').val(), EquID: $('select[name*="EquName"]').val(), LogStatus: $('select[name*="LogStatus"]').val(), CardTime: $("#CalendarTextBox").val() },
             function (data) {
                 alert("新增完成!!");
                 $("#ParaExtDiv").remove();
                 $("#popOverlay").remove();
