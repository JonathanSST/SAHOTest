...
function SetMode(sMode) {
    switch (sMode) {
        ...
        case 'Edit':
            ...
            //Input_PsnNo.style.disabled = true;
            break;
        case '':
            //Input_PsnNo.disabled = true;
            //Input_CardNo.disabled = true;
...
function CallEdit(sNo) {    
    if (sNo != '') {
        ...
        SetMode('Edit');
        $('select[name*="List_CardInfo"]').dblclick(function () {
            CallCardEdit("卡片資料編輯");
        });
    }
}
...
function SaveData(UserId) {
    if ($("select[name*='dropOrg_C']").val() == "0") {
        alert("公司資料尚未選取");
        return false;
    }
    //生日組合
