$(document).ready(function () {
    ...
    //隱藏讀卡結果選單
    $("#ctl00_ContentPlaceHolder1_DropDownList_LogStatus_Panel2").css("display", "none")
    $("#ContentPlaceHolder1_QueryLabel_LogStatus").css("display", "none")
...
//初始化
let Cardid = "";
function SetCardStatus(recordid) {
    //記錄目前頁面的select的recordid 要更新db資料的key
    Cardid = recordid;
    $("#dialog-form").dialog({
        closeText: ""
    });
    // $("#select_op").empty();
    let ChkselectValue = $("#select_op").val();
    if (ChkselectValue == null) {
        $.ajax({
            type: "POST",
            url: "QueryCardlogTP.aspx",
            data: { "PageEvent": "QueryCardLogStatus" },
            dataType: "json",
            success: function (data) {
                if (data.message == "OK") {
                    $('#select_op').append("<option value='0'>--請選擇--</option>");
                    let Cardlog = JSON.parse(data.card_log);
                    $.each(Cardlog, function (index, item) {
                        var code = item.code;
                        var StateDesc = item.StateDesc;
                        //構造動態option
                        $('#select_op').append("<option value='" + code + "'>" + StateDesc + "</option>")
                    });
        ...
        $('select option:contains("--請選擇--")').prop('selected', true);
    }
}

function CardLogSave() {
    let Recordid = Cardid;
    let Selecttext = $('#select_op :selected').text();
    let SelectStatus = $('#select_op').val();
    if (SelectStatus != 0 && SelectStatus != null) {
        $.ajax({
            type: "POST",
            url: "QueryCardlogTP.aspx",
            data: { "Recordid": Recordid, "SelectStatus": SelectStatus, "PageEvent": "UpdateCardLogStatus" },
            dataType: "json",
            success: function (data) {
                if (data.message == "OK") {
                    alert("出勤狀態修改成功");
