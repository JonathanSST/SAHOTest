    ...
    if (CheckData[4] === "false") {
        if (confirm("該組織架構不存在，該人員無預設權限，是否繼續處理?")) {
            ...
            if (SelectValue.value == '') {                
                    ...
                    txt1,txt2,txt3,txt4,txt5, ddlVerifiMode.value, UserId, Input_CardVer.value.trim(), Excutecomplete, onPageMethodsError);                
            }
            else {
                PageMethods.Update(SelectValue.value, SelectNowNo.value, Input_PsnNo.value.trim(), Input_PsnName.value.trim(),
                    ...
                    txt1,txt2,txt3,txt4,txt5, ddlVerifiMode.value, UserId, Excutecomplete, onPageMethodsError);
            }
        }
    }
    else {
...
function ShowCardDefaultData(DataArray) {
    ...
    if (DataArray[0] != '') {        
        ...
        for (var i = 0; i < str.length; i += 3) {
            ...
            option.text = str[i + 1];
            option.value = str[i];
            if ($("#ItemCardType" + str[i]).length > 0) {
                $("#ItemCardType" + str[i]).remove();
            }
            $(document.body).append('<input type="hidden" id="ItemCardType' + str[i] + '" name="ItemCardType' + str[i] + '" value="' + str[i + 2] + '"  />');
            $("#ItemCardType" + str[i]).val(str[i + 2]);
            popInput_CardType.options.add(option);
        }
    }
    $('[name*="popInput_CardType"]').change(function () {
        $('[name*="popInput_CardNo"]:eq(0)').prop('maxlength', $("#ItemCardType" + $(this).val()).val());
    });
