...
function SaveData() {
    ...
    var pemail = document.getElementById("Personal").value;
    var demail = document.getElementById("Supervisor").value;
    if (pemail != "") {
        if (regex.test(pemail) != true) {
            alert("個人電子郵件格式錯誤");
            $("#PersonEmail").focus();
            return false;
        }
    ...
    if (JsFunBASE_VERTIFY() && checkend) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $.unblockUI();
                //沒有錯誤訊息再進行UI更新
                console.log($(data).find("#ErrResult").val());
                if ($(data).find("#ErrResult").val() == "False") {
