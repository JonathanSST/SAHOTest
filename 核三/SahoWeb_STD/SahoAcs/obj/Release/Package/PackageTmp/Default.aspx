...
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="SahoAcs.Default" EnableViewStateMac="true" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Saho Manage System(SMS)</title>
...
<body>
    <form id="form1" runat="server">
        ...
        <div class="BgLogin">
            ...
            <asp:Panel ID="Panel1" runat="server">
                <ul>
                    <li>
                        ...
                        <input type="text" id="txtUid" style="width:200px" name="txtUid" autocomplete="off" />
                    </li>
                    <li>
                        <asp:Label ID="lblPwd" runat="server" Text="<%$ Resources:Resource, lblPwd %>"></asp:Label>：                        
                        <input type="password" id="txtPwd" style="width:200px" name="txtPwd" autocomplete="off"/>
                        <input type="hidden" name="ClientToken" id="ClientToken" value="<%=this.ChkToken %>" />                       
                        <input type="hidden" name="PageEvent" id="PageEvent" value="Login" />                       
                    </li>
                </ul>
                <ul>
                    <li>
                        <div id="radio" runat="server">
                            <asp:RadioButton ID="rdoUser" runat="server" GroupName="User" Text="<%$ Resources:Resource, rdoUser %>" Checked="true" />
                            <asp:RadioButton ID="rdoPsn" runat="server" GroupName="User" Text="<%$ Resources:Resource, rdoPsn %>" />
                        </div>
                    </li>
                    <asp:Label ID="lblMessage" runat="server" ForeColor="Red" Font-Size="16" BackColor="White"></asp:Label>
                    <asp:LinkButton ID="btnRefresh" runat="server" Visible="false" OnClick="btnRefresh_Click" Text="重新整理"></asp:LinkButton>
                </ul>
                <h2>                    
                    <input type="button" class="BtnLogin" id="BtnLogint" value="<%=GetGlobalResourceObject("Resource","btnLogin") %>" onclick="DoLogin()"/>
                </h2>
                <div style="color: cornsilk; font-size: 20px; text-align: center;">                    
                    <%=Resources.Resource.lblBrowserAlarm %>
                </div>
            ...
            { %>
        <script type="text/javascript">
            /*$.get("AliveDate.aspx",function (data) { */
...
<%--         <%if (!DongleVaries.GetScAliveTime(DateTime.Now))
            { %>
        <script type="text/javascript">
            $.get("/Web/MessagePage/ScAliveMsg.aspx").success(function (data) {
                $(document.body).append('<div id="popOverlay" style="position:absolute; top:0; left:0; z-index:29999; overflow:hidden;'
              + ' -webkit-transform: translate3d(0,0,0);"></div>');
                $("#popOverlay").css("background", "#000");
                $("#popOverlay").css("opacity", "0.5");
