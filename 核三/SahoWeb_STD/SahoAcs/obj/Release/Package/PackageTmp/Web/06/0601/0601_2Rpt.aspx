<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="0601_2Rpt.aspx.cs" Inherits="SahoAcs.Web._06._0601._0601_2Rpt" %>
...
<body>
    ...
    <%
        string CardTimeS = "", CardTimeE = "";
        CardTimeS = Request["CardTimeS"];
        CardTimeE = Request["CardTimeE"];
        //if(Request.QueryString.AllKeys.Where(i=>i))
         %>
    <div runat="server" id="PrintArea">
        ...
        <table  style="border-collapse: collapse">
            <tr class="dataclass">
                      <td style="width: 16%; text-align: left;">刷卡時間
                      </td>
    ...
    <%
        StringBuilder sb = new StringBuilder(@"<html><style> .DataBar
                {
                    border-bottom-style:solid; border-bottom-width:1px; border-bottom-color:Black;width:70px;font-size:10pt;font-family:標楷體;
                } </style>
        ...
        if (this.ListLog.Count > 0)
        {
            ...
            {
                sb.Append("<div style='width:100%;text-align:center;font-family:標楷體;font-size:14pt;'>刷卡資料列印</div>");
                sb.Append(string.Format("<table style='width:100%;font-family:標楷體;font-size:10pt;'><tr><td style='width:50%'>刷卡時間區間：{0}~{1}</td></tr></table>",Request["CardTimeS"],Request["CardTimeE"], ""));
                sb.Append(string.Format("<table style='width:100%;font-family:標楷體;font-size:10pt;'><tr><td style='width:50%'>列印日期：{0:yyyy/MM/dd HH:mm:ss}</td><td>頁次：{1}</td></tr></table>",DateTime.Now,p));
                sb.Append("<p style='height:2px;font-size:2pt'>&nbsp;</p>");
                sb.Append(@"<table style='width:100%; border-collapse:collapse;border-spacing:0px 0px;font-family:標楷體;font-size:10pt;'>");                
                sb.Append("<tr><td class='DataBar' style='width:20%'>刷卡時間</td>");
        ...
        Response.AddHeader("content-disposition", "attachment;filename=讀卡資料查詢.pdf");
        Response.End();
         %>
    </form>
</body>
