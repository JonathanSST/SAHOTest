...
function AddExcute(UserID) {        
    SelectNowNo.value = $('[name*="popInput_CNo"]').val();
    $.ajax({
        url: location.href,
        type: "POST",
        data: {
            "CNo": $('[name*="popInput_CNo"]').val(), "CName": $('[name*="popInput_CName"]').val(), "PageEvent": "Add", "OBTime": $('[name*="popInput_OBTime"]').val(), "OETime": $('[name*="popInput_OETime"]').val(), 
            "WBTime": $('[name*="popInput_WBTime"]').val(), "WETime": $('[name*="popInput_WETime"]').val(), "BERange": $('[name*="popInput_BERange"]').val(), "WTMin": $('[name*="popInput_WTMin"]').val()
        },
        dataType:'json',
        success: function (data) {
            console.log(data);
...
function CallEdit(Title, Msg) {
    ...
    if (IsEmpty(SelectValue.value)) {
        ...
        $.ajax({
            url: location.href,
            type: "POST",
            data: {"PageEvent":"Edit", "CID":SelectValue.value },
            success: function (data) {
                var cdata = JSON.parse(data).cdata;
                ShowOver('1');
                popInput_CNo.value = cdata.CNo;
                popInput_CName.value = cdata.CName;               
                popInput_WBTime.value = cdata.WBTime;
                popInput_WETime.value = cdata.WETime;
                popInput_OBTime.value = cdata.OBTime;
                popInput_OETime.value = cdata.OETime;
                popInput_BERange.value = cdata.BERange;
                popInput_WTMin.value = cdata.WTMin;
...
function EditExcute(UserID) {
    ...
    $.ajax({
        ...
        type: "POST",
        data: {
            "CID": SelectValue.value, "CNo": $('[name*="popInput_CNo"]').val(), "CName": $('[name*="popInput_CName"]').val(), "PageEvent": "Update", "OBTime": $('[name*="popInput_OBTime"]').val(), "OETime": $('[name*="popInput_OETime"]').val(), 
            "WBTime": $('[name*="popInput_WBTime"]').val(), "WETime": $('[name*="popInput_WETime"]').val(), "BERange": $('[name*="popInput_BERange"]').val(), "WTMin": $('[name*="popInput_WTMin"]').val()
        },
        dataType: 'json',
        success: function (data) {
...
function DeleteExcute(UserID) {
    $.ajax({
        url: location.href,
        type: "POST",
        data: { 'CID': SelectValue.value, "PageEvent": "Delete" },
        dataType: 'json',
        success: function (data) {
            console.log(data);
            if (data.result) {
