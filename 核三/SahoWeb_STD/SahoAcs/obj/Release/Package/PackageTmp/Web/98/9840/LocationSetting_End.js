    ...
    if (event.srcElement.type != undefined) {
        ...
        if (noteItemType[parseInt(s)] != "NONE") {
            if (noteItemType[parseInt(s)] == "SMS") {
                document.getElementById('new_childItem').innerText = "新增區域";
            ...
            else if (noteItemType[parseInt(s)] == "AREA") {
                document.getElementById('new_childItem').innerText = "新增棟別";
                document.getElementById('del_childItem').innerText = "刪除區域";
                document.getElementById('new_childItem').style.display = "";
                document.getElementById('del_childItem').style.display = "";
                NodeAct.value = "AREA";
                RightClickItem.value = noteID[parseInt(s)];
            }
            else if (noteItemType[parseInt(s)] == "BUILDING") {
                document.getElementById('new_childItem').innerText = "新增樓層";
                document.getElementById('del_childItem').innerText = "刪除棟別";
                document.getElementById('new_childItem').style.display = "";
                document.getElementById('del_childItem').style.display = "";
                NodeAct.value = "BUILDING";
                RightClickItem.value = noteID[parseInt(s)];
            }
            else if (noteItemType[parseInt(s)] == "FLOOR") {
                document.getElementById('new_childItem').innerText = "";
                document.getElementById('del_childItem').innerText = "刪除樓層";
...
function clickNewMenu() {
    SetDivMode("Loc");
    SetLocMode("Add", NodeAct.value, RightClickItem.value);
}

// 點擊刪除目錄動作
function clickdelMenu() {
    SetDivMode("Loc");
    SetLocMode("Delete", NodeAct.value, RightClickItem.value);

    LoadLocInfo(RightClickItem.value);

    ShowConFirmDialog('確認刪除資料？資料刪除後無法恢復。').then(function (answer) {
        var ansbool = Boolean.parse(answer.toString());
        if (ansbool) {
            DeleteLocExcute(); return false; 
