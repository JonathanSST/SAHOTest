...
function ShowPsnData(ResultArray) {    
    ...
    if (DataArray[0] != 'Saho_SysErrorMassage') {
        ...
        var arrOrgIDList = DataArray[DataArray.length - 6].slice(1, DataArray[DataArray.length - 6].length - 1).split('\\');
        var arrOrgNoList = DataArray[DataArray.length - 5].slice(1, DataArray[DataArray.length - 5].length - 1).split('\\');
        var j = 0;        
        ...
        if (DataArray[DataArray.length - 4] != '') {
            str = DataArray[DataArray.length - 4].split('|');
        ...
        Input_PsnRule.selectedIndex = DataArray[DataArray.length - 3]; //允許遞迴
        Input_PsnLock.selectedIndex = DataArray[DataArray.length - 2]; //遞迴鎖定
        ...
        if (confirm("該組織架構不存在，該人員無預設權限，是否繼續處理?")) {
            ...
            if (SelectValue.value == '') {                
                    ...
                    Input_PsnETime.value, Input_PsnPicSource.value.trim(), Input_Remark.value,
                        txt1, txt2, txt3, txt4, txt5, ddlVerifiMode.value, UserId, Input_CardVer.value.trim(), Input_PsnRule.options[Input_PsnRule.selectedIndex].value, Input_PsnLock.options[Input_PsnLock.selectedIndex].value, Excutecomplete, onPageMethodsError);                
            }
            else {
                PageMethods.Update(SelectValue.value, SelectNowNo.value, Input_PsnNo.value.trim(), Input_PsnName.value.trim(),
                    ...
                    txt1, txt2, txt3, txt4, txt5, ddlVerifiMode.value, UserId, Input_PsnRule.options[Input_PsnRule.selectedIndex].value, Input_PsnLock.options[Input_PsnLock.selectedIndex].value, Excutecomplete, onPageMethodsError);
            }
        } else {
            $.unblockUI();
        }
