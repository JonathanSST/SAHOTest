...
function SetMode(sMode) {
    ...
    switch (sMode) {
        ...
        case 'Edit':
            ...
            btnMakeCard.style.display = "inline";
            ...
            $('input[name*="Input_PsnIDNum"]').attr('disabled', 'disabled');
        ...
        case '':
            ...
            Input_PsnNo.value = '';
            DefaultData();
            $('input[name*="Input_PsnIDNum"]').attr('disabled', false);
            break;
    }
...
function ShowPsnData(ResultArray) {
    ...
    if (DataArray[0] != 'Saho_SysErrorMassage') {
        ...
        Input_TrainingEDate.value = DataArray[21]; //訓練期限
        ...
        if (DataArray[DataArray.length - 2] != '') {
            ...
            for (var i = 0; i < str.length; i += 5) {
                ...
                Input_CardRule.selectedIndex = str[i + 3];
                Input_CardLock.selectedIndex = str[i + 4];
            }
...
function SaveData(UserId) {
    ...
    if ($('input[name*="Input_PsnIDNum"]').attr("MUST_KEYIN_YN") == "Y" && $('input[name*="Input_PsnIDNum"]').prop("maxlength") != $('input[name*="Input_PsnIDNum"]').val().length) {
        alert("身份證號長度必須為" + $('input[name*="Input_PsnIDNum"]').prop("maxlength") + "\n身份證號為必填");
        $('input[name*="Input_PsnIDNum"]').focus();
        return false;
    }
    //生日組合
        ...
        if (confirm("該組織架構不存在，該人員無預設權限，是否繼續處理?")) {
            ...
            if (SelectValue.value == '') {
                PageMethods.Insert(Input_PsnNo.value.trim(), Input_PsnName.value.trim(), Input_PsnEName.value.trim(),
                    ...
                    txt1, txt2, txt3, txt4, txt5, ddlVerifiMode.value, UserId, Input_CardVer.value.trim(),
                    Input_CardRule.options[Input_CardRule.selectedIndex].value,
                    Input_CardLock.options[Input_CardLock.selectedIndex].value,
                    Input_TrainingEDate.value.trim(), Excutecomplete, onPageMethodsError);
            }
            else {
                ...
                PageMethods.Update(SelectValue.value, SelectNowNo.value, Input_PsnNo.value.trim(), Input_PsnName.value.trim(),
                    ...
                    txt1, txt2, txt3, txt4, txt5, ddlVerifiMode.value, UserId,
                    Input_CardRule.options[Input_CardRule.selectedIndex].value,
                    Input_CardLock.options[Input_CardLock.selectedIndex].value,
                    Input_TrainingEDate.value.trim(), Excutecomplete, onPageMethodsError);
            }
    ...
    if (DataArray[0] != 'Saho_SysErrorMassage') {
        ...
        if (DataArray[DataArray.length - 1] != '') {
            ...
            for (var i = 0; i < str.length; i += 2) {
                ...
                if (str[i + 1].indexOf("ADM100FP") > 0 || str[i + 1].indexOf("SST9500FP") > 0) {
...
//呼叫製卡程式
function CellMakeCard() {
    console.log("A");
    console.log(SelectValue.value);

    PageMethods.CellMakeCard(SelectValue.value, MakeCardExcute, onPageMethodsError);
}

function MakeCardExcute(sRet) {
    console.log("B");
    console.log(SelectValue.value);
    console.log(sRet);
    $.unblockUI();
