...
{
    public partial class DateTimeSelect : System.Web.UI.Page
    {
        public string DateTimeNow = Pub.GetNowTime;
        protected void Page_Load(object sender, EventArgs e)
        {
            ClientScript.RegisterClientScriptInclude("DateTimeSelect", Request.ApplicationPath.TrimEnd('/')+"/DialogBox/DateTimeSelect.js");//加入同一頁面所需的JavaScript檔案                        
            if (Request.Form["para_date"] != null)
            {
                this.DateTimeNow = Request.Form["para_date"].ToString();
            }
            if (!IsPostBack)
            {
